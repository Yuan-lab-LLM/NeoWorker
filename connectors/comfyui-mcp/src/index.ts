...
function projectRoot(): string {
  ...
  if (!raw.trim()) {
    throw new Error("NEOWORKER_ARCH_PROJECT_ROOT or NEOWORKER_WORKSPACE_ROOT is required for ComfyUI file path tools.");
  }
...
async function submitWorkflow(args: Record<string, any>): Promise<any> {
  const body = {
    ...
    client_id: args.clientId || `neoworker-${Date.now()}`,
  };
