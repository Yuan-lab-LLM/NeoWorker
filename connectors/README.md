...
Connectors are MCP servers that expose enterprise APIs (Salesforce, Jira, etc.) to NeoWorker via tools. They are designed to run outside the desktop app so they can be deployed locally or as a managed service.
...
(`NEOWORKER_ARCH_PROJECT_ROOT` or `NEOWORKER_WORKSPACE_ROOT`) before dispatching to
