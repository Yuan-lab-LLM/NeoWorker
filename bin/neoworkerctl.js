...
function usage() {
  console.error(
    [
      'Usage:',
      '  node bin/neoworkerctl.js [--url <ws://host:port>] [--token <token>] call <method> [paramsJson]',
      '  node bin/neoworkerctl.js [--url <ws://host:port>] [--token <token>] watch [--event <name>] [--task <taskId>] [--pretty]',
      '  node bin/neoworkerctl.js [--url <ws://host:port>] [--token <token>] tail <taskId> [--limit <n>] [--pretty]',
      '',
      'Env:',
      ...
      '',
      'Examples:',
      '  node bin/neoworkerctl.js --token $TOKEN call workspace.list',
      '  node bin/neoworkerctl.js --token $TOKEN call workspace.create \'{"name":"main","path":"/workspace"}\'',
      '  node bin/neoworkerctl.js --token $TOKEN call approval.list',
      '  node bin/neoworkerctl.js --token $TOKEN call task.create \'{"title":"Test","prompt":"Say hi","workspaceId":"<id>"}\'',
      '  node bin/neoworkerctl.js --token $TOKEN call task.events \'{"taskId":"<id>","limit":200}\'',
      '  node bin/neoworkerctl.js --token $TOKEN call approval.respond \'{"approvalId":"...","approved":true}\'',
      '  node bin/neoworkerctl.js --token $TOKEN call config.get',
      '  node bin/neoworkerctl.js --token $TOKEN call channel.list',
      '  node bin/neoworkerctl.js --token $TOKEN call channel.create \'{"type":"telegram","name":"telegram","config":{"botToken":"..."},"securityConfig":{"mode":"pairing"}}\'',
      '  node bin/neoworkerctl.js --token $TOKEN call channel.test \'{"channelId":"..."}\'',
      '  node bin/neoworkerctl.js --token $TOKEN call channel.enable \'{"channelId":"..."}\'',
      '  node bin/neoworkerctl.js --token $TOKEN watch --event task.event',
      '  node bin/neoworkerctl.js --token $TOKEN tail <taskId>',
    ].join('\n')
  );
}
...
async function main() {
  ...
  const url = getFlagValue(argv, '--url') || process.env.NEOWORKER_CONTROL_PLANE_URL || 'ws://127.0.0.1:18789';
  const token = getFlagValue(argv, '--token') || process.env.NEOWORKER_CONTROL_PLANE_TOKEN || '';
  const deviceName = getFlagValue(argv, '--device-name') || 'neoworkerctl';
  ...
  if (!token) {
    console.error('Missing token. Provide --token or set NEOWORKER_CONTROL_PLANE_TOKEN.');
    process.exitCode = 1;
    return;
  }
