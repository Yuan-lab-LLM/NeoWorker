...
// Apply NEOWORKER_TZ → TZ before any other code (systemd/Docker env support).
// Validate: invalid IANA timezone can cause silent date bugs.
if (process.env.NEOWORKER_TZ) {
  try {
    const test = new Date().toLocaleString("en-US", { timeZone: process.env.NEOWORKER_TZ });
    if (test && test !== "Invalid Date") {
      process.env.TZ = process.env.NEOWORKER_TZ;
    } else {
      console.warn(`[neoworkerd] Invalid NEOWORKER_TZ='${process.env.NEOWORKER_TZ}', using default`);
    }
  } catch {
    console.warn(`[neoworkerd] Invalid NEOWORKER_TZ='${process.env.NEOWORKER_TZ}', using default`);
  }
...
/**
 * neoworkerd: Headless daemon entrypoint.
 *
 * Defaults:
 * - headless (no UI)
...
async function main() {
  ...
  if (hasFlag(argv, '-h') || hasFlag(argv, '--help')) {
    ...
    console.log([
      'NeoWorker daemon (headless)',
      '',
      'Usage:',
      '  node bin/neoworkerd.js [electronArgs...]',
      '',
      ...
      '',
      'Common env vars:',
      '  NEOWORKER_USER_DATA_DIR=/var/lib/neoworker',
      '  NEOWORKER_CONTROL_PLANE_HOST=127.0.0.1',
      '  NEOWORKER_CONTROL_PLANE_PORT=18789',
      '  NEOWORKER_LLM_PROVIDER=openai',
      '  OPENAI_API_KEY=...',
      '',
      'Examples:',
      '  node bin/neoworkerd.js --print-control-plane-token',
    ].join('\n'));
    return;
  }
  ...
  if (!fs.existsSync(mainPath)) {
    // eslint-disable-next-line no-console
    console.log('NeoWorker: Building (electron + connectors)...');
    try {
  ...
  if (!ok) {
    // eslint-disable-next-line no-console
    console.log('NeoWorker: Rebuilding native deps for Electron (better-sqlite3)...');
    try {
