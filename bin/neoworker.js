...
function buildAppAndLaunch() {
  console.log('[neoworker] Build artifacts not found, running npm run build...');
  ...
  build.on('exit', (code) => {
    if (code !== 0) {
      console.error(
        '[neoworker] Build failed. The installed package may be incomplete. ' +
...
if (fs.existsSync(mainPath) && fs.existsSync(rendererIndexPath)) {
  ...
  if (!fs.existsSync(mainPath)) {
    console.log('[neoworker] Main process build artifacts are missing.');
  }
  if (!fs.existsSync(rendererIndexPath)) {
    console.log('[neoworker] Renderer build artifacts are missing.');
  }
  buildAppAndLaunch();
}
...
function ensureRuntimeDeps() {
  ...
  console.log(
    `[neoworker] Missing runtime dependencies detected (${missing.length}); repairing install...`
  );
...
function runNativeSetup(onDone) {
  ...
  if (!fs.existsSync(setupScript)) {
    console.error('[neoworker] Missing native setup script at scripts/setup_native.mjs');
    process.exit(1);
  }

  console.log('[neoworker] Preparing native modules for Electron (first run)...');
  ...
  setup.on('exit', (code, signal) => {
    if (signal) {
      ...
      console.error(
        `[neoworker] Native setup was terminated (${signal}).` +
          (signal === 'SIGKILL'
...
function prepareAndLaunchApp() {
  if (!ensureRuntimeDeps()) {
    console.error('[neoworker] Failed to repair runtime dependencies.');
    process.exit(1);
  }
  ...
  const launchAfterSetup = () => {
    ...
    if (!electronBinary) {
      console.error(
        '[neoworker] Electron runtime is still missing after setup. Reinstall with:\n' +
          '  npm install --ignore-scripts --omit=optional --no-audit --no-fund neoworker@latest\n'
      );
      process.exit(1);
    }
  ...
  if (!electronBinary) {
    console.log('[neoworker] Electron runtime is missing. Running setup...');
    runNativeSetup(() => launchAfterSetup());
    return;
  }
