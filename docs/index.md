...
features:
  ...
  - title: 149 Built-in Skills
    details: Document creation, web research, code generation, image analysis, React/Next.js implementation guidance, and specialized bundled workflows such as `llm-wiki` for persistent research vaults, `architecture-design` for Rhino/Blender/ComfyUI concept architecture workflows, `kami` for editorial PDFs and slide decks, `react-best-practices` for React workspace changes, `unbroker` for consent-gated data-broker cleanup, and `taste-skill` for high-agency frontend design. The bundled NeoWorker Shortcuts pack adds slash-searchable workflow aliases on top of the same skills runtime. Extensible via custom skills, ClawHub installs, external skill imports, and optional read-only external skill directories, with staged scanning and warning/quarantine handling for managed imports.
