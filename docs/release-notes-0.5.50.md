...
Release `0.5.50` brings NeoWorker's largest post-`0.5.49` capability update: a main-screen Automation Studio and deterministic workflow runtime, GPT-5.6 subscription controls, Mixture of Agents orchestration, browser annotations, inline email review, video understanding, stronger browser/computer-use approvals, new Calendar and architecture connectors, governed memory writes, session retention controls, and a bundled privacy-cleanup workflow.
...
- Fixed ChatGPT OAuth token exchange and added Browser Workbench annotations.
- Added architecture orchestration connectors, Google Calendar tools, and video analysis.
- Hardened build-health evidence checks, memory-write governance, and visible-use approvals.
- Added Mixture of Agents, inline mail compose frames, session archiving, and Automation Studio workflows.
