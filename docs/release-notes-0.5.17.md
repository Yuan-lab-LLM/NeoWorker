...
This page summarizes the product changes included in `0.5.17`, based on changes merged after `v0.5.14` on 2026-03-29.

## Overview

The 0.5.17 release adds a visible operator-facing runtime layer for learning, recall, shell state, and model routing; introduces a governed Discord supervisor loop for bot-to-bot review and escalation; opens the Skill Store to ClawHub and other external skill sources; and hardens mailbox/email flows around Microsoft OAuth, encryption at rest, and safer mailbox actions. It also refreshes Mission Control with an all-workspaces operator view, folds Dispatch onboarding into Devices, tightens Inbox Agent ergonomics, fixes the tag-blocking release-gate issue around empty eval corpora in ephemeral CI databases, stabilizes macOS release validation around shell/electron test isolation, and broadens the docs/comparison set so the new runtime surfaces are easier to understand and ship.
...
### Release automation and validation reliability
...
- **Test-runtime isolation**: shell command unit tests no longer import the full Electron daemon/runtime graph just to satisfy type references, which avoids spurious Electron bootstrap failures during release validation.
- **CI test stability**: shell approval mocks are reset between cases, unit tests stub persistent shell execution explicitly, tool-group risk metadata now matches the enforced security invariants, and the shell-session integration test has an explicit timeout budget for slower CI shells.
...
- This page is the canonical summary for the changes included in `0.5.17`.
