...
Check the environment from the same shell that launches NeoWorker:
...
Use `npm run dev`, not `npm run dev:react`, so the renderer receives the Electron preload APIs and platform flags. Restart NeoWorker after moving between a native Linux session and WSL. If neither WSL signal is present, native-frame mode is not selected automatically.
