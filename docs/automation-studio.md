...
Automation Studio is NeoWorker's visual builder and deterministic runtime for structured flows. Open it from **Automations** in the main sidebar. It is a main-screen product surface, not a Settings page.
...
- Describe the outcome. Generation is deliberately conservative: NeoWorker deterministically matches a known template when confidence is sufficient. Otherwise it creates a manual-starter draft with one bounded agent step. It does not invent unknown connector arguments or activate the result.
...
- open a backing NeoWorker task created by an AI or agent step.
...
- NeoWorker AI summary, extraction, decision, recap, and prompt actions;
- bounded NeoWorker agent tasks;
...
- Cancellation aborts active Google API requests, uploads, signed webhooks, and backing NeoWorker agent tasks, then marks unfinished steps cancelled.
...
Signing secrets are stored through NeoWorker's secure settings repository, which uses operating-system-backed encryption. The renderer receives only the secret id, name, timestamps, and configured state. Secret values are not returned to the renderer or embedded in workflow definitions, run inputs, or activity output.

A secret value is limited to 16 KiB. NeoWorker refuses to remove a secret referenced by an enabled flow's active version. Turn off every listed flow first, remove or replace the reference, and validate before turning the flow on again.
...
- includes `X-NeoWorker-Timestamp`, `X-NeoWorker-Signature`, and `X-NeoWorker-Idempotency-Key`.
...
Do not delete the NeoWorker database. `RoutineService.ensureSchema()` adds legacy routine-run columns before creating indexes. Capture `logs/dev-latest.log`, confirm the current Electron build is running, and inspect the migration error before considering any destructive recovery.
