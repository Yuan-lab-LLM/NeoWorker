# I Gave NeoWorker Workflow Intelligence, And Now It Learns From Reviewable Work | Full Guide
...
What we actually built in NeoWorker is a core runtime that learns from traces, distills memory, clusters recurring failures, generates evals, proposes experiments, and only promotes changes after a gate.

In other words:

we did not build a vague "self-improving AI" story.

We gave NeoWorker Workflow Intelligence: a governed loop that turns memory, heartbeat signals, reflection, Dreaming, and user-reviewed suggestions into durable learning.
...
And that is what lets NeoWorker improve without turning the whole product into an opaque autopilot.

For NeoWorker, continual learning happens across three layers:

- the `model` layer
- the `harness` layer
- the `context` layer

The important shift is that NeoWorker does not treat all three layers the same way.

## The Short Version

NeoWorker is deliberately conservative at the model layer and aggressive at the harness and context layers.
...
In practice, NeoWorker's continual-learning story is:
...
NeoWorker is model-agnostic. It supports many providers and local models, but it does not depend on hidden weight mutation to improve your runtime over time.
...
So NeoWorker treats model learning as optional and external. You can swap providers, change models, or run local models, but the core product does not promise “we secretly retrain the model for you.”

Instead, NeoWorker puts most of its learning investment in the layers you can inspect and govern.

### 2. Harness Learning

Harness learning is how the runtime itself improves.

In NeoWorker, that means improving the operating system around the model:
...
That hard boundary matters because it gives NeoWorker one narrow place where learning is allowed to accumulate and improve the system.

### 3. Context Learning

Context learning is the most practical layer for production agents.

This is where NeoWorker updates the durable knowledge around the runtime rather than the model weights themselves.

In NeoWorker, context learning includes:
...
The attached continual-learning paper makes one point that maps directly to NeoWorker:

traces are the core.

That is exactly how NeoWorker is designed.
...
NeoWorker turns those traces into structured runtime assets instead of leaving them as dead logs.

That is the difference between “history” and “learning.”

---

## How NeoWorker Learns In Practice
...
NeoWorker uses both styles of context learning:
...
NeoWorker treats failures as first-class learning input.
...
### 4. NeoWorker does not auto-mutate itself without a gate

This is where NeoWorker differs from a lot of “self-improving agent” narratives.
...
NeoWorker solves that by giving core-created tasks a real autonomy policy rather than just disabling user input.
...
NeoWorker exposes the learning loop in Mission Control through the `Core Harness` surfaces:
...
That is why NeoWorker made the hard cut:
...
## What NeoWorker Is Actually Optimizing For

NeoWorker is not trying to be a magical black box that “becomes conscious” over time.
...
## The NeoWorker Position In One Sentence

NeoWorker treats continual learning as a trace-native operating-system problem, not just a model-training problem.
...
That is the real learning loop in NeoWorker.
