---
title: "NeoWorker: A Beginner's Guide"
description: A practical guide to what NeoWorker is, how it differs from chat-only AI tools, how to get started, and which workflows to try first.
---

# NeoWorker: A Beginner's Guide
...
The first question people ask after installing NeoWorker is simple:

> What should I actually do with it?

That is a fair question. NeoWorker can look like a lot at first because it is not just a chatbot, a coding assistant, or an automation tool. It is a runtime that can combine all three. This guide is here to make the first hour feel obvious.

The goal is not to teach every setting. It is to help you understand what NeoWorker is good at, what it should not be trusted with blindly, and which workflows are worth trying first.

## The basics

### What is NeoWorker?

NeoWorker is a local-first AI operating system for real work.
...
If ChatGPT feels like “ask a smart model a question,” NeoWorker is closer to “give an operator a job, tools, memory, and rules.”

### How is it different from ChatGPT, Claude, or Codex?

The difference is not just the model. It is the operating model.

Chat-first tools are great when you want an answer, a draft, or a quick burst of reasoning. NeoWorker is better when you want the system to stay with the work.
...
In short: most AI tools are conversations. NeoWorker is a governed execution environment.

### What can it access?

By default, NeoWorker is only as capable as the permissions and connections you give it.
...
NeoWorker is powerful, but it is not magic.
...
## What it feels like to work with NeoWorker
...
NeoWorker reduces that context switching. You can create a task in the desktop app, start from an Ideas prompt, operate through a channel, or run it from a VPS control plane. The task, the outputs, the approvals, and the memory all stay in one operating surface.
...
This is where NeoWorker starts to feel less like “generate text” and more like “run the job.”

### It can become a background operator

Once your setup is stable, NeoWorker does not have to wait for you to start every task manually.
...
NeoWorker is especially strong when work crosses channels, people, and systems.
...
If you want NeoWorker running continuously without depending on your laptop, use the packaged Linux server release. It is a Linux x64 tarball from GitHub Releases that runs `neoworkerd-node`, serves the Control Plane Web UI, and can be installed as a systemd service.
...
This teaches you the most important mental model: NeoWorker is not just replying. It is inspecting, planning, executing, and stopping when approval matters.
...
Inbox work is where NeoWorker starts paying rent quickly.
...
This is one of the clearest examples of NeoWorker acting like a durable operating environment rather than a stateless chat window.
...
If you want work to run on another machine, use the Devices tab and connect a remote NeoWorker node. This is useful for persistent environments, isolated workloads, and automation that should not depend on your main laptop.

### 3. Build a governed company loop

If you are a founder or operator, NeoWorker can go beyond personal assistance into company execution.

That usually means combining:

- a git-backed workspace
- company files in `.neoworker/`
...
People get the most value out of NeoWorker when they shift from a chatbot mindset to an operator mindset.
...
NeoWorker is designed with stronger controls than most agent tools, but the controls only help if you use them well.
...
NeoWorker is best when you use it to operate ongoing work with clear boundaries, not when you use it as a fancier search box.
