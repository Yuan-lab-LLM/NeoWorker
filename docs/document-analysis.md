...
NeoWorker has a bounded, read-only review path for directly referenced long
...
If the request does not resolve to one exact path, NeoWorker ranks supported files
...
- hidden directories;
- `.git`, `node_modules`, `dist`, `build`, and `release`.
...
If final synthesis times out after all chunks were analyzed, NeoWorker preserves
