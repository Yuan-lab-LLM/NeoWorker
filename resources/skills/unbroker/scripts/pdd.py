...
def cmd_doctor(args) -> None:
    ...
    cfg = config_mod.load_config()
    caps = config_mod.detect_capabilities(config_mod.dotenv_env())  # see creds in the runtime .env too
    ...
    rows = [
        ...
        ("Email auto (AgentMail)", caps["agentmail"],
         "send + auto-verify, per-broker aliases (Mode B/C)", "set AGENTMAIL_API_KEY if using AgentMail"),
    ...
    if engaged:
        L.append(f"        key: {crypto.identity_path()} (0600) - guards casual/backup/commit "
                 "exposure, NOT a full runtime-home read")
