...
function createMockGateway() {
  ...
  return {
    ...
    resolveWhatsAppAuthDir(channel?: { config?: { authDir?: string } }): string {
      ...
      return "/tmp/test-neoworker/whatsapp-auth";
    },
...
describe("ChannelGateway Cleanup", () => {
  ...
  describe("resolveWhatsAppAuthDir", () => {
    ...
    it("should return default path if authDir is empty", () => {
      ...
      expect(result).toBe("/tmp/test-neoworker/whatsapp-auth");
    });
  ...
  describe("clearWhatsAppAuthDir", () => {
    it("should remove auth directory if it exists", () => {
      ...
      expect(fs.existsSync).toHaveBeenCalledWith("/tmp/test-neoworker/whatsapp-auth");
      expect(fs.rmSync).toHaveBeenCalledWith("/tmp/test-neoworker/whatsapp-auth", {
        recursive: true,
        force: true,
      });
    });
