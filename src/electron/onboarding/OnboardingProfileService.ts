...
import { DEFAULT_ASSISTANT_NAME, type UserFact } from "../../shared/types";
import {
...
const EMPTY_ONBOARDING_PRIORITIES_LINE =
  "- No explicit onboarding priorities recorded yet.";
...
function sectionBlock(marker: string, heading: string, body: string): string {
  return [
    heading,
    `<!-- neoworker:auto:${marker}:start -->`,
    body.trim(),
    `<!-- neoworker:auto:${marker}:end -->`,
  ].join("\n");
}
...
function upsertAutoSection(
  ...
  const regex = new RegExp(
    `## ${heading.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\n<!-- neoworker:auto:${marker}:start -->[\\s\\S]*?<!-- neoworker:auto:${marker}:end -->`,
    "m",
  );
...
async function writeManagedKitDoc(
  ...
  const existing = fs.existsSync(absPath)
    ...
    : withKitFrontmatter(
        relPath,
        ensureDefaultMarkdown(path.basename(relPath)),
        updated,
      );
  ...
  const nextBody = upsertAutoSection(
...
export class OnboardingProfileService {
  static applyGlobalProfile(data: OnboardingProfileData): void {
    ...
    for (const fact of existingFacts) {
      if (
        fact.lastTaskId === ONBOARDING_FACT_TASK_ID ||
        isLegacyOnboardingFact(fact)
      ) {
  ...
  static async applyWorkspaceProfile(
    ...
    await writeManagedKitDoc(
      ...
      [
        ...
        data.userContext.trim()
          ? `- Current focus: ${data.userContext.trim()}`
          : "",
    ...
    await writeManagedKitDoc(
      ...
      "Onboarding Identity",
      [
        `- Name: ${data.assistantName.trim() || DEFAULT_ASSISTANT_NAME}`,
        `- Role: NeoWorker operator for this workspace`,
