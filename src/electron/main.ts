import { LEGACY_TASK_DEEPLINK_PROTOCOLS } from "./migrations/legacy-brand-compat";
import { migrateLegacyWorkspaceKits } from "./migrations/legacy-workspace-kit";
...
import { resolveSafePackagedWorkingDirectory } from "./utils/startup-working-directory";

// Put NeoWorker's embedded OfficeCLI ahead of system installations before
// skills, shell sessions, and document tools are initialized.
installBundledOfficeCliRuntime();
void checkOfficeCliHealth().then((health) => {
  if (!health.ready) {
    console.warn(`[Office tools] ${health.diagnosticCode}: ${health.message}`);
  }
});
...
import { AsyncShutdownCoordinator } from "./utils/async-shutdown-coordinator";
import {
...
} from "./web-preview/web-preview-protocol";
...
function shouldAutoEnableDesktopControlPlane(): boolean {
  ...
  const raw = (process.env.NEOWORKER_DESKTOP_AUTO_CONTROL_PLANE || "")
    .trim()
    .toLowerCase();
  return raw === "1" || raw === "true" || raw === "yes" || raw === "on";
}

const logStartupLane = (
  lane: string,
...
function buildDefaultAutomationProfile(
  ...
  return {
    ...
    dispatchCooldownMinutes:
      ...
      (role.autonomyLevel === "lead" ? 90 : 120),
    maxDispatchesPerDay:
      role.maxDispatchesPerDay || (role.autonomyLevel === "lead" ? 6 : 4),
    profile:
      role.heartbeatProfile ||
      (role.autonomyLevel === "lead" ? "dispatcher" : "observer"),
    activeHours: role.activeHours,
  };
}
...
function ensureCoreAutomationProfiles(): void {
  ...
  const existingProfiles = new Map(
    automationProfileRepo
      .listAll()
      .map((profile) => [profile.agentRoleId, profile]),
  );
  ...
  logger.info("Core automation profiles ready", {
    ...
    enabledRoles: enabledProfiles
      .map(
        (profile) =>
          agentRoleRepo.findById(profile.agentRoleId)?.name ||
          profile.agentRoleId,
      )
      .slice(0, 10),
  });
...
function getDevServerUrl(): string {
  const configured = String(process.env.NEOWORKER_DEV_SERVER_URL || "").trim();
  if (configured.length > 0) {
    return configured;
  }

  const port =
    String(process.env.NEOWORKER_DEV_SERVER_PORT || "5173").trim() || "5173";
...
function healResettableSecureSettings(): void {
  ...
  const preserveLegacyEncryptedSettings = fsSync.existsSync(
    path.join(getUserDataDir(), ".legacy-brand-migration.json"),
  );
  for (const category of RESETTABLE_SECURE_SETTINGS_CATEGORIES) {
    logger.debug(`Checking secure settings recovery for ${category}`);
    const status = repository.checkHealth(category, { logErrors: false });
    logger.debug(`Secure settings recovery status for ${category}: ${status}`);
    if (status === "decryption_failed" || status === "checksum_mismatch") {
      if (preserveLegacyEncryptedSettings && status === "decryption_failed") {
        logger.warn(
          `Preserved legacy encrypted secure settings category ${category}; it requires one-time credential migration.`,
        );
        continue;
      }
  ...
  logger.debug("Secure settings recovery checks complete");
}
...
const TASK_DEEPLINK_PROTOCOL = "neoworker";
const ACCEPTED_TASK_DEEPLINK_PROTOCOLS = new Set([
  TASK_DEEPLINK_PROTOCOL,
...
function parseTaskDeeplink(value: string): string | null {
  ...
  try {
    ...
    if (
      !ACCEPTED_TASK_DEEPLINK_PROTOCOLS.has(parsed.protocol.replace(/:$/, ""))
    )
      return null;
...
applyApplicationIdentity();
const stableUserDataDir = applyStableUserDataPath();
const hardwareAccelerationDisabled = shouldDisableHardwareAcceleration({
  userDataDir: stableUserDataDir,
});
if (hardwareAccelerationDisabled) {
  // This must run before app.ready. A recent GPU subprocess crash enables the
  // safe path automatically; support can also opt in/out through environment flags.
  app.disableHardwareAcceleration();
  logger.warn("Hardware acceleration disabled for desktop runtime stability");
}
app.on("child-process-gone", (_event, details) => {
  const processType = String(details.type || "unknown");
  logger.error(
    `Electron child process gone: type=${processType}, reason=${details.reason}, exitCode=${details.exitCode}`,
  );
  appendRuntimeDiagnostic(stableUserDataDir, {
  ...
  if (processType.toLowerCase() === "gpu") {
    recordGpuProcessCrash(stableUserDataDir);
  }
});
const safePackagedWorkingDirectory = resolveSafePackagedWorkingDirectory({
  isPackaged: app.isPackaged,
  cwd: process.cwd(),
  userDataDir: stableUserDataDir,
  platform: process.platform,
});
if (safePackagedWorkingDirectory) {
  try {
    fsSync.mkdirSync(safePackagedWorkingDirectory, { recursive: true });
    process.chdir(safePackagedWorkingDirectory);
    logger.info(
      `Using safe packaged working directory: ${safePackagedWorkingDirectory}`,
    );
  } catch (error) {
    logger.warn("Failed to apply safe packaged working directory:", error);
  }
}

const CLI_DIRECT_RUN_FLAG = "--neoworker-cli-direct-run";
const CLI_APPROVAL_RESPONSE_FLAG = "--neoworker-cli-approval-response";
...
async function runCliDirectMode(): Promise<void> {
  try {
    ...
    if (!fsSync.existsSync(directRunPath)) {
      throw new Error(
        `NeoWorker CLI runtime is missing at ${directRunPath}. Run npm run build:cli or reinstall NeoWorker.`,
      );
    }
    // oxlint-disable-next-line typescript-eslint(no-require-imports)
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const directRun = require(directRunPath) as {
...
if (isCliDirectRunMode()) {
  ...
  // Ensure only one NeoWorker instance runs at a time.
  ...
  if (!gotTheLock) {
    ...
    if (startupApprovalResponse) {
      logger.warn(
        "CLI approval response requested, but no existing NeoWorker instance accepted it",
      );
      app.exit(1);
    }
    ...
    function createWindow() {
      ...
      // Resolve the solid native window background before creating the window.
      let windowBgColor = "#1a1a1c";
      try {
        const saved = AppearanceManager.loadSettings();
        windowBgColor = resolveOpaqueWindowBackground(
...
<head>
  ...
  <title>NeoWorker - Installation Error</title>
  <style>
...
<body>
  <div class="wrap">
    <h1>NeoWorker could not load UI assets</h1>
    ...
    <pre>${rendererIndex}</pre>
      ...
      mainWindow.webContents.on("did-finish-load", () => {
        // Chromium reapplies the remembered per-origin zoom while loading.
        // Reset it after navigation so the desktop UI opens at its designed scale.
        ...
        if (process.env.NODE_ENV === "development") {
          ...
          setTimeout(() => {
            ...
            void contents
              ...
              .then((snapshot) => {
                logger.info(`[RendererDomProbe] ${snapshot}`);
              })
              .catch((error: Error) => {
                logger.warn(`Renderer DOM probe failed: ${error.message}`);
              });
          }, 1200);
        }
      });
      ...
      mainWindow.webContents.on("console-message", (details) => {
        ...
        const shouldLog =
          level === "warning" ||
          level === "error" ||
          process.env.NEOWORKER_LOG_RENDERER_CONSOLE === "1";
        if (!shouldLog) return;

        const location = sourceId ? ` (${sourceId}:${lineNumber})` : "";
        const line = `[RendererConsole:${level}] ${message}${location}`;
        if (level === "error") {
          logger.error(line);
        } else {
          logger.warn(line);
      ...
      mainWindow.webContents.on(
        ...
        (_event, preloadPath, error) => {
          logger.error(
            `Main window preload error in ${preloadPath}: ${error.message}`,
          );
        },
      );
    ...
    app.whenReady().then(async () => {
      ...
      if (startupQuietMode) {
        logger.info(
          "Startup quiet mode enabled; background autostart is disabled.",
        );
      }
      ...
      const runDeferredStartupTasks = (): void => {
        ...
        deferredStartupTasks.forEach(({ name, task }, index) => {
          const timer = setTimeout(
            () => {
              if (index === 0) {
                logStartupLane("idle_startup", {
                  event: "first_deferred_task_start",
                });
              }
              ...
              void task()
                ...
                .catch((error) => {
                  logger.error(
                    `Deferred startup task "${name}" failed:`,
                    error,
                  );
                });
            },
      ...
      const resolvedUserDataDir =
        startupUserDataDir || applyStableUserDataPath();
              ...
              responseHeaders: {
                ...
                "Content-Security-Policy": [
                  "default-src 'self'; " +
                    ...
                    "frame-src 'self' web-preview:; " + // Allow tokenized local HTML previews in the artifact iframe
      ...
      deferStartupTask("legacy-workspace-recovery", async () => {
        logger.debug("Starting legacy workspace path recovery");
        ...
        const repairs = healMovedDesktopWorkspacePaths(
          ...
          (workspaceId, nextPath) =>
            workspaceRepo.updatePath(workspaceId, nextPath),
          {
        ...
        if (kitMigrations.length) {
          logger.info("Migrated legacy workspace kits.", { kitMigrations });
        }
        logger.debug("Legacy workspace path recovery complete");
      });
      ...
      try {
        ...
        const coreHarnessExperimentRepo = new CoreHarnessExperimentRepository(
          db,
        );
        ...
        coreTraceService = new CoreTraceService(
          coreTraceRepo,
        ...
        coreEvalCaseService = new CoreEvalCaseService(
          coreFailureClusterRepo,
          coreEvalCaseRepo,
        );
        ...
        const coreRegressionGateService = new CoreRegressionGateService(
          coreRegressionGateRepo,
        );
      ...
      if (HEADLESS) {
        try {
          ...
          if (!hasAnyLlmCreds) {
            logger.warn(
              "No LLM credentials configured. In headless mode, set NEOWORKER_IMPORT_ENV_SETTINGS=1 and an LLM key (e.g. OPENAI_API_KEY or ANTHROPIC_API_KEY), then restart.",
            );
          }
        } catch (error) {
      ...
      try {
        ...
        const ftsWorkerClient = new FtsWorkerClient(
          path.join(getUserDataDir(), "neoworker.db"),
        );
      ...
      try {
        ...
        await ChronicleCaptureService.getInstance().applySettings(
          chronicleSettings,
        );
      ...
      try {
        ...
        logger.error(
          "Failed to initialize Symphony issue orchestration:",
          error,
        );
      }
      ...
      try {
        ...
        if (
          ...
          if (!existing) {
            ...
            const ws = agentDaemon.createWorkspace(
              workspaceName,
              workspacePath,
            );
            logger.info(
      ...
      try {
        ...
        // Historical rebuilding can synchronously touch thousands of stale or
        // unavailable workspace paths. Keep live lore capture enabled, but never
        // let historical filesystem recovery block the first application window.
        await loreService.start(agentDaemon, { rebuildRecentTasks: false });
        logger.info(
          "LoreService initialized (historical rebuild skipped at startup)",
        );
      } catch (error) {
      ...
      try {
        subconsciousLoopService = new SubconsciousLoopService(
          dbManager.getDatabase(),
          {
            ...
            coreLearningPipelineService:
              coreLearningPipelineService || undefined,
          },
        );
        if (startupQuietMode) {
          logger.info(
            "SubconsciousLoopService initialized (quiet mode; not started)",
          );
        } else {
          // A full target refresh reads and writes workspace-local artifacts.
          // Keep the service available immediately while leaving that expensive
          // refresh to explicit/on-demand operations after the UI is interactive.
          await subconsciousLoopService.start(agentDaemon, {
            refreshTargets: false,
          });
          logger.info(
            "SubconsciousLoopService initialized (startup target refresh skipped)",
          );
        }
      } catch (error) {
        ...
        cronService = new CronService({
          ...
          createTask: async (params) => {
            ...
            if (councilService) {
              try {
                preparedCouncilTask =
                  await councilService.prepareTaskForTrigger(
                    params.prompt,
                    params.workspaceId,
                  );
              } catch (err) {
          ...
          sendTaskMessage: async (params) => {
            ...
            return agentDaemon.sendMessage(
              ...
              undefined,
              {
                agentConfigOverride:
                  params.agentConfig &&
                  Object.keys(params.agentConfig).length > 0
                    ? {
                        ...params.agentConfig,
                        allowUserInput:
                          params.allowUserInput ??
          ...
          resolveTemplateVariables: async ({
            ...
            const sinceMs = Math.max(
              0,
              Number.isFinite(prevRunAtMs)
                ? prevRunAtMs!
                : runAtMs - sevenDaysMs,
            );

            // Fetch a bounded window; formatting further caps message count/size.
            const raw = channelMessageRepo.findByChatId(
              channel.id,
              chatId,
              500,
            );
          ...
          onEvent: async (evt) => {
            ...
            try {
              ...
              logger.debug(
                "[Routines] Failed to record cron routine run:",
                error,
              );
            }
            ...
            if (evt.action === "finished") {
              ...
              if (notificationService) {
                try {
                  ...
                  const task = evt.taskId
                    ? taskRepo.findById(evt.taskId)
                    : null;
                  ...
                  console.error(
                    "[Cron] Failed to add in-app notification:",
                    err,
                  );
                }
              }
        ...
        if (!xMentionBridgeService) {
          xMentionBridgeService = initializeXMentionBridgeService(agentDaemon, {
            isNativeXChannelEnabled: () => {
              ...
              return (
                nativeX?.enabled === true && nativeX.status === "connected"
              );
            },
          });
        }
      ...
      try {
        ...
        const resolveDefaultWorkspace = ():
          ReturnType<typeof workspaceRepo.findById> | undefined => {
        ...
        const heartbeatDeps: HeartbeatServiceDeps = {
          ...
          getTasksForAgent: (agentRoleId, workspaceId) => {
            const activeAssignedStatuses = Array.from(
              ACTIVE_FOREGROUND_TASK_STATUSES,
            );
            const tasks = workspaceId
              ? taskRepo
                  .findByWorkspace(workspaceId)
                  .filter((task) =>
                    ACTIVE_FOREGROUND_TASK_STATUSES.has(task.status),
                  )
              : taskRepo.findByStatus(activeAssignedStatuses);
            return tasks.filter(
          ...
          runWorkflowReflection: async ({ workspaceId }) => {
            const run =
              await subconsciousLoopService?.runFromHeartbeat(workspaceId);
        ...
        try {
          ...
          if (startupQuietMode) {
            logger.info(
              "AwarenessService initialized (quiet mode; not started)",
            );
          } else {
          ...
          logger.error(
            "Failed to initialize AwarenessService:",
            awarenessError,
          );
        }
      } catch (error) {
        ...
        if (coreMemoryDistiller) {
          ...
          const runCoreDistill = async () => {
            for (const profile of automationProfileRepo.listEnabled()) {
              try {
                await coreMemoryDistiller?.runOffline({
      ...
      try {
        ...
        logger.error(
          "Failed to initialize Plugin Distribution handlers:",
          error,
        );
      }
      ...
      if (HEADLESS) {
        ...
        try {
          ...
          logger.error(
            "Failed to initialize Channel Gateway (headless):",
            error,
          );
      ...
      createWindow();
      logStartupLane("first_window_startup", {
        event: "create_window_returned",
      });
      ...
      if (mainWindow) {
        ...
        await startControlPlaneFromSettings({
          ...
          forceEnable:
            FORCE_ENABLE_CONTROL_PLANE || shouldAutoEnableDesktopControlPlane(),
        });
        ...
        eventTriggerService = new EventTriggerService(
          {
            ...
            getDefaultWorkspaceId: () => "",
            getActiveTaskCount: () =>
              agentDaemon.getQueueStatus().runningTaskIds.length,
            log: (...args: unknown[]) =>
              console.log("[EventTriggers]", ...args),
        ...
        const syncMcpTriggerSubscriptions = async (): Promise<void> => {
          ...
          await mcpClientManager.syncTriggerResourceSubscriptions(
            subscriptions,
          );
        };
                ...
                eventName:
                  typeof githubPayload.eventName === "string"
                    ...
                    : typeof githubPayload.event === "string"
                      ...
                      : "",
                action:
                  typeof githubPayload.action === "string"
                    ? githubPayload.action
                    : "",
                repository:
                  typeof githubPayload.repository === "string"
                    ...
                    : typeof githubPayload.repo === "string"
                      ...
                      : "",
                ref:
                  typeof githubPayload.ref === "string"
                    ? githubPayload.ref
                    : "",
                resourceUri: event.resourceUri || "",
                payload: JSON.stringify(event.payload || {}),
        ...
        setupTriggerHandlers(
          currentTriggerService,
          syncMcpTriggerSubscriptions,
        );
        ...
        routineService = new RoutineService({
          ...
          createManagedSession: async (params) => {
            ...
            if (!agent) {
              throw new Error(
                "No managed agents are available for routine execution",
              );
            }
          ...
          getTaskSnapshot: (taskId) => {
            ...
            return {
              ...
              resultSummary:
                task.resultSummary || task.semanticSummary || undefined,
        ...
        void routineService.reconcileStaleTimeoutRuns().catch((error) => {
          logger.debug(
            "[Routines] Failed to reconcile stale timeout runs:",
            error,
          );
        });
        const refreshRoutineRunsForTask = (payload: { taskId?: string }) => {
          ...
          void routineService?.refreshRunsForTask(taskId).catch((error) => {
            logger.debug(
              "[Routines] Failed to refresh routine task run:",
              error,
            );
          });
        };
        ...
        dailyBriefingService = new DailyBriefingService(
          {
            getRecentTasks: (_workspaceId, _sinceMs) => {
              try {
                const taskRepo = new TaskRepository(db);
                return (
                  taskRepo.findByWorkspace(_workspaceId, 200) || []
                ).filter(
                  (task) =>
            ...
            getMailboxDigest: async (workspaceId) =>
              getMailboxServiceInstance()?.getMailboxDigest(workspaceId) ||
              null,
        ...
        const loadWebAccessSettings = (): WebAccessConfig => {
          try {
            ...
            const repository = SecureSettingsRepository.getInstance();
            const stored =
              repository.load<Partial<WebAccessConfig>>("webaccess");
            if (!stored) {
              return { ...DEFAULT_WEB_ACCESS_CONFIG };
            }
          ...
          handleIpcInvoke: async (channel: string, ...args: Any[]) => {
            switch (channel) {
              ...
              case "task:sendMessage": {
                ...
                let message: string;
                let images:
                  import("../shared/types").ImageAttachment[] | undefined;
                let quotedAssistantMessage:
                  import("../shared/types").QuotedAssistantMessage | undefined;
                let options:
                  | Pick<
        ...
        if (process.platform === "darwin" || process.platform === "win32") {
          await trayManager.initialize(
            ...
            agentDaemon,
            process.env.NODE_ENV === "development"
              ? { startMinimized: false, showDockIcon: true }
              : undefined,
          );
          if (process.env.NODE_ENV === "development") {
    ...
    const shutdownCoordinator = new AsyncShutdownCoordinator({
      shutdown: async () => {
        try {
          ...
          if (xMentionBridgeService) {
            try {
              ...
              console.error(
                "[Main] Failed to stop X mention bridge service:",
                error,
              );
            }
          ...
          if (subconsciousLoopService) {
            try {
              ...
              console.error(
                "[Main] Failed to stop SubconsciousLoopService:",
                error,
              );
            }
          ...
          try {
            // Agent shutdown saves resumable snapshots, persists the interrupted
            // lifecycle state, and cancels in-flight provider/tool requests. Keep
            // the database open until every one of those operations has finished.
            if (agentDaemon) {
            ...
            if (detachTaskLifecycleSync) {
              try {
                ...
                console.error(
                  "[Main] Failed to detach task lifecycle sync:",
                  error,
                );
              }
      ...
      completeQuit: () => app.quit(),
      onError: (error) => {
        logger.error("Application shutdown cleanup failed:", error);
      },
    });
    ...
    ipcMain.handle(
      IPC_CHANNELS.BROWSER_WORKBENCH_STATUS,
      (_event, data: Any) => {
        if (!data || typeof data.taskId !== "string") return;
    ...
    ipcMain.handle(
      ...
      async (_event, data: Any) => {
        if (
          ...
          return {
            success: false,
            error: "Invalid browser screenshot request",
          };
        }
        ...
        if (!result)
          return {
            success: false,
            error: "No active browser workbench session",
          };
    ...
    ipcMain.handle(
      ...
      async (_event, data: Any) => {
        ...
        if (!target)
          return {
            success: false,
            error: "No inspectable element at that point",
          };
    ...
    ipcMain.handle(
      ...
      async (_event, data: Any) => {
        if (
          ...
          return {
            success: false,
            error: "Invalid browser annotation target resolve request",
          };
        }
        const targets =
          await getBrowserWorkbenchService().resolveAnnotationTargets({
            taskId: data.taskId,
            sessionId:
              typeof data.sessionId === "string" ? data.sessionId : "default",
            targets: data.targets.filter(
              (target: Any) => target && typeof target === "object",
            ),
          });
    ...
    const getAnnotationRepo = () =>
      new AnnotationRepository(dbManager.getDatabase());
    const annotationSurfaceTypes = new Set([
      "browser",
      "diff",
      "file",
      "artifact",
    ...
    ipcMain.handle(IPC_CHANNELS.ANNOTATION_CREATE, (_event, data: Any) => {
      ...
      const surfaceType =
        typeof data.surfaceType === "string" ? data.surfaceType.trim() : "";
      ...
      const annotation = getAnnotationRepo().create({
        ...
        surfaceId:
          typeof data.surfaceId === "string" ? data.surfaceId : undefined,
        body,
        targetRef: data.targetRef,
        stylePatch:
          data.stylePatch && typeof data.stylePatch === "object"
            ? data.stylePatch
            : undefined,
        artifactId:
        ...
        screenshotPath:
          typeof data.screenshotPath === "string"
            ...
            : undefined,
        createdBy:
          data.createdBy === "agent" || data.createdBy === "review"
            ? data.createdBy
            : "user",
      });
    ...
    ipcMain.handle(IPC_CHANNELS.ANNOTATION_LIST, (_event, query: Any) => {
      ...
      return getAnnotationRepo().list({
        taskId:
          typeof normalized.taskId === "string" ? normalized.taskId : undefined,
        workspaceId:
          typeof normalized.workspaceId === "string"
            ? normalized.workspaceId
            : undefined,
        surfaceType:
          typeof normalized.surfaceType === "string"
            ? normalized.surfaceType
            : undefined,
        surfaceId:
          typeof normalized.surfaceId === "string"
            ? normalized.surfaceId
            : undefined,
        statuses: Array.isArray(normalized.statuses)
          ? normalized.statuses
          : undefined,
        limit:
    ...
    ipcMain.handle(IPC_CHANNELS.ANNOTATION_RESOLVE, (_event, data: Any) => {
      ...
      const annotation = getAnnotationRepo().update(id, {
        status: "resolved",
        resolvedByEventId:
          typeof data?.resolvedByEventId === "string"
            ? data.resolvedByEventId
            : undefined,
      });
    ...
    const resolveDialogDefaultPath = async (candidate?: string | null) => {
      ...
      const tempWorkspaceRoot = path.resolve(
        os.tmpdir(),
        TEMP_WORKSPACE_ROOT_DIR_NAME,
      );
      ...
      const recentWorkspaceCandidates = (() => {
        try {
          ...
          logger.warn(
            "[Dialog] Failed to load recent workspace fallback paths:",
            error,
          );
          return [] as string[];
        }
    ...
    ipcMain.handle(
      ...
      async (_, defaultPath?: string | null) => {
    ...
    // Let output-path parameters choose a new destination and filename instead
    // of forcing users to type a path to a file that does not exist yet.
    ipcMain.handle(
      IPC_CHANNELS.DIALOG_SELECT_SAVE_PATH,
      async (_, defaultPath?: string | null) => {
        const requestedPath =
          typeof defaultPath === "string" ? defaultPath.trim() : "";
        const suggestedName = requestedPath ? path.basename(requestedPath) : "";
        const requestedDirectory = requestedPath
          ? path.dirname(requestedPath)
          : undefined;
        const resolvedDirectory =
          await resolveDialogDefaultPath(requestedDirectory);
        const resolvedDefaultPath = resolvedDirectory
          ? suggestedName && suggestedName !== "."
            ? path.join(resolvedDirectory, suggestedName)
            : resolvedDirectory
          : undefined;
