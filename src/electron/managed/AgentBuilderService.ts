...
type IntegrationKey =
  "slack" | "gmail" | "calendar" | "github" | "linear" | "notion" | "drive";
...
function uniqueToolFamilies(
  values: Array<string | undefined | null>,
): ManagedAgentToolFamily[] {
  const result: ManagedAgentToolFamily[] = [];
  for (const value of values) {
    if (!value || !VALID_TOOL_FAMILIES.has(value as ManagedAgentToolFamily))
      continue;
...
function explicitIntegrationPromptMatches(
  ...
  return descriptor.explicitKeywords.some((keyword) =>
...
function genericIntegrationPromptMatches(
  ...
  if (
    descriptor.genericKeywords.some((keyword) =>
      keywordMatches(prompt, keyword),
    )
  )
    return true;
  if (
    descriptor.key === "linear" &&
    /\bissues?\b/.test(lower) &&
...
function serverMatchesAliases(
  server: MCPServerConfig,
  aliases: string[],
): boolean {
  const haystack =
    `${server.id} ${server.name} ${server.description || ""}`.toLowerCase();
...
function findConnectedServers(
  ...
  return inventory.mcpServers.filter(
    (server) =>
      server.enabled && serverMatchesAliases(server, descriptor.serverAliases),
  );
}
...
function connectionOptionFromServer(
  ...
  return {
    ...
    description:
      server.description ||
      `Use the enabled ${server.name || descriptor.label} integration.`,
    status: "available",
    ...
    if (explicitMatch) {
      ...
      selectionRequirements.push({
        ...
        options: connectedServers.map((server) =>
          connectionOptionFromServer(server, descriptor),
        ),
      });
    } else {
      missingConnections.push(
      ...
      missingConnections.push(
        makeMissingConnection(
          ...
          inventory.channels.some(
            (channel) => channel.type === "slack" && channel.enabled,
          )
            ? "missing"
            : "needs_auth",
        ),
...
export function inferExplicitSchedule(
  prompt: string,
): ManagedAgentScheduleConfig {
  const lower = prompt.toLowerCase();
  const everyMatch = lower.match(
    /\bevery\s+(\d+)\s*(minute|minutes|min|hour|hours|day|days)\b/,
  );
  if (everyMatch) {
...
function inferSkills(
  ...
  const promptWords = new Set(lowercaseWords(prompt));
  const enabledSkills = inventory.skills.filter(
    (skill) => skill.enabled !== false,
  );
  const exact = enabledSkills.filter((skill) =>
    skillPromptMatchesExact(prompt, skill),
  );
  if (exact.length > 0) {
    return {
  ...
  if (viable.length === 1) {
    return {
      selectedSkills: uniqueStrings(
        [...(template?.skills || []), viable[0].skill.id],
        8,
      ),
      selectionRequirements: [],
    };
...
function defaultStarterPrompts(
  planName: string,
  prompt: string,
): AgentStarterPrompt[] {
  return [
...
function buildInstructions(
  ...
  return [
    `You are ${planName}, a private NeoWorker managed agent created for personal use.`,
    "",
    "Mission:",
    prompt || "Help the user run a focused workflow inside NeoWorker.",
    "",
    "Operating rules:",
...
export function buildFallbackAgentPlan(
  ...
  const selectedToolFamilies = uniqueToolFamilies([
    ...
    promptWords.includes("image") || promptWords.includes("design")
      ? "images"
      : undefined,
    promptWords.includes("file") || promptWords.includes("folder")
      ? "files"
      : undefined,
    promptWords.includes("doc") || promptWords.includes("report")
      ? "documents"
      : undefined,
    promptWords.includes("code") || promptWords.includes("script")
      ? "shell"
      : undefined,
    "search",
    "files",
  ...
  const name = deriveAgentNameFromPrompt(prompt, template?.name || "New Agent");
  const description =
    prompt || template?.description || "A private NeoWorker agent.";
  ...
  return {
    ...
    capabilities: uniqueStrings(
      [
        ...
        selectedToolFamilies.includes("communication")
          ...
          : undefined,
        selectedToolFamilies.includes("documents")
    ...
    enableShell:
      selectedToolFamilies.includes("shell") ||
      !!template?.environmentConfig?.enableShell,
  ...
  return {
    ...
    mcpServers: inventory.mcpServers.slice(0, 60).map((server) => ({
      ...
      description: server.description
        ? truncate(server.description, 180)
        : undefined,
...
function normalizeSelectionRequirements(
  value: unknown,
): AgentBuilderSelectionRequirement[] {
  if (!Array.isArray(value)) return [];
  ...
  for (const entry of value) {
    ...
    const options: AgentBuilderSelectionOption[] = rawOptions
      .map((option): AgentBuilderSelectionOption | null => {
        ...
        return {
          ...
          selectedMcpServers: normalizeStringArray(
            optionRecord.selectedMcpServers,
          ),
          selectedSkills: normalizeStringArray(optionRecord.selectedSkills),
          missingConnections: normalizeConnections(
            optionRecord.missingConnections,
          ),
        };
      })
    ...
    requirements.push({
      ...
      reason:
        ...
        "This choice is required before creating the agent.",
...
function unresolvedOptionServerIds(
  requirements: AgentBuilderSelectionRequirement[],
): Set<string> {
  const serverIds = new Set<string>();
  for (const requirement of requirements) {
    if (!requirement.required || requirement.selectedOptionId) continue;
    for (const option of requirement.options) {
      for (const serverId of option.selectedMcpServers || [])
...
function unresolvedOptionSkillIds(
  requirements: AgentBuilderSelectionRequirement[],
): Set<string> {
...
function normalizePlanFromJson(
  ...
  const inferredSkills = inferSkills(
    ...
    inventory,
    fallback.templateId
      ? inventory.templates.find(
          (template) => template.id === fallback.templateId,
        )
      : undefined,
  );
  const explicitSchedule = inferExplicitSchedule(prompt);
  ...
  const selectedMcpServers = uniqueStrings([
    ...llmMcpServers.filter(
      (serverId) =>
        enabledServers.has(serverId) && !unresolvedServerIds.has(serverId),
    ),
    ...inferred.selectedMcpServers,
  ]);
  ...
  const generatedName = truncate(normalizeText(record.name), 80);
  const name =
    !generatedName || isGenericAgentName(generatedName)
      ? fallback.name
      : generatedName;
  const scheduleConfig = explicitSchedule.enabled
    ? explicitSchedule
  ...
  return {
    ...
    name,
    subtitle: truncate(
      normalizeText(record.subtitle) || "Private in NeoWorker",
      120,
    ),
    description: truncate(
      normalizeText(record.description) || fallback.description,
      500,
    ),
    ...
    capabilities: uniqueStrings(
      [
        ...normalizeStringArray(record.capabilities, 8),
    ...
    operatingNotes:
      normalizeText(record.operatingNotes) || fallback.operatingNotes,
...
export class AgentBuilderService {
  ...
  async generatePlan(
    ...
    try {
      const provider =
        this.options.createProvider?.() || LLMProviderFactory.createProvider();
      const model =
        this.options.getSelectedModel?.() ||
        LLMProviderFactory.getSelectedModel();
      const compressedInventory = compressAgentBuilderInventory(inventory);
      const response = await provider.createMessage({
        model,
        maxTokens: 2600,
        system: [
          "You create private personal NeoWorker managed agents from a user's prompt.",
          ...
          "Use private sharing, ownerLabel You, NeoWorker surface, read-only auto approval, write approval gates.",
        ].join("\n"),
        messages: [
          {
            role: "user",
      ...
      const parsed = extractFirstJsonObject(
        responseTextFromContent(response.content),
      );
      return normalizePlanFromJson(
        parsed,
        request,
        inventory,
        fallback,
