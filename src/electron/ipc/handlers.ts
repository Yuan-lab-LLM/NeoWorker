...
function shouldDisableBackgroundAutostart(): boolean {
  return (
    ...
    /^(0|false|no|off)$/i.test(String(process.env.NEOWORKER_BACKGROUND_AUTOSTART || "").trim())
  );
}
...
import { pollWeixinQrStatus, requestWeixinQrCode } from "../gateway/channels/weixin";
...
const TaskAccessUpdateSchema = z
  .object({
    ...
    patch: z
      .object({
        connectorIds: z.array(z.string().trim().min(1).max(200)).max(100).optional(),
        workspaceScopes: z
          .array(
            z
              .object({
...
function isIpcPerfTelemetryEnabled(): boolean {
  return process.env.NEOWORKER_DEV_LOG_CAPTURE === "1" || getDevLogCaptureEnabled();
}
...
const VIDEO_PREVIEW_CACHE_DIR = path.join(os.tmpdir(), "neoworker-video-preview-cache");
...
rateLimiter.configure(IPC_CHANNELS.TASK_FOLLOW_UP_QUEUE_LIST, RATE_LIMIT_CONFIGS.frequent);
rateLimiter.configure(IPC_CHANNELS.TASK_FOLLOW_UP_QUEUE_UPDATE, RATE_LIMIT_CONFIGS.limited);
rateLimiter.configure(IPC_CHANNELS.TASK_FOLLOW_UP_QUEUE_REMOVE, RATE_LIMIT_CONFIGS.limited);
...
rateLimiter.configure(IPC_CHANNELS.TASK_PROVENANCE_LIST, RATE_LIMIT_CONFIGS.frequent);
rateLimiter.configure(IPC_CHANNELS.TASK_PROVENANCE_OPEN, RATE_LIMIT_CONFIGS.standard);
rateLimiter.configure(IPC_CHANNELS.TASK_ACCESS_GET, RATE_LIMIT_CONFIGS.frequent);
rateLimiter.configure(IPC_CHANNELS.TASK_ACCESS_UPDATE, RATE_LIMIT_CONFIGS.limited);
...
// Model settings are local encrypted writes. A user can legitimately refresh,
// add, and switch several models in one minute, so the generic 5/min settings
// limit causes false failures in the model console.
rateLimiter.configure(IPC_CHANNELS.LLM_SAVE_SETTINGS, RATE_LIMIT_CONFIGS.standard);
...
rateLimiter.configure(IPC_CHANNELS.LLM_REFRESH_CUSTOM_PROVIDER_MODELS, RATE_LIMIT_CONFIGS.standard);
...
export async function setupIpcHandlers(
  ...
  const buildViewerPathCandidates = (filePath: string, workspacePath?: string): string[] => {
    ...
    if (path.isAbsolute(normalizedInput)) {
      ...
      if (hasWorkspace) {
        ...
        if (neoworkerIdx >= 0 && neoworkerIdx < segments.length - 1) {
          addCandidate(
            path.join(normalizedWorkspace, ".neoworker", ...segments.slice(neoworkerIdx + 1)),
          );
        }
    ...
    if (hasWorkspace) {
      ...
      if (!hasParentTraversal) {
        addCandidate(path.join(normalizedWorkspace, ".neoworker", normalizedRelativeInput));
        addCandidate(path.join(normalizedWorkspace, "artifacts", normalizedRelativeInput));

        if (basename && basename !== "." && basename !== "..") {
          addCandidate(path.join(normalizedWorkspace, ".neoworker", "tmp", basename));
          addCandidate(path.join(normalizedWorkspace, ".neoworker", "artifacts", basename));
        }
  ...
  const resolveExistingPathForViewer = async (
    ...
    for (const candidate of candidates) {
      if (requireWorkspaceContainment && workspaceRoot) {
        ...
        if (!isPathWithinWorkspace(candidate, workspaceRoot) && !isApprovedExternalImport) {
      ...
      const candidateIsInsideWorkspace =
        workspaceRoot && isPathWithinWorkspace(candidate, workspaceRoot);
      const realPath =
        requireWorkspaceContainment && workspaceRoot && candidateIsInsideWorkspace
    ...
    if (workspaceRoot) {
      const recoveredPath = await findUniqueViewerArtifactByBasename(filePath, workspaceRoot);
      if (recoveredPath && !attemptedPaths.includes(recoveredPath)) {
        if (!requireWorkspaceContainment || isPathWithinWorkspace(recoveredPath, workspaceRoot)) {
  ...
  const requireViewerWorkspaceContainment = (workspacePath: string): boolean => {
    const normalizedWorkspacePath = path.resolve(normalizePotentialPath(workspacePath));
    const workspace =
      workspaceRepo.findByPath(workspacePath) ||
      workspaceRepo
        .findAll()
        .find(
          (item) => path.resolve(normalizePotentialPath(item.path)) === normalizedWorkspacePath,
        );
  ...
  const renderPdfFirstPageThumbnail = async (pdfPath: string): Promise<string | undefined> => {
    ...
    try {
      tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "neoworker-pdf-thumb-"));
  ...
  const normalizeExistingTempWorkspacePath = (existing: Workspace): string => {
    try {
      return ensureTempWorkspaceDirectoryPathSync(tempWorkspaceRoot, existing.path);
      ...
      try {
        ...
        logger.warn("Replaced unusable temp workspace path with a fresh NeoWorker workspace", {
          workspaceId: existing.id,
  ...
  ipcMain.handle(IPC_CHANNELS.FILE_OPEN, async (_, filePath: string, workspacePath?: string) => {
    ...
    const { resolvedPath, realPath } = await resolveExistingPathForViewer(filePath, workspacePath, {
      requireWorkspaceContainment: requireViewerWorkspaceContainment(workspacePath),
    });
    if (!resolvedPath) {
  ...
  ipcMain.handle(
    ...
    async (_, filePath: string, workspacePath?: string) => {
      ...
      const options: Electron.SaveDialogOptions = {
        defaultPath: path.join(_app.getPath("downloads"), path.basename(sourcePath)),
      };
  ...
  ipcMain.handle(
    ...
    async (_, data: { workspaceId: string; files: string[] }) => {
      ...
      const ensureUploadRoot = async (): Promise<string> => {
        ...
        uploadRoot = path.join(workspace.path, ".neoworker", "uploads", `${Date.now()}`);
      ...
      for (const filePath of validated.files) {
        ...
        if (isPathWithinWorkspace(absolutePath, workspace.path)) {
          if (workspace.isTemp === true || isTempWorkspaceId(workspace.id)) {
            persistTempWorkspaceArtifactSync({
              userDataPath: getUserDataDir(),
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const uploadRoot = path.join(workspace.path, ".neoworker", "uploads", `${Date.now()}`);
      await fs.mkdir(uploadRoot, { recursive: true });
      const usedNames = new Set<string>();
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_SET_ARCHIVED, async (_, id: string, archived: boolean) => {
    const workspace = workspaceRepo.findById(id);
  ...
  const ensureTerminalWorkspacePath = (workspace: Workspace): string => {
    if (workspace.isTemp || isTempWorkspaceId(workspace.id)) {
      return ensureTempWorkspaceDirectoryPathSync(tempWorkspaceRoot, workspace.path);
    }
    return workspace.path;
  };
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        cwd = await resolveWorkspaceContainedCwd(workspacePath, data.cwd || tab.cwd);
      } catch {
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_CREATE, async (_, data) => {
    ...
    if (projectId) {
      ...
      if (!isLinkedWorkspace) {
        throw new Error("The selected workspace is not linked to this project.");
      }
    }
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_PROVENANCE_LIST, async (_, input) => {
    ...
    const validated = validateInput(TaskProvenanceListSchema, input, "task provenance list");
    if (!taskRepo.findById(validated.taskId)) {
      return [];
    }
    return taskProvenanceRepo
      .listRecentByTaskId(validated.taskId, validated.limit ?? 20, validated.offset ?? 0)
      .map((record) => ({
        ...record,
        metadata: undefined,
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_PROVENANCE_OPEN, async (_, input) => {
    ...
    const validated = validateInput(TaskProvenanceOpenSchema, input, "task provenance open");
    const record = taskProvenanceRepo.findByIdForTask(validated.taskId, validated.provenanceId);
    if (!record) {
      return { opened: false, reason: "source-not-found" };
    }
    if (record.openTarget?.kind !== "external_url" || !record.openTarget.locator) {
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_ACCESS_GET, async (_, input) => {
    ...
    const validated = validateInput(TaskAccessGetSchema, input, "task access get");
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_ACCESS_UPDATE, async (_, input) => {
    ...
    const validated = validateInput(TaskAccessUpdateSchema, input, "task access update");
    const task = taskRepo.findById(validated.taskId);
    ...
    if (!current) {
      if (expectedRevision !== 0) throw new Error("TASK_ACCESS_REVISION_CONFLICT");
    ...
    const patch: Partial<
      Pick<
        TaskAccessPolicy,
        "connectorIds" | "workspaceScopes" | "blockedTools" | "permissionMode" | "shellAccess"
      >
    > = { ...validated.patch };
    if (patch.workspaceScopes) {
      patch.workspaceScopes = patch.workspaceScopes.map((scope) => {
        if (scope.workspaceId !== workspace.id) {
          throw new Error("Workspace is outside this task's allowed project scope");
        }
        if (!workspace.permissions.read) {
          throw new Error("Workspace read access is blocked by the global policy");
        }
        if (scope.access === "write" && !workspace.permissions.write) {
          throw new Error("Workspace write access is blocked by the global policy");
        }
    ...
    if (patch.permissionMode) {
      ...
      if (allowedModes.length > 0 && !allowedModes.includes(patch.permissionMode)) {
        throw new Error("Permission mode is blocked by the administrator policy");
      }
      if (
        (patch.permissionMode === "dont_ask" || patch.permissionMode === "bypass_permissions") &&
        current.permissionMode !== patch.permissionMode
      ) {
        throw new Error("High-risk permission modes require the existing approval flow");
      }
    }

    if (patch.connectorIds) {
      const availableById = new Map(connectors.map((connector) => [connector.id, connector]));
      ...
      const connectorToolNames = new Set(connectors.flatMap((connector) => connector.tools));
      const originalRestrictions = new Set(task.agentConfig?.toolRestrictions || []);
      const nextBlocked = new Set(
        (current.blockedTools || []).filter(
          (toolName) => !connectorToolNames.has(toolName) || originalRestrictions.has(toolName),
        ),
      );
    ...
    const effectiveFromTurn = Math.max(
      1,
      (task.lifetimeTurnsUsed || task.budgetUsage?.lifetimeTurns || task.budgetUsage?.turns || 0) +
        1,
    );
    try {
      ...
      taskEventRepo.create({
        ...
        payload: {
          ...
          effectiveFromTurn,
          message: "Task access policy updated; changes apply from the next turn.",
        },
        schemaVersion: 2,
      });
      return deriveTaskAccessSummary({
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const tasks = projectId
        ? taskRepo.findByProjectId(projectId, limit, offset, opts?.includeArchivedSessions === true)
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_UPDATE_WORKSPACE, async (_, data) => {
    ...
    if (task.projectId) {
      ...
      if (!isLinkedWorkspace) {
        throw new Error("Link this workspace to the project before moving the task.");
      }
    }
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_UPDATE_PROJECT, async (_, data) => {
    ...
    const validated = validateInput(TaskProjectUpdateSchema, data, "task project update");
    const task = taskRepo.findById(validated.taskId);
    if (!task) throw new Error(`Task not found: ${validated.taskId}`);

    const project = controlPlaneCore.getProject(validated.projectId);
    if (!project || project.status === "archived") {
      throw new Error("Choose an active project before adding this conversation.");
    }
    const isLinkedWorkspace = controlPlaneCore
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_PURGE_ARCHIVED, async (_, id: string) => {
    ...
    return sessionRetentionService.purgeArchivedSession(task.sessionId || task.id, {
      deleteTask: async (sessionTask) => {
        ...
        try {
          await agentDaemon.getWorktreeManager().cleanup(sessionTask.id, true);
        } catch (error) {
          logger.warn(
      ...
      const result = await agentDaemon.sendMessage(
        ...
        validated.quotedAssistantMessage,
        {
          ...
          ...(validated.agentConfigOverride
            ? { agentConfigOverride: validated.agentConfigOverride }
            : {}),
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_FOLLOW_UP_QUEUE_LIST, async (_, data) => {
    ...
    const validated = validateInput(TaskFollowUpQueueListSchema, data, "task follow-up queue list");
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_FOLLOW_UP_QUEUE_UPDATE, async (_, data) => {
    ...
    return agentDaemon.updateQueuedFollowUp(validated.taskId, validated.queueId, validated.message);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_FOLLOW_UP_QUEUE_REORDER, async (_, data) => {
    ...
    return agentDaemon.reorderQueuedFollowUps(validated.taskId, validated.queueIds);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.TASK_FOLLOW_UP_QUEUE_REMOVE, async (_, data) => {
    ...
    const result = agentDaemon.removeQueuedFollowUp(validated.taskId, validated.queueId);
  ...
  ipcMain.handle(IPC_CHANNELS.LLM_TEST_PROVIDER, async (_, config: Any) => {
    ...
    const providerConfig: LLMProviderConfig = {
      ...
      openrouterBaseUrl,
      deepseekApiKey: validateOptionalProviderApiKey(validatedConfig.deepseek?.apiKey, "DeepSeek"),
      deepseekBaseUrl,
      ...
      kimiBaseUrl,
      openaiCompatibleDisplayName: validatedConfig.openaiCompatible?.displayName,
  ...
  ipcMain.handle(IPC_CHANNELS.WEIXIN_LOGIN_POLL, async (_, qrcode: unknown) => {
    ...
    const validated = validateInput(z.string().trim().min(1).max(500), qrcode, "WeChat QR code");
  ...
  ipcMain.handle(IPC_CHANNELS.GATEWAY_ADD_CHANNEL, async (_, data) => {
    ...
    if (validated.type === "dingtalk") {
      ...
      return toPublicChannel(gateway.getChannel(channel.id) || channel, "connected");
    }
  ...
  ipcMain.handle(
    ...
    async (_, data: { channelType: string; channelDbId?: string; chatId: string }) => {
      ...
      await gateway.sendMessage(resolvedType as Any, data.chatId, "Test delivery from NeoWorker", {
  ...
  ipcMain.handle(IPC_CHANNELS.APPEARANCE_SAVE_SETTINGS, async (_event, settings) => {
  ...
  ipcMain.handle(IPC_CHANNELS.TEAM_RUN_LIST_RECENT_TASKS, async (_, requestedLimit?: number) => {
    const limit =
...
function setupMemoryHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.MEMORY_OBSERVATIONS_DELETE, async (_, data: unknown) => {
    ...
    return {
      success: MemoryObservationService.delete(validated.workspaceId, validated.memoryId),
    };
  });
  ...
  const migrationMarkerPath = path.join(userDataPath, ".migrated-from-neoworker");
