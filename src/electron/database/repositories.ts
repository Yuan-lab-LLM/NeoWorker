...
export class WorkspaceRepository {
  ...
  updatePermissions(id: string, permissions: WorkspacePermissions): void {
    const stmt = this.db.prepare(
      "UPDATE workspaces SET permissions = ? WHERE id = ?",
    );
...
export class TaskSessionMetadataRepository {
  ...
  upsert(
    ...
    const name = Object.prototype.hasOwnProperty.call(updates, "name")
      ...
      : (existing?.name ?? null);
    const archivedAt = Object.prototype.hasOwnProperty.call(
      updates,
      "archivedAt",
    )
    ...
    if (!metadata) {
      throw new Error(
        `Failed to save session metadata: ${normalizedSessionId}`,
      );
    }
    return metadata;
  }
...
export class TaskRepository {
  private static readonly UPDATE_FIELD_TO_COLUMN: Partial<
  ...
  private static buildSidebarCursorPredicate(cursor?: {
    ...
    const activeRank = TaskRepository.SIDEBAR_ACTIVE_STATUSES.has(
  ...
  create(task: Omit<Task, "id" | "createdAt" | "updatedAt">): Task {
    ...
    stmt.run(
      ...
      newTask.bestKnownOutcome
        ? JSON.stringify(newTask.bestKnownOutcome)
        : null,
      ...
      typeof newTask.lastProgressScore === "number"
        ? newTask.lastProgressScore
        : null,
      newTask.autoContinueBlockReason || null,
      newTask.compactionCount ?? 0,
      typeof newTask.lastCompactionAt === "number"
        ? newTask.lastCompactionAt
        : null,
  ...
  update(id: string, updates: Partial<Task>): void {
    ...
    if (isActiveTaskStatus(normalizedUpdates.status)) {
      ...
      if (
        !Object.prototype.hasOwnProperty.call(normalizedUpdates, "failureClass")
      ) {
    ...
    Object.entries(normalizedUpdates).forEach(([key, value]) => {
      ...
      if (!TaskRepository.ALLOWED_UPDATE_FIELDS.has(key)) {
        taskRepositoryLogger.warn(
          `Ignoring unknown field in task update: ${key}`,
        );
        return;
      }
      ...
      if (!dbKey) {
        taskRepositoryLogger.warn(
          `No database column mapping found for task field: ${key}`,
        );
        return;
      }
      fields.push(`${dbKey} = ?`);
  ...
  findAll(
    ...
    const excludedSources = Array.isArray(options?.excludeSources)
      ? options.excludeSources.filter(
          (source): source is NonNullable<Task["source"]> => Boolean(source),
        )
      : [];
  ...
  countByWorkspace(workspaceId: string): number {
    const stmt = this.db.prepare(
      "SELECT COUNT(1) as count FROM tasks WHERE workspace_id = ?",
    );
  ...
  delete(id: string): void {
    ...
    const deleteTransaction = this.db.transaction((taskId: string) => {
      ...
      const deleteInputRequests = this.db.prepare(
        "DELETE FROM input_requests WHERE task_id = ?",
      );
      deleteInputRequests.run(taskId);

      // Delete activity feed entries for this task
      const deleteActivities = this.db.prepare(
        "DELETE FROM activity_feed WHERE task_id = ?",
      );
      deleteActivities.run(taskId);

      // Delete agent mentions for this task
      const deleteMentions = this.db.prepare(
        "DELETE FROM agent_mentions WHERE task_id = ?",
      );
      ...
      const deleteWorktreeInfo = this.db.prepare(
        "DELETE FROM worktree_info WHERE task_id = ?",
      );
      deleteWorktreeInfo.run(taskId);

      // Delete hook_sessions (task_id NOT NULL)
      const deleteHookSessions = this.db.prepare(
        "DELETE FROM hook_sessions WHERE task_id = ?",
      );
    ...
    for (const tableRow of tableRows) {
      ...
      for (const foreignKey of foreignKeys) {
        ...
        if (columnIsNullable && onDelete !== "CASCADE") {
          this.db
            .prepare(
              `UPDATE ${quotedTable} SET ${quotedColumn} = NULL WHERE ${quotedColumn} = ?`,
            )
            .run(taskId);
        } else {
          this.db
            .prepare(`DELETE FROM ${quotedTable} WHERE ${quotedColumn} = ?`)
            .run(taskId);
        }
      }
    }
  ...
  private mapRowToTask(row: Any): Task {
    return normalizeTaskLifecycleState({
      ...
      lastRunDurationMs:
        typeof row.last_run_duration_ms === "number" &&
        Number.isFinite(row.last_run_duration_ms)
      ...
      // Task Board fields
      labels: row.labels
        ? safeJsonParse<string[]>(row.labels, [], "task.labels")
        : undefined,
      ...
      mentionedAgentRoleIds: row.mentioned_agent_role_ids
        ? safeJsonParse<string[]>(
            row.mentioned_agent_role_ids,
            [],
            "task.mentionedAgentRoleIds",
          )
        : undefined,
      // Git Worktree fields
      ...
      worktreeStatus:
        (row.worktree_status as Task["worktreeStatus"]) || undefined,
      ...
      continuationCount:
        typeof row.continuation_count === "number"
          ...
          : undefined,
      continuationWindow:
        typeof row.continuation_window === "number"
          ? row.continuation_window
          : undefined,
      lifetimeTurnsUsed:
        typeof row.lifetime_turns_used === "number"
          ? row.lifetime_turns_used
          : undefined,
      lastProgressScore:
        typeof row.last_progress_score === "number"
          ? row.last_progress_score
          : undefined,
      autoContinueBlockReason: row.auto_continue_block_reason || undefined,
      awaitingUserInputReasonCode:
        row.awaiting_user_input_reason_code || undefined,
      compactionCount:
        typeof row.compaction_count === "number"
          ? row.compaction_count
          : undefined,
      lastCompactionAt:
        typeof row.last_compaction_at === "number"
          ? row.last_compaction_at
          : undefined,
      lastCompactionTokensBefore:
      ...
      lastCompactionTokensAfter:
        typeof row.last_compaction_tokens_after === "number"
          ...
          : undefined,
      noProgressStreak:
        typeof row.no_progress_streak === "number"
          ? row.no_progress_streak
          : undefined,
  ...
  private mapRowToSidebarTask(row: Any): Task {
    ...
    setBooleanAgentConfig(
      "videoGenerationMode",
      row.agent_config_video_generation_mode,
    );
    setBooleanAgentConfig("multitaskMode", row.agent_config_multitask_mode);
    setBooleanAgentConfig(
      "collaborativeMode",
      row.agent_config_collaborative_mode,
    );
  ...
  pruneByTargetNodeIds(
    ...
    if (normalizedKeepTaskIds.length > 0) {
      where.push(
        `id NOT IN (${normalizedKeepTaskIds.map(() => "?").join(", ")})`,
      );
...
export class TaskEventRepository {
  ...
  private static readonly DEFAULT_TIMELINE_PAGE_LIMIT =
    TASK_TIMELINE_HISTORY_LIMIT;
  private static readonly MAX_TIMELINE_PAGE_LIMIT =
    TASK_TIMELINE_MAX_PAGE_LIMIT;
  private static readonly DEFAULT_TIMELINE_PAGE_BYTE_LIMIT =
    TASK_TIMELINE_HISTORY_BYTE_LIMIT;
  private static readonly MAX_TIMELINE_PAGE_BYTE_LIMIT =
    TASK_TIMELINE_MAX_PAGE_BYTE_LIMIT;
  ...
  create(event: Omit<TaskEvent, "id"> & { id?: string }): TaskEvent {
    ...
    stmt.run(
      ...
      typeof storedEvent.legacyType === "string"
        ? storedEvent.legacyType
        : null,
    );
    ...
    if (
      ...
      try {
        ...
        UsageInsightsProjector.getIfInitialized()?.enqueueTaskEvent(
          row?.workspace_id,
          storedEvent,
        );
      } catch {
  ...
  findRecentByTaskId(taskId: string, maxEvents: number): TaskEvent[] {
    ...
    const structuralRows = structuralRowsStmt.all(
      taskId,
      ...noiseTypes,
      safeLimit,
    ) as Any[];
    ...
    if (structuralRows.length < safeLimit) {
      ...
      const noiseRowsStmt = this.db.prepare(`
        ...
        LIMIT ?
      `);
      const noiseRows = noiseRowsStmt.all(
        taskId,
        ...noiseTypes,
        noiseBudget,
      ) as Any[];
   ...
   * state of early steps out of that tail, making progress appear to go
   * backwards after an app restart.
   */
  findLatestPlanStepStateEvents(
    taskId: string,
    planEvent: TaskEvent,
  ): TaskEvent[] {
    const normalizedTaskId = typeof taskId === "string" ? taskId.trim() : "";
    const rawSteps = Array.isArray(planEvent?.payload?.plan?.steps)
      ? planEvent.payload.plan.steps
      : [];
    const planStepIds = Array.from(
      new Set(
        rawSteps
          .map((step: Any) => String(step?.id || "").trim())
          .filter(Boolean),
      ),
    );
    if (!normalizedTaskId || planStepIds.length === 0) return [];

    const planOrder =
      Number(planEvent.seq ?? planEvent.ts ?? planEvent.timestamp) || 0;
    const stepPlaceholders = planStepIds.map(() => "?").join(", ");
    const rows = this.db
      .prepare(
        `
        SELECT *
      ...
      .all(normalizedTaskId, planOrder, ...planStepIds) as Any[];

    const latestByStepId = new Map<string, Any>();
    for (const row of rows) {
      const stepId = typeof row.step_id === "string" ? row.step_id.trim() : "";
      if (!stepId || latestByStepId.has(stepId)) continue;
      latestByStepId.set(stepId, row);
    }

    const latestRows = Array.from(latestByStepId.values()).sort((a, b) => {
      const aOrder = Number(a.seq ?? a.timestamp) || 0;
      const bOrder = Number(b.seq ?? b.timestamp) || 0;
      return (
        aOrder - bOrder ||
    ...
    return this.mapRowsToEvents(latestRows, {
  ...
  findLatestConversationSnapshot(taskId: string): TaskEvent | null {
    ...
    return row
      ? (this.mapRowsToEvents([row], { persistMigrations: false }).events[0] ??
          null)
      : null;
  }
  ...
  findEventCursorById(
    ...
    const row = this.db
      ...
      { id?: string; timeline_order?: number; timestamp?: number } | undefined;
  ...
  findReplayTailAfterCursor(
    ...
    const boundaryOrder =
      typeof afterCursor?.order === "number" &&
      Number.isFinite(afterCursor.order)
        ? Math.floor(afterCursor.order)
        : 0;
    const boundaryTimestamp =
      typeof afterCursor?.timestamp === "number" &&
      Number.isFinite(afterCursor.timestamp)
        ? Math.floor(afterCursor.timestamp)
        : 0;
    const boundaryId =
      typeof afterCursor?.id === "string" && afterCursor.id.trim()
        ? afterCursor.id.trim()
        : "";
    const safeLimit =
  ...
  findTimelinePage(request: TaskTimelinePageRequest): TaskTimelinePageResult {
    ...
    const additionalTaskIds = Array.from(
      new Set(
        (Array.isArray(request.additionalTaskIds)
          ? request.additionalTaskIds
          : []
        )
          .map((value) => (typeof value === "string" ? value.trim() : ""))
    ...
    const cursorOrder =
      cursor &&
      typeof cursor.order === "number" &&
      Number.isFinite(cursor.order)
    ...
    if (additionalTaskIds.length > 0 && additionalTaskEventTypes.length > 0) {
      ...
      const typePlaceholders = additionalTaskEventTypes
        .map(() => "?")
        .join(", ");
      for (
        let index = 0;
    ...
    for (const row of rows.slice(0, safeLimit)) {
      const rowPayloadBytes =
        ...
        Number.isFinite(row.payload_bytes)
          ? row.payload_bytes
          : Buffer.byteLength(String(row.payload ?? ""), "utf8");
      largestEventPayloadBytes = Math.max(
      ...
      const rawHydratedBytes = Buffer.byteLength(
        String(row.payload ?? ""),
        "utf8",
      );
      const hydratedBytes =
    ...
    if (selectedRows.length === 0 && rows.length > 0) {
      ...
      hydratedPayloadBytes = hydratedBytes;
      previewPayloadBytes =
        rowPayloadBytes > singleEventByteLimit ? hydratedBytes : 0;
      selectedRows.push(
        rowPayloadBytes > singleEventByteLimit
          ? this.buildTimelineTruncatedPayloadRow(row, rowPayloadBytes)
          : row,
    ...
    const planContextRow =
      cursorWhere.length === 0
        ? this.findLatestTimelinePlanContextRow(
            taskId,
    ...
    for (const contextRow of uniqueContextRows) {
      const contextPayloadBytes =
        typeof contextRow.payload_bytes === "number" &&
        Number.isFinite(contextRow.payload_bytes)
    ...
    const nextCursor =
      hasMoreHistory && oldestSelected
        ? {
            order:
              Number(
                oldestSelected.timeline_order ??
                  oldestSelected.seq ??
                  oldestSelected.timestamp,
              ) || 0,
            timestamp: Number(oldestSelected.timestamp) || 0,
            id:
              typeof oldestSelected.id === "string"
                ? oldestSelected.id
                : undefined,
          }
        : null;
  ...
  private findLatestTimelinePlanContextRow(
    taskId: string,
    ...
    if (!taskId || normalizedTypes.length === 0) return null;
    const selectedIds = new Set(
      selectedRows
        .map((row) => (typeof row.id === "string" ? row.id : ""))
        .filter((id) => id.length > 0),
    ...
    const row = this.db
      .prepare(
        ...
        WHERE task_id = ?
          ...
          AND json_type(payload, '$.plan.steps') = 'array'
        ORDER BY COALESCE(seq, timestamp) DESC, timestamp DESC, id DESC
        LIMIT 1
      `,
      )
      .get(taskId, ...normalizedTypes) as Any;
  ...
  /**
   * Pin durable delivery state into the first timeline page.
   *
   * Follow-up turns can push the original result and file events outside the
   * newest page. The transcript and workspace panel still need those small
   * durable records after an app restart, without loading the full tool trace.
   */
  private findDurableTimelineContextRows(
    taskId: string,
    selectedRows: Any[],
    singleEventByteLimit: number,
  ): Any[] {
    ...
    const pathExpression =
      "TRIM(COALESCE(json_extract(payload, '$.path'), json_extract(payload, '$.to'), json_extract(payload, '$.filePath'), json_extract(payload, '$.file_path'), ''))";
    const rows = this.db
      .prepare(
        `
        /* durable_timeline_context */
        SELECT
          id,
          task_id,
          timestamp,
        ...
        WHERE task_id = ?
          AND (
            COALESCE(legacy_type, type) = 'task_completed'
            OR (
              COALESCE(legacy_type, type) IN ('file_created', 'file_modified', 'file_deleted')
              AND json_valid(payload)
              AND ${pathExpression} <> ''
              AND COALESCE(json_extract(payload, '$.source'), '') <> 'artifact_bootstrap'
              AND COALESCE(json_extract(payload, '$.provisional'), 0) <> 1
              AND LOWER(${pathExpression}) NOT LIKE '%.neoworker/tmp/%'
              AND LOWER(${pathExpression}) NOT LIKE '%.neoworker/uploads/%'
              AND LOWER(${pathExpression}) NOT LIKE '%agent.md/%'
            )
          )
        ORDER BY COALESCE(seq, timestamp) DESC, timestamp DESC, id DESC
        LIMIT 160
  ...
  private isPersistedPlanDefinitionRow(row: Any): boolean {
    const effectiveType = String(row?.legacy_type || row?.type || "").trim();
    if (
    ...
    try {
      const payload =
        typeof row?.payload === "string"
          ...
          : row?.payload || {};
      return Array.isArray(payload?.plan?.steps);
    } catch {
      return false;
    }
  }
  ...
  private findLatestPlanStepContextRows(
    ...
    try {
      payload =
        typeof planRow.payload === "string"
          ? JSON.parse(planRow.payload)
          : planRow.payload || {};
    } catch {
      return [];
    }
    const rawSteps = Array.isArray(payload?.plan?.steps)
      ? payload.plan.steps
      : [];
    ...
    const planOrder =
      Number(planRow.timeline_order ?? planRow.seq ?? planRow.timestamp) || 0;
    const rows = this.db
      .prepare(
        `
        SELECT
  ...
  findEventDetailById(
    ...
    const normalizedTaskId =
      typeof scope?.taskId === "string" ? scope.taskId.trim() : "";
    const additionalTaskIds = Array.from(
      new Set(
    ...
    const additionalTaskEventTypes = Array.from(
      new Set(
        (Array.isArray(scope?.additionalTaskEventTypes)
          ? scope.additionalTaskEventTypes
          : []
        )
          .map((type) => (typeof type === "string" ? type.trim() : ""))
          .filter(Boolean),
    ...
    if (normalizedTaskId) {
      ...
      if (
        !row &&
        additionalTaskIds.length > 0 &&
        additionalTaskEventTypes.length > 0
      ) {
        const typePlaceholders = additionalTaskEventTypes
          .map(() => "?")
          .join(", ");
  ...
  updatePayloadById(eventId: string, payload: Record<string, unknown>): void {
    ...
    `);
    stmt.run(
      JSON.stringify(sanitizeTimelinePayloadForStorage(payload ?? {})),
      normalizedEventId,
    );
  }

  private buildTimelineTruncatedPayloadRow(
    row: Any,
    payloadBytes: number,
  ): Any {
    ...
    for (const event of events) {
      ...
      if (
        (effectiveType === "plan_created" ||
          effectiveType === "plan_updated" ||
          effectiveType === "plan_revised") &&
        Array.isArray((payload as Any).plan?.steps)
      ) {
        planStepCount = (payload as Any).plan.steps.length;
      }
      if (
        (payload as Any).checklist &&
        Array.isArray((payload as Any).checklist.items)
      ) {
        hasChecklist = true;
      }
      if (
  ...
  private buildTimelinePageWarnings(
    ...
    if (truncatedEventCount > 0) {
      warnings.push(
        `task ${taskId} returned ${truncatedEventCount} truncated timeline events`,
      );
    }
  ...
  private mapRowsToEvents(
    ...
    for (const row of rows) {
      ...
      const isV2 =
        Number(row.schema_version) === 2 && isTimelineEventType(row.type);
      if (isV2) {
        events.push({
          id: row.id,
          taskId,
          ...
          legacyType:
            typeof row.legacy_type === "string" ? row.legacy_type : undefined,
        });
        continue;
      }
  ...
  getLatestSeq(taskId: string): number {
    const row = this.db
      .prepare(
        "SELECT MAX(COALESCE(seq, 0)) as max_seq FROM task_events WHERE task_id = ?",
      )
      .get(taskId) as { max_seq?: number } | undefined;
    const maxSeq = row?.max_seq;
    return typeof maxSeq === "number" && Number.isFinite(maxSeq)
      ? Math.floor(maxSeq)
      : 0;
  }
  ...
  migrateLegacyEventsForTask(taskId: string): number {
    ...
    const legacyCount =
      typeof legacyCountRow?.count === "number" &&
      Number.isFinite(legacyCountRow.count)
  ...
  pruneOldSnapshots(taskId: string): void {
    ...
    `);
    const snapshots = findStmt.all(taskId) as {
      id: string;
      timestamp: number;
    }[];
  ...
  vacuumIfNeeded(thresholdMB: number = 500): boolean {
    const freelistCount =
      (this.db.pragma("freelist_count") as { freelist_count: number }[])[0]
        ?.freelist_count ?? 0;
    const pageSize =
      (this.db.pragma("page_size") as { page_size: number }[])[0]?.page_size ??
      4096;
...
export class TaskTraceRepository {
  ...
  private listSessionTasks(task: Task): Task[] {
    if (!(
      typeof task.sessionId === "string" && task.sessionId.trim().length > 0
    )) {
      return [task];
    }
    ...
    this.db
      ...
      .run(
        ...
        patch.artifactId === null
          ? null
          : patch.artifactId || current.artifactId || null,
  ...
  private mapRowToAnnotation(row: Any): Annotation {
    ...
    return {
      ...
      stylePatch: row.style_patch_json
        ? safeJsonParse(
            row.style_patch_json,
            undefined,
            "annotation.style_patch_json",
          )
        : undefined,
...
export class WorkspacePermissionRuleRepository {
  ...
  deleteById(id: string): PersistedPermissionRule | null {
    ...
    this.db
      .prepare(`DELETE FROM workspace_permission_rules WHERE id = ?`)
      .run(id);
    return existing;
  }
  ...
  private mapRowToRule(row: Any): PersistedPermissionRule {
    ...
    return {
      ...
      metadata: safeJsonParse(
        row.metadata_json,
        {},
        "workspacePermissionRule.metadata",
      ),
      createdAt: row.created_at,
    };
  }
...
export class InputRequestRepository {
  ...
  private mapRowToInputRequest(row: Any): InputRequest {
    return {
      ...
      answers: row.answers
        ? safeJsonParse<InputRequest["answers"]>(
            row.answers,
            undefined,
            "inputRequest.answers",
          )
        : undefined,
...
function encryptChannelConfig(json: string): string {
  try {
    ...
    if (safeStorage?.isEncryptionAvailable()) {
      return (
        CHANNEL_CONFIG_ENCRYPTED_PREFIX +
  ...
  try {
    ...
    if (safeStorage?.isEncryptionAvailable()) {
      const buf = Buffer.from(
        value.slice(CHANNEL_CONFIG_ENCRYPTED_PREFIX.length),
        "base64",
      );
      return {
...
export class ChannelRepository {
  ...
  update(id: string, updates: Partial<Channel>): void {
    const existingChannel =
      updates.config !== undefined ? this.findById(id) : undefined;
  ...
  findAllByType(type: string): Channel[] {
    const stmt = this.db.prepare(
      "SELECT * FROM channels WHERE type = ? ORDER BY created_at ASC",
    );
...
export class ChannelUserRepository {
  ...
  create(
    user: Omit<
      ChannelUser,
      "id" | "createdAt" | "lastSeenAt" | "pairingAttempts"
    >,
  ): ChannelUser {
    const now = Date.now();
  ...
  update(id: string, updates: Partial<ChannelUser>): void {
    ...
    values.push(id);
    const stmt = this.db.prepare(
      `UPDATE channel_users SET ${fields.join(", ")} WHERE id = ?`,
    );
    stmt.run(...values);
  }
  ...
  findByChannelUserId(
    ...
    const row = stmt.get(channelId, channelUserId) as
  ...
  findByPairingCode(
    ...
    const row = stmt.get(channelId, pairingCode) as
      Record<string, unknown> | undefined;
    return row ? this.mapRowToUser(row) : undefined;
  }
...
export class ChannelSessionRepository {
  ...
  update(id: string, updates: Partial<ChannelSession>): void {
    ...
    const hasContextUpdate =
      "context" in updates ||
      "shellEnabled" in updates ||
      "debugMode" in updates;
    if (hasContextUpdate) {
      ...
      const newContext = {
        ...
        ...("shellEnabled" in updates
          ? { shellEnabled: updates.shellEnabled }
          : {}),
    ...
    values.push(id);
    const stmt = this.db.prepare(
      `UPDATE channel_sessions SET ${fields.join(", ")} WHERE id = ?`,
    );
    stmt.run(...values);
  }
  ...
  findByChatId(channelId: string, chatId: string): ChannelSession | undefined {
    ...
    const row = stmt.get(channelId, chatId) as
      Record<string, unknown> | undefined;
    return row ? this.mapRowToSession(row) : undefined;
  }
  ...
  findByTaskId(taskId: string): ChannelSession | undefined {
    const stmt = this.db.prepare(
      "SELECT * FROM channel_sessions WHERE task_id = ?",
    );
...
export class ChannelSpecializationRepository {
  ...
  update(
    request: UpdateChannelSpecializationRequest,
    ...
    if ("chatId" in request)
      push("chat_id", this.cleanOptionalString(request.chatId) ?? null);
    if ("threadId" in request)
      push("thread_id", this.cleanOptionalString(request.threadId) ?? null);
    if ("name" in request)
      push("name", this.cleanOptionalString(request.name) ?? null);
    if ("workspaceId" in request)
      push(
        "workspace_id",
        this.cleanOptionalString(request.workspaceId) ?? null,
      );
    if ("agentRoleId" in request)
      push(
        "agent_role_id",
        this.cleanOptionalString(request.agentRoleId) ?? null,
      );
    if ("systemGuidance" in request)
      push(
        "system_guidance",
        this.cleanOptionalString(request.systemGuidance) ?? null,
      );
    ...
    if ("allowSharedContextMemory" in request)
      push(
        "allow_shared_context_memory",
        request.allowSharedContextMemory ? 1 : 0,
      );
    ...
    this.db
      .prepare(
        `UPDATE channel_specializations SET ${fields.join(", ")} WHERE id = ?`,
      )
      .run(...values);
  ...
  delete(id: string): boolean {
    const result = this.db
      .prepare("DELETE FROM channel_specializations WHERE id = ?")
      .run(id);
...
export class ChannelMessageRepository {
  ...
  findByChatId(
    ...
    const rows = stmt.all(channelId, chatId, limit) as Record<
      string,
      unknown
    >[];
    return rows.map((row) => this.mapRowToMessage(row)).reverse();
  ...
  private mapRowToMessage(row: Record<string, unknown>): ChannelMessage {
    ...
    const direction: ChannelMessage["direction"] =
      directionRaw === "outgoing" || directionRaw === "outgoing_user"
        ? directionRaw
        : "incoming";
...
function asTaskProvenanceObject(
  value: unknown,
): Record<string, unknown> | undefined {
...
export class TaskProvenanceRepository {
  ...
  listByTaskIds(taskIds: string[], limitPerTask = 3): TaskProvenanceRecord[] {
    const uniqueTaskIds = Array.from(new Set(taskIds.filter(Boolean))).slice(
      0,
      100,
    );
  ...
  deleteByTaskId(taskId: string): number {
    return this.db
      .prepare("DELETE FROM task_provenance WHERE task_id = ?")
      .run(taskId).changes;
  }
...
function normalizeTaskAccessWorkspaceScopes(
  scopes: unknown,
): TaskAccessWorkspaceScope[] {
...
export class TaskAccessPolicyRepository {
  ...
  createInitial(
    taskId: string,
    policy: Omit<TaskAccessPolicy, "taskId" | "revision">,
  ): TaskAccessPolicy {
  ...
  delete(taskId: string): number {
    return this.db
      .prepare("DELETE FROM task_access_policies WHERE task_id = ?")
      .run(taskId).changes;
  }
  ...
  private normalize(policy: TaskAccessPolicy): TaskAccessPolicy {
    return {
      ...
      workspaceScopes: normalizeTaskAccessWorkspaceScopes(
      ...
      effectiveFromTurn:
        typeof policy.effectiveFromTurn === "number" &&
        Number.isFinite(policy.effectiveFromTurn)
  ...
  private mapRow(row: Record<string, unknown>): TaskAccessPolicy {
    return this.normalize({
      ...
      connectorIds: safeJsonParse(
        String(row.connector_ids_json || "[]"),
        [],
        "taskAccess.connectorIds",
      ),
      workspaceScopes: safeJsonParse(
        String(row.workspace_scopes_json || "[]"),
        [],
        "taskAccess.workspaceScopes",
      ),
      allowedTools: row.allowed_tools_json
        ? safeJsonParse(
            String(row.allowed_tools_json),
            [],
            "taskAccess.allowedTools",
          )
        : undefined,
      blockedTools: row.blocked_tools_json
        ? safeJsonParse(
            String(row.blocked_tools_json),
            [],
            "taskAccess.blockedTools",
          )
        : undefined,
      permissionMode:
        (row.permission_mode as TaskAccessPolicy["permissionMode"]) ||
        undefined,
      shellAccess: Number(row.shell_access || 0) === 1,
      effectiveFromTurn:
        row.effective_from_turn == null
          ? undefined
          : Number(row.effective_from_turn),
...
export class MessageQueueRepository {
  ...
  enqueue(
    item: Omit<QueuedMessage, "id" | "createdAt" | "attempts" | "status">,
  ): QueuedMessage {
  ...
  update(id: string, updates: Partial<QueuedMessage>): void {
    ...
    values.push(id);
    const stmt = this.db.prepare(
      `UPDATE message_queue SET ${fields.join(", ")} WHERE id = ?`,
    );
    stmt.run(...values);
  }
...
export class ScheduledMessageRepository {
  ...
  update(id: string, updates: Partial<ScheduledMessage>): void {
    ...
    values.push(id);
    const stmt = this.db.prepare(
      `UPDATE scheduled_messages SET ${fields.join(", ")} WHERE id = ?`,
    );
    stmt.run(...values);
  }
...
export class DeliveryTrackingRepository {
  ...
  update(id: string, updates: Partial<DeliveryRecord>): void {
    ...
    values.push(id);
    const stmt = this.db.prepare(
      `UPDATE delivery_tracking SET ${fields.join(", ")} WHERE id = ?`,
    );
    stmt.run(...values);
  }
  ...
  findByChatId(
    ...
    const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
    const rows = stmt.all(channelType, chatId, limit) as Record<
      string,
      unknown
    >[];
    return rows.map((row) => this.mapRowToItem(row));
  ...
  deleteOld(olderThanMs: number): number {
    const cutoff = Date.now() - olderThanMs;
    const stmt = this.db.prepare(
      "DELETE FROM delivery_tracking WHERE created_at < ?",
    );
    const result = stmt.run(cutoff);
    return result.changes;
  }
...
export class RateLimitRepository {
  ...
  getOrCreate(channelType: string, userId: string): RateLimitRecord {
    ...
    const row = stmt.get(channelType, userId) as
      Record<string, unknown> | undefined;
  ...
  update(
    channelType: string,
    userId: string,
    updates: Partial<RateLimitRecord>,
  ): void {
    const fields: string[] = [];
    const values: unknown[] = [];
...
export type PendingMemoryWriteStatus =
  "pending" | "applying" | "applied" | "rejected" | "failed";
...
export class MemoryRepository {
  ...
  update(
    id: string,
    updates: Partial<
      Pick<Memory, "summary" | "tokens" | "isCompressed" | "content">
    >,
  ): void {
    const fields: string[] = [];
    const values: unknown[] = [];
  ...
  searchByContentMarker(
    ...
    const like = `%${marker}%`;
    return mapRows(
      likeStmt.all(workspaceId, like, like, limit) as Record<string, unknown>[],
    );
  }
  ...
  findByWorkspace(workspaceId: string, limit = 100, offset = 0): Memory[] {
    const stmt = this.db.prepare(`
      ...
      LIMIT ? OFFSET ?
    `);
    const rows = stmt.all(workspaceId, limit, offset) as Record<
      string,
      unknown
    >[];
    return rows.map((row) => this.mapRowToMemory(row));
   ...
   */
  getImportedStats(workspaceId: string): {
    count: number;
    totalTokens: number;
  } {
    const stmt = this.db.prepare(`
  ...
  private tokenizeSearchQuery(raw: string): string[] {
    return (raw || "")
      ...
      .filter(
        (t) =>
          t.length > 1 && !MemoryRepository.MEMORY_SEARCH_STOP_WORDS.has(t),
      );
  }
  ...
  private shouldTryRawFtsQuery(raw: string): boolean {
    ...
    return (
      this.tokenizeSearchQuery(raw).length <=
      MemoryRepository.MEMORY_FTS_RAW_MAX_TOKENS
    );
  }
...
export class CuratedMemoryRepository {
  ...
  update(
    ...
    this.db
      .prepare(
        `UPDATE curated_memory_entries SET ${fields.join(", ")} WHERE id = ?`,
      )
      .run(...values);
  ...
  findByNormalizedKey(
    ...
    const row = this.db
      ...
      .get(workspaceId, target, kind, normalizedKey) as
      Record<string, unknown> | undefined;
    return row ? this.mapRow(row) : undefined;
  }
...
export class MemoryEmbeddingRepository {
  ...
  getImportedGlobal(
    limit = 5000,
    offset = 0,
  ): Array<MemoryEmbedding & { workspaceId: string }> {
    const stmt = this.db.prepare(`
...
export class MemorySummaryRepository {
  ...
  findByWorkspaceAndPeriod(
    ...
    const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
    const rows = stmt.all(workspaceId, timePeriod, limit) as Record<
      string,
      unknown
    >[];
  ...
  deleteByWorkspace(workspaceId: string): number {
    const stmt = this.db.prepare(
      "DELETE FROM memory_summaries WHERE workspace_id = ?",
    );
    const result = stmt.run(workspaceId);
    return result.changes;
  }
  ...
  private mapRowToSummary(row: Record<string, unknown>): MemorySummary {
    return {
      ...
      memoryIds: safeJsonParse(
        row.memory_ids as string,
        [] as string[],
        "memorySummary.memoryIds",
      ),
      tokens: row.tokens as number,
      createdAt: row.created_at as number,
    };
...
export class MemorySettingsRepository {
  ...
  delete(workspaceId: string): void {
    const stmt = this.db.prepare(
      "DELETE FROM memory_settings WHERE workspace_id = ?",
    );
...
export class PendingMemoryWriteRepository {
  ...
  countPending(workspaceId?: string): number {
    ...
    const row = this.db
      .prepare(
        "SELECT COUNT(*) AS count FROM pending_memory_writes WHERE status = 'pending'",
      )
...
export class WorktreeInfoRepository {
  ...
  findByTaskId(taskId: string): WorktreeInfo | undefined {
    const stmt = this.db.prepare(
      "SELECT * FROM worktree_info WHERE task_id = ?",
    );
    const row = stmt.get(taskId) as Record<string, unknown> | undefined;
    return row ? this.mapRow(row) : undefined;
  }
  ...
  update(taskId: string, updates: Partial<WorktreeInfo>): void {
    ...
    values.push(taskId);
    const stmt = this.db.prepare(
      `UPDATE worktree_info SET ${fields.join(", ")} WHERE task_id = ?`,
    );
    stmt.run(...values);
  }
...
export class ComparisonSessionRepository {
  ...
  create(
    params: Omit<ComparisonSession, "id" | "createdAt">,
  ): ComparisonSession {
    ...
    stmt.run(
      ...
      session.comparisonResult
        ? JSON.stringify(session.comparisonResult)
        : null,
    );
    return session;
  }
  ...
  delete(id: string): void {
    const stmt = this.db.prepare(
      "DELETE FROM comparison_sessions WHERE id = ?",
    );
    stmt.run(id);
  }
  ...
  syncTaskIdsFromTasks(sessionId: string): string[] {
    ...
    const stmt = this.db.prepare(
      "UPDATE comparison_sessions SET task_ids = ? WHERE id = ?",
    );
