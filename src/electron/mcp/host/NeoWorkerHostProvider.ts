...
export class NeoWorkerHostProvider {
  constructor(
    private deps: {
  ...
  getResources(): MCPResource[] {
    ...
    for (const workspace of workspaces) {
      resources.push({
        uri: `neoworker://workspaces/${workspace.id}`,
        name: workspace.name,
    ...
    for (const task of tasks) {
      resources.push(
        {
          uri: `neoworker://tasks/${task.id}`,
          name: task.title,
          description: `Task ${task.status}`,
          mimeType: "application/json",
        },
        {
          uri: `neoworker://tasks/${task.id}/events`,
          name: `${task.title} events`,
          description: "Ordered task event log",
          mimeType: "application/json",
        },
        {
          uri: `neoworker://tasks/${task.id}/artifacts`,
      ...
      for (const artifact of this.deps.artifactRepo.findByTaskId(task.id).slice(0, 10)) {
        resources.push({
          uri: `neoworker://artifacts/${artifact.id}`,
