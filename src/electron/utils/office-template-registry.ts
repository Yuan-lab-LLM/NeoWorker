...
const BUILT_IN_TEMPLATES: OfficeTemplateDefinition[] = [
  ...
  ...(["docx", "xlsx"] as const).flatMap((format) => [
    {
      id: `neoworker-${format}-research-report`,
      version: "1.0.0",
      format,
      useCases: ["research-report", "financing-analysis"] as OfficeTemplateUseCase[],
      label: { zh: format === "docx" ? "专业研究报告" : "分析工作簿", en: format === "docx" ? "Professional report" : "Analysis workbook" },
      tokens: {
        ...BASE,
        primaryColor: "176B87",
        accentColor: "F59E0B",
        ...
        visualMode: "research" as const,
        density: "compact" as const,
      },
    },
    {
      id: `neoworker-${format}-baseline`,
      version: "1.0.0",
      format,
      useCases: ["general", "operating-review", "product-launch", "teaching-deck"] as OfficeTemplateUseCase[],
      label: { zh: format === "docx" ? "商务文档" : "商务表格", en: format === "docx" ? "Business document" : "Business workbook" },
      tokens: {
        ...BASE,
        primaryColor: "2563EB",
        accentColor: "10B981",
