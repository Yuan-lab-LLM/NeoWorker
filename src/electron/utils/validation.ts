...
export const AgentConfigSchema = z
  .object({
    ...
    maxPlanSteps: z.number().int().min(2).max(8).optional(),
...
export const ImageAttachmentSchema = z
  .object({
    ...
    videoFramePaths: z
      .array(z.string().trim().max(MAX_PATH_LENGTH))
      .max(12)
      .optional(),
  ...
  imageGeneration: z
    .object({
      defaultProvider: z
        ...
        .optional(),
      defaultModel: z
        .enum(["gpt-image-2", "gpt-image-1.5", "nano-banana-2"])
        .optional(),
      backupProvider: z
        .enum(["openai", "openai-codex", "azure", "openrouter", "gemini"])
        .optional(),
  ...
  weixinBaseUrl: z
    ...
    .refine((value) => {
      try {
        ...
        return (
          url.protocol === "https:" && url.hostname === "ilinkai.weixin.qq.com"
        );
      } catch {
        return false;
      }
...
const HEARTBEAT_PROFILE_VALUES = [
  "observer",
  "operator",
  "dispatcher",
] as const;
...
const CommunicationStyleObjectSchema = z.object({
  ...
  structurePreference: z
    .enum(["freeform", "bullets", "structured", "headers"])
    .optional(),
