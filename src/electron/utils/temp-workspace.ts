...
const hasWorkspaceReferences = (
  ...
  const taskRef = db
    .prepare("SELECT 1 FROM tasks WHERE workspace_id = ? LIMIT 1")
    .get(workspaceId);
...
export function pruneTempWorkspaces(options: TempWorkspacePruneOptions): TempWorkspacePruneResult {
  ...
  // A completed or failed task may receive a follow-up later. Keep its source
  // workspace until the task itself is removed by session retention; otherwise
  // persisted relative attachment paths become dangling after a restart.
  ...
  for (const workspaceId of toDeleteIds) {
    ...
    if (hasWorkspaceReferences(options.db, workspaceId, sessionActiveCutoffMs)) {
      continue;
    }
