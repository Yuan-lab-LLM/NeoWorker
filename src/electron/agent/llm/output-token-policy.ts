...
export type OutputBudgetRequestKind =
  "agentic_main" | "tool_followup" | "continuation";
...
export type OutputBudgetProviderFamily =
  ...
  | "openrouter"
  | "generic";
export type OutputTokenParamName =
  "max_tokens" | "max_completion_tokens" | "max_output_tokens";
export type OutputTruncationClassification =
  "visible_partial_output" | "reasoning_exhausted";
...
// Keep tool turns large enough for structured payloads without advertising an
// enormous completion to slower OpenAI-compatible gateways. Large budgets made
// providers such as Kimi spend minutes before a transport timeout, even when
// the expected result was a compact create_spreadsheet call.
const DEFAULT_TOOL_FOLLOW_UP_INITIAL_MAX_TOKENS = 12_000;
const DEFAULT_CONTINUATION_MAX_TOKENS = 16_000;
const DEFAULT_LARGE_ARTIFACT_INITIAL_MAX_TOKENS = 16_000;
const DEFAULT_LARGE_ARTIFACT_ESCALATED_MAX_TOKENS = 32_000;
...
function estimateContextLimit(
  ...
  if (manager && typeof manager.estimateMaxOutputTokens === "function") {
    const estimated = normalizePositiveInteger(
      manager.estimateMaxOutputTokens(messages, system),
    );
...
function getPolicyDefault(
  ...
  const initialOverride = readEnvLimit(
    "NEOWORKER_LLM_AGENTIC_INITIAL_MAX_TOKENS",
  );
  const escalatedOverride = readEnvLimit(
  ...
  if (phase === "escalated") {
    ...
    if (effectiveFamily === "anthropic")
      return DEFAULT_ANTHROPIC_ESCALATED_MAX_TOKENS;
    if (effectiveFamily === "generic")
      return DEFAULT_GENERIC_ESCALATED_MAX_TOKENS;
...
export function inferOutputBudgetRequestKind(
  messages: LLMMessage[],
): OutputBudgetRequestKind {
...
export function inferOutputWorkloadProfile(
  ...
  const explicitArtifact =
    /\.(?:html?|pptx?|docx?|xlsx?|pdf|csv|md|tsx?|jsx?|py|java|go|rs)\b/i.test(
      text,
    ) ||
...
export function resolveOutputTokenParamName(opts: {
  ...
  if (providerType === "azure" && apiMode === "responses")
...
export function resolveOutputTokenBudget(
  ...
  const contextLimit = estimateContextLimit(
  ...
  const knownHardCap = getKnownHardCap(
    providerFamily,
    routedFamily,
    input.modelId,
  );
...
export function buildReasoningExhaustedGuidance(): string {
  return (
    "模型在自动切换为直接行动模式后，仍未产出可用内容。" +
    "这通常表示当前模型或接入服务存在单次输出硬上限；任务上下文已保留，可以继续重试。"
  );
}
