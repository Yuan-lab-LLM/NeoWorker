...
function normalizeProviderConfig(config: LLMProviderConfig): LLMProviderConfig {
  return {
    ...
    openaiCompatibleSupportsImages: config.openaiCompatibleSupportsImages,
    ...
    providerSupportsImages: config.providerSupportsImages,
  };
}
...
export class LLMProviderFactory {
  ...
  private static selectionSupportsImageInput(
    ...
    return this.getImageCapabilities(selection.providerType, selection.modelId)
      .supportsImages;
  }

  /**
   * Resolve image input support using an explicit custom-provider override
   * first, then safe model-name inference and provider defaults.
   */
  static getImageCapabilities(
    providerType: LLMProviderType,
    modelId?: string,
  ): LLMProviderImageCaps {
    const resolvedProviderType = resolveCustomProviderId(providerType);
    const settings = this.loadSettings();
    const customEntry = getCustomProviderEntry(resolvedProviderType);
    const explicitSupportsImages = customEntry
      ? getCustomProviderConfig(settings.customProviders, resolvedProviderType)
          ?.supportsImages
      : resolvedProviderType === "openai-compatible"
        ? settings.openaiCompatible?.supportsImages
        : undefined;

    return getProviderImageCaps(
      resolvedProviderType,
      modelId,
  ...
  static createProvider(
    ...
    const config: LLMProviderConfig = {
      ...
      openaiCompatibleBaseUrl:
        ...
        settings.openaiCompatible?.baseUrl,
      openaiCompatibleSupportsImages:
        overrideConfig?.openaiCompatibleSupportsImages ??
        settings.openaiCompatible?.supportsImages,
      ...
      providerSupportsImages:
        overrideConfig?.providerSupportsImages ?? customConfig?.supportsImages,
    };
    ...
    switch (config.type) {
      ...
      case "openai-compatible": {
        ...
        provider = new ProviderClass({
          ...
          supportsImages: config.openaiCompatibleSupportsImages,
        });
        break;
      }
      case "moa": {
  ...
  static async getCustomProviderModels(
    ...
    try {
      ...
      if (fallbackCachedModels.length === 0) throw error;
      const updatedSettings = this.loadSettings();
      updatedSettings.customProviders = {
        ...updatedSettings.customProviders,
        [resolvedProviderType]: {
