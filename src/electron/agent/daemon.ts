...
import {
  buildEntropySweepPrompt,
  collectBlastRadiusPaths,
} from "./post-task-entropy-sweep";
import {
...
function buildStructuredInputSelectionMessage(
  ...
  for (const question of request.questions) {
    ...
    lines.push(
      `- ${label}: ${truncate(selection || "no selection recorded", 220)}`,
    );
  }
...
function normalizeReadOnlyRoleToolRestrictions(
  ...
  if (
    !role.isSystem ||
    !READ_ONLY_SHELL_CAPABLE_SYSTEM_ROLE_NAMES.has(role.name)
  ) {
    return normalized;
  }
...
function sanitizeTaskOverrides(
...
function buildRecoveredApprovalOnceGrantKey(
...
export class AgentDaemon extends EventEmitter {
   ...
   */
  private deferredUserFollowUps: Map<string, DeferredUserFollowUp[]> =
    new Map();
  ...
  private recoveredApprovalGrants: Map<string, RecoveredApprovalGrant[]> =
    new Map();
  ...
  private pendingRetries: Map<string, ReturnType<typeof setTimeout>> =
    new Map();
  ...
  constructor(
    ...
    this.workspacePermissionRuleRepo = new WorkspacePermissionRuleRepository(
      db,
    );
    ...
    this.orchestrationGraphEngine.on("node_notification", (notification) => {
      void this.handleOrchestrationNodeNotification(notification).catch(
        (error) => {
          console.error(
            "[AgentDaemon] Orchestration notification handling failed:",
            error,
          );
        },
      );
  ...
  private isStaleAttachedCliTask(task: Task): boolean {
    ...
    const lastSeenAt =
      cli.lastSeenAt || cli.startedAt || task.updatedAt || task.createdAt;
  ...
  private getCliTaskOwnership(task: Task): CliTaskOwnership | undefined {
    ...
    if (!cli || cli.owner !== "neoworker-run" || typeof cli.runId !== "string")
      return undefined;
    return cli;
  }
  ...
  private async runSessionAutoPrune(
    taskEventRepo: TaskEventRepository,
  ): Promise<void> {
    ...
    const minIntervalHours =
      typeof autoPrune.minIntervalHours === "number" &&
      Number.isFinite(autoPrune.minIntervalHours)
        ? Math.max(1, autoPrune.minIntervalHours)
        : 24;
    const lastRunAt =
      typeof autoPrune.lastRunAt === "number" &&
      Number.isFinite(autoPrune.lastRunAt)
        ? autoPrune.lastRunAt
        : 0;
    if (Date.now() - lastRunAt < minIntervalHours * 60 * 60 * 1000) return;

    const olderThanMs = parseSessionRetentionDurationMs(
      autoPrune.olderThan || "90d",
    );
    if (!olderThanMs) {
      ...
      },
      {
        deleteTask: async (task) => {
          ...
          try {
            ...
            log.warn(
              `Session auto-prune worktree cleanup failed for ${task.id}:`,
              error,
            );
          }
        },
  ...
  private applyTaskWorkspaceOverrides(
    task: Task,
    workspace: Workspace,
  ): Workspace {
    const accessSnapshot = this.taskAccessSnapshotByTaskId.get(task.id);
    if (accessSnapshot) {
      const scope = accessSnapshot.workspaceScopes.find(
      ...
      return {
        ...
        permissions: {
          ...
          write: Boolean(
            scope?.access === "write" && workspace.permissions.write,
          ),
          delete: Boolean(
            scope?.access === "write" && workspace.permissions.delete,
          ),
  ...
  private applyTaskFollowUpOverrides(
    ...
    if (hasExecutionMode) {
      ...
      const nextConversationMode =
        nextExecutionMode === "chat" ? "chat" : "task";
  ...
  private syncTaskAccessPolicyForFollowUp(
    ...
    const nextTurn = Math.max(
      ...
      (task.lifetimeTurnsUsed ||
        task.budgetUsage?.lifetimeTurns ||
        task.budgetUsage?.turns ||
        0) + 1,
    );
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        updatedPolicy = this.taskAccessPolicyRepo.update(
          task.id,
          current.revision,
          patch,
        );
        ...
        if (!(error instanceof TaskAccessPolicyConflictError) || attempt > 0)
          throw error;
      }
    }
    ...
    if (updatedPolicy) {
      this.taskAccessSnapshotByTaskId.set(task.id, {
        ...
        workspaceScopes: updatedPolicy.workspaceScopes.map((scope) => ({
          ...scope,
        })),
        allowedTools: updatedPolicy.allowedTools
          ? [...updatedPolicy.allowedTools]
          : undefined,
        blockedTools: updatedPolicy.blockedTools
          ? [...updatedPolicy.blockedTools]
          : undefined,
      });
    }
    return true;
   ...
   */
  private applyAgentRoleOverrides(task: Task): {
    task: Task;
    changed: boolean;
  } {
    ...
    const denied = normalizeReadOnlyRoleToolRestrictions(
      role,
      role.toolRestrictions?.deniedTools,
    );
    ...
    return {
      task: changed ? { ...task, agentConfig: nextAgentConfig } : task,
      changed,
    };
  }
  ...
  private maybeCaptureMentionedAgentRoleIds(task: Task): void {
    ...
    try {
      const activeRoles = this.agentRoleRepo
        .findAll(false)
        .filter((role) => role.isActive);
      if (activeRoles.length === 0) return;

      const mentioned = extractMentionedRoles(
        `${task.title}\n${task.prompt}`,
        activeRoles,
      );
      ...
      console.warn(
        "[AgentDaemon] Failed to capture mentioned agent roles:",
        error,
      );
    }
  }
  ...
  private deriveTaskStrategy(input: {
    ...
    const canonicalRoutingPrompt = buildCanonicalTaskIntentQuery({
      title: input.title,
      prompt: input.prompt,
      rawPrompt: input.routingPrompt ?? input.prompt,
    });
    const routingSource = String((input.routingPrompt ?? input.prompt) || "");
    const route = IntentRouter.route(
      input.title,
      canonicalRoutingPrompt || input.title,
    );
    const attachmentDerivedStrategyIsStale =
      /\[\[ATTACHMENT_EXTRACTED_CONTENT_START\]\]/i.test(routingSource) &&
      Boolean(
        input.agentConfig?.taskDomain &&
          input.agentConfig.taskDomain !== "auto" &&
          input.agentConfig.taskDomain !== route.domain,
      );
    const strategyConfig = attachmentDerivedStrategyIsStale
      ? {
          ...input.agentConfig,
          taskDomain: "auto" as const,
          qualityPasses: undefined,
          preflightRequired: undefined,
          llmProfileHint: undefined,
          taskIntent: undefined,
          taskStrategySnapshot: undefined,
        }
      : input.agentConfig;
    const strategy = TaskStrategyService.derive(route, strategyConfig, {
      title: input.title,
      // Attachment extracts are task data, not routing instructions. Feeding
      // them into strategy derivation can promote an Excel request to code or
      // PPT merely because the source document mentions APIs/presentations.
  ...
  private resolveCronBudgetProfile(input: {
    ...
    const searchSignal =
      /\b(search|research|scan|look up|latest|breaking|news|trend|developments)\b/.test(
        text,
    ...
    const boundedWindowSignal =
      /\b(last|past|previous)\s+\d+\s*(hour|hours|day|days|week|weeks|month|months)\b/.test(
        text,
      ) || /\b24-hour\b|\b24h\b|\blast-24h\b/.test(text);

    if (
      (searchSignal && sourceMentions >= 2) ||
      (searchSignal && boundedWindowSignal)
    ) {
      return "aggressive";
    }
  ...
  private applyRuntimeTaskStrategy(task: Task): {
    ...
    const autoReviewPolicyEnabled = parseBooleanEnv(
      "NEOWORKER_REVIEW_POLICY_ENABLE_AUTO",
      false,
    );
    if (autoReviewPolicyEnabled && !nextAgentConfig.reviewPolicy) {
      if (
        derived.strategy.taskDomain === "code" ||
        derived.strategy.taskDomain === "operations"
      ) {
        const configured = (
          process.env.NEOWORKER_REVIEW_POLICY_AUTO_DEFAULT || "balanced"
        )
          .trim()
          .toLowerCase();
        nextAgentConfig = {
  ...
  async initialize(): Promise<void> {
    ...
    if (eagerMigrationCount > 0) {
      console.log(
        `[AgentDaemon] Migrated ${eagerMigrationCount} legacy event(s) to timeline v2`,
      );
    }
    ...
    if (orphanedTasks.length > 0) {
      ...
      for (const task of orphanedTasks) {
        ...
        // Plan definitions are rare and small. Do not limit this lookup to the
        // latest event: older plan_revised records did not persist a full plan
        // snapshot, so the most recent usable definition may be plan_created.
        const planEvents = this.eventRepo.findByTaskIdAndTypes(task.id, [
        ...
        if (hasSnapshot || hasPlan) {
          ...
          this.logEvent(task.id, "task_interrupted", {
            message:
              "Task interrupted by unexpected application exit. Will resume on restart.",
          });
  ...
  /**
   * Bind app-uploaded files to the task before it can be queued, interrupted,
   * or resumed in another temporary workspace. The task-scoped mirror is the
   * stable identity; the workspace-relative path remains the model-facing path.
   */
  private persistTaskAttachmentBindings(
    task: Task,
    workspace: Workspace,
    message: string,
    images?: ImageAttachment[],
  ): void {
    if (!(workspace.isTemp || isTempWorkspaceId(workspace.id))) return;

    const workspaceRoot = path.resolve(workspace.path);
    const attachmentPaths = new Set(extractWorkspaceUploadPaths(message));
    const requiredImagePaths = new Set<string>();

    for (const image of images ?? []) {
      const rawPath = typeof image.filePath === "string" ? image.filePath.trim() : "";
      if (!rawPath) continue;
      ...
      if (
        ...
        requiredImagePaths.add(relativePath);
      }
    }

    for (const attachmentPath of attachmentPaths) {
      let durablePath = persistTaskAttachmentSync({
      ...
      if (!durablePath) {
        ...
        if (restoredPath) {
          durablePath = persistTaskAttachmentSync({
            userDataPath: getUserDataDir(),
            taskId: task.id,
            workspacePath: workspace.path,
            artifactPath: attachmentPath,
      ...
      this.logEvent(task.id, "log", {
        metric: "task_attachment_binding_failed",
        attachmentPath,
      });
      if (requiredImagePaths.has(attachmentPath)) {
        throw new Error(
          `附件已失效或无法读取：${path.basename(attachmentPath)}。请重新上传该文件后再试。`,
        );
      }
    }
  }
  ...
  async startTaskImmediate(task: Task): Promise<void> {
    ...
    if (roleOverridesChanged) {
      try {
        this.taskRepo.update(effectiveTask.id, {
          agentConfig: effectiveTask.agentConfig,
        });
      } catch (error) {
        console.warn(
          "[AgentDaemon] Failed to persist agent role overrides:",
          error,
        );
      }
    }
    ...
    if (runtimeStrategy.agentConfigChanged) {
      try {
        this.taskRepo.update(effectiveTask.id, {
          agentConfig: executionTask.agentConfig,
        });
      } catch (error) {
        console.warn(
          "[AgentDaemon] Failed to persist runtime strategy agent config:",
          error,
        );
      }
    }
    if (runtimeStrategy.promptChanged || runtimeStrategy.agentConfigChanged) {
      this.logEvent(effectiveTask.id, "log", {
        ...
        directResponseMode:
          runtimeStrategy.strategy.snapshot.directResponseMode,
    ...
    if (wasQueued) {
      ...
      const retrySuffix = isRetry
        ? ` (retry ${count}/${this.maxTaskRetries})`
        : "";
    ...
    const requiresWorktree =
      executionTask.agentConfig?.requireWorktree === true;
    ...
    try {
      ...
      console.error(
        `[AgentDaemon] Task ${effectiveTask.id} failed to initialize:`,
        error,
      );
      this.failTask(
        effectiveTask.id,
    ...
    console.log(
      `[AgentDaemon] Task status updated to 'planning', starting execution...`,
    );
  ...
  private async resumeInterruptedTasks(tasks: Task[]): Promise<void> {
    for (const task of tasks) {
      try {
        ...
        this.failTask(
          task.id,
          `Failed to resume after interruption: ${error.message}`,
        );
  ...
  /**
   * Executors stay cached after a completed run so that follow-up messages can
   * reuse their conversation. A failed follow-up restores the persisted parent
   * task to its terminal status, but the cached executor can still be labelled
   * "active". Treating that cache label as authoritative during app shutdown
   * resurrects the old run on next launch and can append an earlier success
   * response below the newer failed follow-up.
   */
  private shouldInterruptCachedTaskOnShutdown(
    cached: { status: string },
    persistedTask?: Task,
  ): boolean {
    if (cached.status !== "active") return false;
    if (!persistedTask) return true;
    return !isTerminalTaskStatus(deriveCanonicalTaskStatus(persistedTask));
  }
  ...
  private async resumeInterruptedTask(task: Task): Promise<void> {
    ...
    if (!workspace) {
      throw new Error(
        `Workspace ${task.workspaceId} not found - cannot resume task`,
      );
    }

    const checkpoint = TranscriptStore.loadCheckpointSync(
      workspace.path,
      task.id,
    );
    ...
    if (hasPlan) {
      ...
      for (const event of events) {
        if (
          this.isLegacyEventType(event, "step_completed") &&
          event.payload?.step?.id
        ) {
          completedStepIds.add(event.payload.step.id);
        }
        if (
          this.isLegacyEventType(event, "step_failed") &&
          event.payload?.step?.id
        ) {
          failedStepIds.add(event.payload.step.id);
        }
        if (
          this.isLegacyEventType(event, "step_skipped") &&
          event.payload?.step?.id
        ) {
   ...
   */
  private isTurnLimitContinuationEligible(
    task: Task,
    events: TaskEvent[],
  ): boolean {
    ...
    const planEvent = events
      .filter(
        (event) =>
          ...
          ) && Array.isArray(event.payload?.plan?.steps),
      )
      .pop();
  ...
  private async startQueuedContinuation(task: Task): Promise<void> {
    console.log(
      `[AgentDaemon] Starting queued continuation for task ${task.id}: ${task.title}`,
    );
    const events = this.getTaskEventsForReplay(task.id);
    if (!this.isTurnLimitContinuationEligible(task, events)) {
      ...
      this.logEvent(task.id, "error", {
        error:
          "Queued continuation was rejected because latest task error is not turn-limit.",
      });
      return;
    }
    ...
    let executor: TaskExecutor;
    try {
      ({ effectiveTask, executor } = this.createContinuationExecutor(
        task,
        events,
      ));
    } catch (error: Any) {
  ...
  async continueTask(taskId: string): Promise<void> {
    ...
    if (this.activeTasks.has(taskId)) {
      console.log(
        `[AgentDaemon] Task ${taskId} is already active, ignoring continue request`,
      );
      return;
    }
    ...
    const { effectiveTask, executor } = this.createContinuationExecutor(
      task,
      events,
    );
  ...
  async forkTaskSession(params: {
    ...
    const branchLabel =
      typeof params.branchLabel === "string" &&
      params.branchLabel.trim().length > 0
    ...
    if (params.sideChat && initialMessage) {
      void this.sendMessage(forkedTask.id, initialMessage).catch((error) => {
        log.error(
          `[AgentDaemon] Failed to start sidechat ${forkedTask.id}:`,
          error,
        );
        this.logEvent(forkedTask.id, "log", {
  ...
  private buildForkHistory(
    ...
    const requestedEventId =
      typeof options.fromEventId === "string" &&
      options.fromEventId.trim().length > 0
      ...
      const selectedIndex = sourceEvents.findIndex((event) => {
        return (
          ...
          (typeof event.payload?.eventId === "string" &&
            event.payload.eventId === requestedEventId)
        );
      });
      if (selectedIndex < 0) {
    ...
    return {
      events: sourceEvents
        .slice(0, cutoff)
        .filter((event) => this.isForkReplayEvent(event)),
  ...
  private refreshSideChatParentSnapshot(task: Task): void {
    ...
    const allMissingEvents = this.getTaskEventsForReplay(parentTaskId).filter(
      (event) => {
  ...
  private buildSideChatParentStatusContext(
    task: Task,
    message: string,
  ): string | undefined {
    if (!this.isSideChatTask(task) || !this.isSideChatStatusQuestion(message))
      return undefined;
    const parentTaskId = task.branchFromTaskId;
    ...
    const activeStepIds = Array.from(
      this.activeStepIdsByTask.get(parentTaskId) || [],
    );
    const failedStepIds = Array.from(
      this.failedPlanStepsByTask.get(parentTaskId) || [],
    );
    ...
    return [
      ...
      parentTask.error
        ? `Parent error: ${this.truncateSideChatContextText(parentTask.error)}`
        : "",
      typeof parentTask.resultSummary === "string" &&
      parentTask.resultSummary.trim().length > 0
      ...
      activeStepIds.length > 0
        ? `Active step ids: ${activeStepIds.slice(0, 8).join(", ")}`
        : "",
      failedStepIds.length > 0
        ? `Failed step ids: ${failedStepIds.slice(0, 8).join(", ")}`
        : "",
  ...
  private buildSideChatTurnAgentConfigOverride(
    ...
    const sideChatTurnContext = this.buildSideChatParentStatusContext(
      task,
      message,
    );
  ...
  private attachInitialTaskProvenance(
    task: Task,
  ...
  private ensureTaskAccessPolicy(task: Task): TaskAccessPolicy {
    ...
    return this.taskAccessPolicyRepo.createInitial(task.id, {
      ...
      shellAccess:
        task.agentConfig?.shellAccess ?? workspace?.permissions.shell ?? false,
      updatedAt: Date.now(),
    });
  }
  ...
  private applyTaskAccessSnapshot(task: Task): Task {
    ...
    if (policy.effectiveFromTurn && policy.effectiveFromTurn > nextTurn)
      return task;
    ...
    const integrationMentions = task.agentConfig?.integrationMentions?.filter(
      (mention) =>
        allowedConnectorIds.has(mention.id) ||
        allowedConnectorIds.has(mention.providerKey),
    );
    return {
      ...task,
      agentConfig: {
  ...
  private inheritTaskAccessPolicy(
    ...
    if (mode === "fork") {
      ...
      if (activeSnapshot) {
        this.taskAccessPolicyRepo.createInitial(targetTask.id, {
          ...
          workspaceScopes: activeSnapshot.workspaceScopes.map((scope) => ({
            ...scope,
          })),
    ...
    const blockedTools = Array.from(
      new Set([
        ...(source.blockedTools || []),
        ...(targetTask.agentConfig?.toolRestrictions || []),
      ]),
    );
    const childConnectorIds = new Set(
      (targetTask.agentConfig?.integrationMentions || []).flatMap((mention) => [
        mention.id,
        mention.providerKey,
      ]),
    );
    const connectorIds =
      childConnectorIds.size > 0
    ...
    const workspaceScopes =
      workspace && inheritedScope && workspace.permissions.read
        ? [
            {
              ...inheritedScope,
              rootPath: workspace.path,
    ...
    const permissionMode =
      sourcePermissionMode && childPermissionMode
        ? permissionModeRank[childPermissionMode] <
          permissionModeRank[sourcePermissionMode]
          ? childPermissionMode
  ...
  findLatestOrchestrationGraphByRootTask(rootTaskId: string) {
    return this.getOrchestrationGraphRepository().findSnapshotByRootTaskId(
      rootTaskId,
    );
  }

  listOrchestrationGraphsByRootTask(rootTaskId: string) {
    return this.getOrchestrationGraphRepository().listSnapshotsByRootTaskId(
      rootTaskId,
    );
  }

  findOrchestrationGraphByTeamRunId(teamRunId: string) {
    return this.getOrchestrationGraphRepository().findSnapshotByTeamRunId(
      teamRunId,
    );
  }
  ...
  async waitForDelegatedNode(
    rootTaskId: string,
    handle: string,
    timeoutSeconds: number,
  ) {
    return this.orchestrationGraphEngine.waitForHandle(
      rootTaskId,
      handle,
      timeoutSeconds,
    );
  ...
  async createChildTask(params: {
    ...
    const mergedAllowUserInput = mergedAutonomousMode
      ? false
      : (params.agentConfig?.allowUserInput ??
        parent?.agentConfig?.allowUserInput);
    ...
    const mergedGatewayContext: AgentConfig["gatewayContext"] | undefined =
      (() => {
        const rank: Record<
          NonNullable<AgentConfig["gatewayContext"]>,
          number
        > = {
          private: 0,
          group: 1,
    ...
    const allowlistMerge = (() => {
      ...
      if (parentAllowsAll && childAllowsAll) {
        ...
        mergedAllowedTools = Array.from(childAllowed).filter(
          (tool) => tool !== "*",
        );
      } else if (childAllowsAll) {
        mergedAllowedTools = Array.from(parentAllowed).filter(
          (tool) => tool !== "*",
        );
      } else {
        mergedAllowedTools = Array.from(childAllowed).filter((tool) =>
          parentAllowed.has(tool),
        );
      }
    ...
    const workerRole =
      resolveWorkerRoleKind(params.workerRole) ||
      resolveDefaultWorkerRoleKind();
    mergedAgentConfig = resolveWorkerRoleAgentConfig(
      workerRole,
      mergedAgentConfig,
    );
    ...
    const initialUpdates: Partial<Task> = {
      ...
      resumeStrategy: memoryFeatures.transcriptStoreEnabled
        ? "checkpoint"
        : "snapshot",
    };
    if (
    ...
    if (
      typeof params.boardColumn === "string" &&
      params.boardColumn.trim().length > 0
    ) {
    ...
    windows.forEach((window) => {
      try {
        if (
          ...
          window.webContents.send(IPC_CHANNELS.ACTIVITY_EVENT, {
            type: "created",
            activity,
          });
        }
    ...
    windows.forEach((window) => {
      try {
        if (
          ...
          window.webContents.send(IPC_CHANNELS.MENTION_EVENT, {
            type: "created",
            mention,
          });
        }
  ...
  ensureCollaborativeRunForParentTask(
    ...
    if (childTasks.length < 2) {
      const db = this.dbManager.getDatabase();
      return (
        new AgentTeamRunRepository(db).findByRootTaskId(parentTaskId) || null
      );
    }
    ...
    if (existingRun) {
      if (parentTask.agentConfig?.childAgentCollaborativeRun === true) {
        this.ensureChildTasksHaveTeamItems(
          existingRun,
          childTasks,
          teamMemberRepo,
          teamItemRepo,
        ...
        if (
          this.childTasksHaveActiveWork(childTasks) &&
    ...
    const assignedLeadRoleId =
      childTasks.find(
        (task) =>
          ...
          activeRoleIds.has(task.assignedAgentRoleId),
    ...
    const run = teamRunRepo.create({
      ...
      status: this.childTasksHaveActiveWork(childTasks)
        ? "running"
        : "completed",
      collaborativeMode: true,
    });
    ...
    if (!this.childTasksHaveActiveWork(childTasks)) {
      const completedRun = teamRunRepo.update(run.id, {
        status: childTasks.some((task) => task.status === "failed")
          ? "failed"
          : "completed",
        phase: "complete",
  ...
  private ensureChildTasksHaveTeamItems(
    ...
    const existingSourceTaskIds = new Set(
      existingItems
        ...
        .filter((sourceTaskId): sourceTaskId is string =>
  ...
  private mapChildTaskStatusToTeamItemStatus(
    status: TaskStatus,
  ): AgentTeamItemStatus {
  ...
  private async maybeLaunchCollaborativeTask(task: Task): Promise<boolean> {
    ...
    if (existingRun) {
      if (!teamRepo.findById(existingRun.teamId)) {
        throw new Error(
          `Collaborative team not found for run ${existingRun.id}`,
        );
      }
      const updatedRun =
        existingRun.status !== "running"
      ...
      void this.teamOrchestrator.tickRun(
        existingRun.id,
        "daemon_existing_collab_run",
      );
      return true;
    }
    ...
    if (task.agentConfig.multiLlmMode && task.agentConfig.multiLlmConfig) {
      ...
      const allRoles = this.agentRoleRepo
        .findAll(false)
        .filter((role) => role.isActive);
      ...
      const maxParallelAgents =
        typeof config.maxParallelParticipants === "number" &&
        config.maxParallelParticipants > 0
          ? Math.min(
              participants.length,
              Math.floor(config.maxParallelParticipants),
            )
          : participants.length;
      const team = teamRepo.create({
  ...
  async dispatchMentionedAgents(taskId: string, plan?: Plan): Promise<void> {
    ...
    const activeRoles = this.agentRoleRepo
      .findAll(false)
      .filter((role) => role.isActive);
    const mentionedRoles = activeRoles.filter((role) =>
      mentionedRoleIds.includes(role.id),
    );
    ...
    const rolesToDispatch = mentionedRoles.filter(
   ...
   */
  private restoreTerminalParentAfterFollowUpCancellation(
    taskId: string,
    message: string,
  ): boolean {
    ...
    let resumeIndex = -1;
    let resumePayload: Record<string, Any> | null = null;
    for (let index = events.length - 1; index >= 0; index -= 1) {
      ...
      const effectiveType =
        typeof payload.legacyType === "string"
          ? payload.legacyType
          : event.type;
      if (effectiveType !== "task_resumed" || payload.newRunStarted !== true) {
        continue;
      }
      resumeIndex = index;
    ...
    const previousStatus = resumePayload?.previousStatus as
      | Task["status"]
      | undefined;
    if (
      resumeIndex < 0 ||
    ...
    // Only restore the parent terminal state for the still-active follow-up
    // that began at this resume event. A completed later run must never be
    // rolled back by a stale cancellation request.
    const hasLaterTerminalEvent = events
      .slice(resumeIndex + 1)
      .some((event) => {
        const payload =
          event.payload && typeof event.payload === "object"
        ...
        return (
          ...
          (effectiveType === "task_status" &&
            isTerminalTaskStatus(payload.status as Task["status"]))
        );
      });
    if (hasLaterTerminalEvent) return false;

    const precedingTerminalEvent = events
      .slice(0, resumeIndex)
      .reverse()
      .find((event) => {
        const payload =
        ...
        return (
          (previousStatus === "completed" &&
            effectiveType === "task_completed") ||
          (previousStatus === "failed" && effectiveType === "error") ||
          (previousStatus === "cancelled" &&
            effectiveType === "task_cancelled")
        );
      });
    const previousCompletedAt = Number(resumePayload?.previousCompletedAt);
    const completedAt =
      Number.isFinite(previousCompletedAt) && previousCompletedAt > 0
        ? previousCompletedAt
        : precedingTerminalEvent?.timestamp || Date.now();
    const previousLastRunDurationMs = Number(
      resumePayload?.previousLastRunDurationMs,
    );
    const terminalStatus = resumePayload?.previousTerminalStatus as
      | Task["terminalStatus"]
      | undefined;
    const failureClass = resumePayload?.previousFailureClass as
      | Task["failureClass"]
      | undefined;
    const previousError =
      typeof resumePayload?.previousError === "string"
        ? resumePayload.previousError
        : null;

    this.cleanupPendingApprovalsForTask(
      taskId,
      "Follow-up ended before the approval request was resolved.",
    );
    this.taskRepo.update(taskId, {
      status: previousStatus,
      completedAt,
      ...(Number.isFinite(previousLastRunDurationMs) &&
      previousLastRunDurationMs >= 0
        ? { lastRunDurationMs: previousLastRunDurationMs }
        : {}),
      error: previousStatus === "failed" ? previousError : null,
      terminalStatus:
        terminalStatus ||
        (previousStatus === "completed"
          ? "ok"
          : previousStatus === "failed"
            ...
            : undefined),
      failureClass:
        previousStatus === "failed" ? failureClass : undefined,
    ...
    this.logEvent(taskId, "log", {
      message:
        "Follow-up was stopped; restored the task state from before this follow-up.",
  ...
  async cancelTask(taskId: string): Promise<void> {
    ...
    // A terminal task can be reopened for a follow-up. Stopping only that
    // follow-up must preserve the already delivered parent result instead of
    // painting the entire task red/cancelled in the sidebar.
    const restoredParentTerminalState =
      this.restoreTerminalParentAfterFollowUpCancellation(
        taskId,
        "Follow-up was stopped by user; previous task result was preserved.",
      );
    if (!restoredParentTerminalState) {
      // Persist cancellation for ordinary running tasks too (important for
      // remote clients querying task status).
  ...
  async wrapUpTask(taskId: string): Promise<void> {
    ...
    if (
      ...
      existing.terminalStatus === "needs_user_action"
    ) {
    ...
    if (cached) {
      ...
      this.cancelTaskRecord(
        taskId,
        "Task removed from queue during wrap-up request",
      );
      this.finishQueueSlot(taskId);
    }
  }

  private async acceptTaskCurrentProgress(
    taskId: string,
    message: string,
  ): Promise<void> {
    const existing = this.taskRepo.findById(taskId);
    ...
    for (const request of pendingRequests) {
      ...
      if (pending && !pending.resolved) {
        ...
        pending.reject(
          new Error("Task stopped by user; current progress accepted."),
        );
      }
  ...
  async resumeTask(taskId: string): Promise<boolean> {
    ...
    if (cached) {
      ...
      const currentStatus = currentTask
        ? deriveCanonicalTaskStatus(currentTask)
        : undefined;
  ...
  setSessionAutoApproveAll(enabled: boolean): void {
    ...
    console.log(
      `[AgentDaemon] Session auto-approve ${enabled ? "ENABLED" : "DISABLED"}`,
    );
  }
  ...
  listRecentSensitiveSources(taskId: string): SensitiveSourceRef[] {
    return (
      this.getExecutorForTask(taskId)?.runtime?.listRecentSensitiveSources() ||
      []
    );
  }
  ...
  private canAutoReviewApprove(
      ...
      const permissionInput =
        details.permissionInput && typeof details.permissionInput === "object"
          ...
          : details.params && typeof details.params === "object"
            ...
            : details;
      const url =
        typeof permissionInput.url === "string" ? permissionInput.url : "";
      if (!url) return { approved: false };
      const toolName =
        typeof details.tool === "string" ? details.tool : "network_access";
    ...
    if (type === "data_export") {
      const task = this.taskRepo.findById(taskId);
      const toolName =
        typeof details.tool === "string" ? details.tool.trim() : "";
      const prompt = details.permissionPrompt as
        PermissionPromptDetails | undefined;
      ...
      const isTrustedWorkspaceSource = (source: SensitiveSourceRef): boolean =>
        ...
        source.sourceKind === "workspace_native";
  ...
  private buildPermissionMode(taskId: string, task?: Task): PermissionMode {
    ...
    return enforceAllowedMode(
      PermissionSettingsManager.loadSettings().defaultMode || "default",
    );
  }
  ...
  private buildPermissionRules(
    ...
    const trustedCommandRules = guardrailSettings.autoApproveTrustedCommands
      ? [
          ...DEFAULT_TRUSTED_COMMAND_PATTERNS,
          ...guardrailSettings.trustedCommandPatterns,
    ...
    const builtinRules: PermissionRule[] =
      BuiltinToolsSettingsManager.getToolAutoApprove("run_command")
        ? [
            {
  ...
  private inferToolNameFromApprovalType(approvalType: string): string {
    switch (approvalType) {
      ...
      case "data_export":
        return "http_request";
      default:
        return approvalType === "external_service"
          ? "external_service"
          : approvalType;
    }
  }
  ...
  private inferPermissionToolName(
  ...
  private evaluatePermissionRequest(
    ...
    if (
      runtime &&
      bundleGrantKey &&
      runtime.hasActiveTemporaryPermissionGrant(bundleGrantKey)
    ) {
  ...
  private persistApprovalActionRule(
    ...
    const effect: PermissionEffect = action.startsWith("deny_")
      ? "deny"
      : "allow";
    ...
    if (workspace) {
      ...
      const manifestResult = appendWorkspacePermissionManifestRule(
        workspace.path,
        {
          ...rule,
  ...
  private applyPendingRecoveredApprovalGrant(
    taskId: string,
    executor: TaskExecutor,
  ): void {
  ...
  private rememberRecoveredApprovalGrant(
    ...
    if (existing.some((entry) => entry.approval.id === grant.approval.id))
      return;
  ...
  async requestApproval(
    ...
    const recoveredOnceGrantKey = buildRecoveredApprovalOnceGrantKey(
      permission.scope,
    );
    if (
      permission.evaluation.decision === "ask" &&
      permission.runtime?.hasActiveTemporaryPermissionGrant(
        recoveredOnceGrantKey,
      )
    ) {
    ...
    if (permission.evaluation.decision === "allow") {
      ...
      if (
        type === "run_command" &&
        enrichedDetails.approvalMode === "single_bundle"
      ) {
        permission.runtime?.addTemporaryPermissionGrant(
          "run_command:single_bundle",
        );
      }
      ...
      const timeoutHandle = setTimeout(
        () => {
          ...
          if (pending && !pending.resolved) {
            ...
            if (isTerminalTaskStatus(currentStatus)) {
              reject(
                new Error("Approval request timed out after task completion"),
              );
              return;
            }
            this.updateTask(taskId, {
  ...
  async respondToApproval(
    ...
    if (existing.exists) {
      if (existing.status === "pending") {
        console.log(
          `[AgentDaemon] Concurrent approval response in progress: ${approvalId}`,
        );
        return "in_progress";
      }
      if (existing.status === "completed") {
        const cachedStatus = existing.result?.status as
          ApprovalResponseStatus | undefined;
        if (
        ...
        console.log(
          `[AgentDaemon] Duplicate approval response ignored: ${approvalId}`,
        );
        return "duplicate";
      }
    ...
    try {
      ...
      if (pending && !pending.resolved) {
        ...
        const runtime =
          this.getExecutorForTask(pending.taskId)?.runtime || null;
        const prompt =
          pending.approval?.details &&
          typeof pending.approval.details === "object"
  ...
  private cleanupPendingApprovalsForTask(
    taskId: string,
    rejectionMessage: string,
  ): number {
    let cleared = 0;

    for (const [approvalId, pending] of Array.from(
  ...
  async respondToInputRequest(
    ...
    try {
      ...
      if (!request) {
        inputRequestIdempotency.complete(idempotencyKey, {
          status: "not_found",
        });
      ...
      if (response.status === "submitted") {
        this.logEvent(request.taskId, "assistant_message", {
          message: buildStructuredInputSelectionMessage(
            request,
            response.answers,
          ),
  ...
  logEvent(taskId: string, type: string, payload: Any): void {
    ...
    const payloadObj: Record<string, unknown> =
      payload && typeof payload === "object" && !Array.isArray(payload)
        ? ({ ...(payload as Record<string, unknown>) } as Record<
            string,
            unknown
          >)
        : payload === undefined
    ...
    const requestedSeq =
      typeof requestedSeqRaw === "number" &&
      Number.isFinite(requestedSeqRaw) &&
      requestedSeqRaw > 0
    ...
    const taskStatusAtEvent = stageSourceType
      ? this.taskRepo.findById(taskId)?.status
      : undefined;
    ...
    if (typeof persistTranscriptArtifacts === "function") {
      void persistTranscriptArtifacts.call(
        this,
        taskId,
        timelineEvent,
        legacyType,
  ...
  private maybeEmitAssistantMediaPreview(
    ...
    if (
      isPreviewableHtml &&
      !this.shouldEmitInlineHtmlFramePreview(task, payload, rawPath)
    ) {
      return;
    }
    ...
    const title =
      typeof payload.label === "string" && payload.label.trim().length > 0
        ...
        : path.basename(rawPath) ||
          (isPreviewableVideo ? "Generated video" : "Generated frame");
  ...
  private shouldEmitInlineHtmlFramePreview(
    ...
    const displayMode =
      typeof payload.display === "string"
        ? payload.display.trim().toLowerCase()
        : "";
    if (
    ...
    const fullPageIntent =
      ...
      /(^|[-_/])(index|landing|homepage|website|webpage|site|app|page)\.html?$/.test(
        filename,
      );
  ...
  private getProviderTypeFromPayload(
  ...
  private getActiveExecutorProviderType(taskId: string): string | null {
    const activeExecutorProvider = (
      this.activeTasks.get(taskId)?.executor as Any
    )?.provider?.type;
    return this.normalizeProviderTypeValue(activeExecutorProvider);
  }

  private rememberTaskLlmProviderType(
    taskId: string,
  ...
  private async persistTranscriptArtifacts(
    ...
    if (features.transcriptStoreEnabled) {
      ...
      await TranscriptStore.appendEvent(workspace.path, legacyEvent).catch(
        () => undefined,
      );
    }
  ...
  private isMeaningfulExchangeEvent(
    type: string | undefined,
    payload: unknown,
  ): boolean {
  ...
  private buildStructuredCheckpointSummary(
    ...
    const snapshotSummary = snapshotPayload
      ? this.getSnapshotSummaryBlock(snapshotPayload)
      : "";
    if (snapshotSummary) {
      return this.parseStructuredCheckpointSummary(
        snapshotSummary,
        "compaction_summary",
      );
    }
  ...
  private buildCheckpointEvidencePacket(
    ...
    const spans = messageEvents
      .map((event) => {
        const excerpt = this.extractCheckpointEventText(event.payload).slice(
          0,
          500,
        );
    ...
    const hashInput = spans
      .map(
        (span) =>
          `${span.objectId}:${span.timestamp}:${span.type}:${span.excerpt}`,
      )
      .join("|");
  ...
  private async maybeCaptureRuntimeCheckpoint(params: {
    ...
    const effectiveType =
      typeof params.legacyType === "string" &&
      params.legacyType.trim().length > 0
        ? params.legacyType
        : params.event.type;
    ...
    const meaningfulEvents = this.eventRepo
      ...
      .filter((event) =>
        this.isMeaningfulExchangeEvent(
          this.resolveLegacyEventType(event),
          event.payload,
        ),
      )
      .slice(-12);
    ...
    const checkpointSourceEventId =
      latestCheckpoint?.sourceEventId?.trim() || "";
    const checkpointTimestamp =
      Number(
        latestCheckpoint?.sourceTimestamp ?? latestCheckpoint?.timestamp ?? 0,
      ) || 0;
    const checkpointCursor = checkpointSourceEventId
      ? this.eventRepo.findEventCursorById(
          params.task.id,
          checkpointSourceEventId,
        )
      : latestCheckpoint
  ...
  private addKnownPlanStepId(taskId: string, rawStepId: string): void {
    ...
    const knownStepIds =
      this.knownPlanStepIdsByTask.get(taskId) || new Set<string>();
  ...
  private isKnownPlanStepId(taskId: string, rawStepId: string): boolean {
    ...
    for (const candidate of knownStepIds) {
      if (this.normalizeStepIdForPlanTracking(candidate) === normalizedStepId)
        return true;
    }
    return false;
  }
  ...
  private trackTimelineStepState(taskId: string, event: TaskEvent): void {
    ...
    if (
      effectiveLegacyType === "plan_created" ||
      effectiveLegacyType === "plan_revised"
    ) {
    ...
    const activeSteps =
      this.activeStepIdsByTask.get(taskId) || new Set<string>();
    const failedPlanSteps =
      this.failedPlanStepsByTask.get(taskId) || new Set<string>();
    ...
    if (event.type === "timeline_step_started") {
      ...
      const isPlanFailureError =
        (effectiveLegacyType === "step_failed" ||
          effectiveLegacyType === "step_timeout") &&
  ...
  private trackEvidenceRefs(taskId: string, event: TaskEvent): void {
    ...
    const existing =
      this.evidenceRefsByTask.get(taskId) || new Map<string, EvidenceRef>();
  ...
  private maybeMaterializeMailComposeInlineFrame(
    ...
    const payload =
      ...
      !Array.isArray(event.payload)
        ? (event.payload as Record<string, unknown>)
        : {};
    if (Array.isArray(payload.inlineFrames) && payload.inlineFrames.length > 0)
      return;
    if (this.materializedMailComposeFrameTasks.has(event.taskId)) return;
    if (effectiveType === "assistant_message" && payload.internal === true)
      return;
    ...
    const sourceUserMessage = [
      typeof (sourceTask as Any)?.rawPrompt === "string"
        ? (sourceTask as Any).rawPrompt
        : "",
      typeof (sourceTask as Any)?.userPrompt === "string"
        ? (sourceTask as Any).userPrompt
        : "",
      ...
      .filter(Boolean)
      .join("\n");
    const draftInput = extractMailboxComposeDraftInputFromText(
      message,
      sourceUserMessage,
    );
    ...
    if (!mailboxService) {
      log.warn(
        "[mail-compose-frame] Mailbox service unavailable while materializing inline frame",
      );
    ...
    void (async () => {
      try {
        ...
        const account = clientState.accounts.find(
        ...
        this.materializedMailComposeFrameTasks.delete(event.taskId);
        log.warn(
          "[mail-compose-frame] Failed to materialize compose draft:",
          error,
        );
      }
    })();
  ...
  private persistTimelineEvent(
    ...
    this.maybeMaterializeMailComposeInlineFrame(
      storedEvent,
      effectiveType,
      task,
    );
    ...
    this.captureToMemory(event.taskId, memoryType, storedLegacyPayload).catch(
      (error) => {
  ...
  private maybeEmitTeamThought(
    ...
    if (!run && task.parentTaskId) {
      ...
      if (run) {
        ...
        teamItem =
          runItems.find((i) => i.sourceTaskId === taskId) || runItems[0];
      }
    }
    ...
    if (phase !== "think" && phase !== "dispatch" && phase !== "synthesize")
      return;
    ...
    try {
      ...
      console.error(
        "[AgentDaemon] Team thought capture failed:",
        error?.message,
      );
    }
  }
  ...
  private async handleOrchestrationNodeNotification(
    ...
    if (this.teamOrchestrator && notification.status !== "running") {
      await this.teamOrchestrator.tickRun(
        node.teamRunId,
  ...
  private async captureToMemory(
    ...
    if (
      ...
      skipMemoryToolNames.has(toolName)
    ) {
      return;
    }
    ...
    const isSubAgentTask =
      (task.agentType ?? "main") === "sub" || !!task.parentTaskId;
    ...
    const isSharedGatewayContext =
      gatewayContext === "group" || gatewayContext === "public";
    const allowProfileIngest =
      !isSharedGatewayContext ||
      task.agentConfig?.allowSharedContextMemory === true;
    if (allowProfileIngest) {
      if (type === "user_message") {
        ...
        if (text) {
          getAwarenessService().captureConversation(
            text,
            task.workspaceId,
            taskId,
          );
        ...
        getAwarenessService().captureFeedback(
          feedbackReason,
          task.workspaceId,
          taskId,
        );
  ...
  private buildActivityFromEvent(
    ...
    const actorType: ActivityActorType = task.assignedAgentRoleId
      ? "agent"
      : "system";
  ...
  relocateArtifact(taskId: string, oldPath: string, newPath: string): void {
    try {
      const relocated = this.artifactRepo.relocateForTask(
        taskId,
        oldPath,
        newPath,
      );
      if (relocated > 0) {
        console.log(
          `[AgentDaemon] Relocated artifact: ${oldPath} -> ${newPath}`,
        );
      }
    } catch (error) {
  ...
  private emitTaskEvent(event: TaskEvent): void {
    ...
    const payloadObj =
      ...
      !Array.isArray(event.payload)
        ...
        : null;
    const payloadLegacyType =
      payloadObj && typeof payloadObj.legacyType === "string"
        ? payloadObj.legacyType
        : "";
    const effectiveType =
      typeof event.legacyType === "string" && event.legacyType.trim().length > 0
        ? event.legacyType.trim()
        : typeof payloadLegacyType === "string" &&
            payloadLegacyType.trim().length > 0
    ...
    try {
      ...
      console.error(
        `[AgentDaemon] Error emitting timeline event ${event.type}:`,
        error,
      );
    }
    ...
    const shouldEmitLegacyAlias =
      !!legacyAlias &&
      !(legacyAlias.type === "error" && this.listenerCount("error") === 0);
      ...
      windows.forEach((window) => {
        ...
        try {
          if (
            ...
            if (effectiveType === "llm_routing_changed") {
              window.webContents.send(
                IPC_CHANNELS.LLM_ROUTING_EVENT,
                event.payload,
              );
            }
          }
  ...
  private resolveLegacyTaskEventAlias(event: TaskEvent): {
    ...
    const payloadObj =
      ...
      typeof event.payload === "object" &&
      !Array.isArray(event.payload)
        ? ({ ...(event.payload as Record<string, unknown>) } as Record<
            string,
            unknown
          >)
        : {};
    const legacyType =
      typeof event.legacyType === "string" && event.legacyType.trim().length > 0
        ? event.legacyType.trim()
        : typeof payloadObj.legacyType === "string" &&
            payloadObj.legacyType.trim().length > 0
          ? payloadObj.legacyType.trim()
          : "";
    ...
    if (
      ...
      legacyType.startsWith("timeline_")
    ) {
      return null;
    }
  ...
  beginFollowUpRun(taskId: string): Task | undefined {
    ...
    this.taskRepo.update(taskId, {
      ...
      failureClass: undefined,
      awaitingUserInputReasonCode: undefined,
      // A follow-up on a terminal task is a fresh run. Keeping the previous
      // run's lifetime counters can make the very first model call fail after
      // the user retries a formerly runaway task.
      continuationCount: 0,
    ...
    this.logEvent(taskId, "task_resumed", {
      ...
      previousLastRunDurationMs: existing.lastRunDurationMs,
      previousError: existing.error ?? null,
      previousTerminalStatus: existing.terminalStatus,
      previousFailureClass: existing.failureClass,
    });
  ...
  private calculateLatestRunDurationMs(
    ...
    const end = Number.isFinite(completedAt)
    ...
    for (const event of events) {
      const ts =
        typeof event.timestamp === "number" && Number.isFinite(event.timestamp)
          ...
          : undefined;
      if (ts === undefined || ts > end) continue;
      if (previousTerminalAt !== undefined && ts <= previousTerminalAt)
        continue;
  ...
  private getTaskEventsForResume(
    ...
    const snapshot = checkpoint
      ? null
      : this.eventRepo.findLatestConversationSnapshot(taskId);
    // Keep every (small) plan-definition event so legacy plan revisions that
    // omitted the aggregate snapshot cannot hide the last usable plan.
    const planDefinitionEvents = this.eventRepo.findByTaskIdAndTypes(taskId, [
      ...RESUME_PLAN_DEFINITION_EVENT_TYPES,
    ]);
    const latestPlanDefinition = planDefinitionEvents
      .filter((event) => Array.isArray(event.payload?.plan?.steps))
      .at(-1);
    const planStateEvents = latestPlanDefinition
      ? this.eventRepo.findLatestPlanStepStateEvents(
          taskId,
          latestPlanDefinition,
        )
      : [];
    ...
    if (checkpoint) {
      const sourceEventId =
        typeof checkpoint.sourceEventId === "string"
          ? checkpoint.sourceEventId.trim()
          : "";
    ...
    const uniqueEvents = Array.from(
      new Map(
        events.map((event) => [event.eventId || event.id, event]),
      ).values(),
    ).sort((a, b) => {
      const aOrder = Number(a.seq ?? a.ts ?? a.timestamp) || 0;
      const bOrder = Number(b.seq ?? b.ts ?? b.timestamp) || 0;
      return (
        aOrder - bOrder || a.timestamp - b.timestamp || a.id.localeCompare(b.id)
      );
    });
  ...
  getTaskEvents(
    ...
    if (normalizedTypes.length > 0) {
      return this.eventRepo.findByTaskIdAndTypes(
        taskId,
        normalizedTypes,
        limit,
      );
    ...
    try {
      ...
      switch (period) {
        ...
        default: {
          return {
            success: false,
            error: `Unsupported period: ${String(period)}`,
          };
        }
      }
      ...
      const messageEvents =
        includeMessages && taskIds.length > 0
          ? this.eventRepo.findByTaskIds(taskIds, [
              "assistant_message",
              "user_message",
            ])
          : [];
      ...
      for (const evt of messageEvents) {
        const msg =
          (evt.payload as Any)?.message ?? (evt.payload as Any)?.content;
        const text = typeof msg === "string" ? msg : "";
        if (!text) continue;
        if (evt.type === "assistant_message")
          lastAssistant.set(evt.taskId, truncate(text, 900));
        if (evt.type === "user_message")
          lastUser.set(evt.taskId, truncate(text, 900));
      }
      ...
      const items = tasks.map((t) => ({
        ...
        lastAssistantMessage: includeMessages
          ? lastAssistant.get(t.id)
          : undefined,
      }));
    ...
    try {
      ...
      const summarizeEvent = (type: string, payload: Any): string => {
        switch (type) {
          ...
          case "step_failed": {
            const desc =
              typeof payload?.step?.description === "string"
                ? payload.step.description
                : "";
            const err = typeof payload?.error === "string" ? payload.error : "";
            const base = desc
      ...
      const events = rows.map((row) => {
        ...
        if (effectiveType === "tool_call") {
          toolCalls += 1;
          const tool = (payloadObj?.tool || payloadObj?.name || "")
            .toString()
            .trim();
          if (tool) {
        ...
        if (effectiveType === "user_feedback") {
          const decision =
            typeof payloadObj?.decision === "string"
              ? payloadObj.decision.trim()
              : "unknown";
        ...
        return {
          ...
          ...(includePayload
            ? { payloadPreview: compactPayloadPreview(payloadObj) }
            : {}),
        };
      });
  ...
  refreshActiveExecutorsForWorkspace(workspaceId: string): void {
    ...
    this.activeTasks.forEach((cached, taskId) => {
      ...
      try {
        ...
        console.warn(
          `[AgentDaemon] Failed to refresh executor for task ${taskId}:`,
          err,
        );
      }
    });
  ...
  private runQuickQualityPass(params: {
    ...
    if (params.explicitEvidenceRequired && params.verificationEvidenceBundle) {
      const hasSuccessfulVerificationEvidence =
        params.verificationEvidenceBundle.entries.some((entry) => entry.ok);
  ...
  private isTaskCompletedTimelineEvent(event: TaskEvent): boolean {
    ...
    if (
      typeof event.legacyType === "string" &&
      event.legacyType === "task_completed"
    ) {
      return true;
    }
    const payloadObj =
  ...
  private computeTimelineTelemetryFromEvents(events: TaskEvent[]): {
    ...
    return {
      timeline_event_drop_rate:
        totalEvents > 0 ? droppedEvents / totalEvents : 0,
      timeline_order_violation_rate:
        totalEvents > 0 ? orderViolations / totalEvents : 0,
      step_state_mismatch_rate:
        totalEvents > 0 ? stepStateMismatches / totalEvents : 0,
  ...
  private backfillTaskCompletionTelemetry(taskId: string): void {
    ...
    const completionEvents = events.filter((event) =>
      this.isTaskCompletedTimelineEvent(event),
    );
    ...
    for (const completionEvent of completionEvents) {
      const payloadObj =
        ...
        !Array.isArray(completionEvent.payload)
          ...
          : {};
      const boundarySeq =
        typeof completionEvent.seq === "number" &&
        Number.isFinite(completionEvent.seq)
          ? completionEvent.seq
          : undefined;
      const boundaryTs =
        typeof completionEvent.ts === "number" &&
        Number.isFinite(completionEvent.ts)
          ? completionEvent.ts
      ...
      const shouldUpdate =
        !existingTelemetry ||
        Number(existingTelemetry.timeline_event_drop_rate) !==
          telemetry.timeline_event_drop_rate ||
        Number(existingTelemetry.timeline_order_violation_rate) !==
          telemetry.timeline_order_violation_rate ||
        Number(existingTelemetry.step_state_mismatch_rate) !==
          telemetry.step_state_mismatch_rate ||
        Number(existingTelemetry.completion_gate_block_count) !==
          telemetry.completion_gate_block_count ||
        Number(existingTelemetry.evidence_gate_fail_count) !==
          telemetry.evidence_gate_fail_count;
  ...
  private extractKeyClaimSentences(summary: string): string[] {
    ...
    return pieces.filter((piece) => {
      ...
      return (
        measurableSignalRe.test(normalized) &&
        declarativeSignalRe.test(normalized)
      );
    });
  }
  ...
  private hasEvidenceForKeyClaims(
    ...
    if (evidenceRefs && evidenceRefs.size > 0)
      return { passed: true, keyClaims };

    const hasSuccessfulVerificationEvidence =
      verificationEvidenceBundle?.entries?.some((entry) =>
        Boolean(entry?.ok),
      ) ?? false;
    if (hasSuccessfulVerificationEvidence) return { passed: true, keyClaims };

    const tokenEvidenceRe =
      /\[(?:evidence|source|cite):[^\]]+\]|\[[0-9]+\]|https?:\/\//i;
  ...
  private async runPostCompletionVerification(
    ...
    const eventType =
      result.verdict === "PASS" ? "verification_passed" : "verification_failed";
    ...
    if (
      result.verdict === "FAIL" ||
      (result.verdict === "PARTIAL" && result.shouldBlock)
    ) {
  ...
  private async runPostTaskEntropySweep(
    ...
    const blastRadius = collectBlastRadiusPaths(
    ...
    if (result.status === "completed") {
      ...
      const clean =
        /ENTROPY:\s*CLEAN/i.test(text) || /\bNO_ISSUES_FOUND\b/i.test(text);
  ...
  completeTask(
    ...
    const trimmedSummary =
      typeof resultSummary === "string" && resultSummary.trim().length > 0
        ? resultSummary.trim()
        : undefined;
    const bestKnownOutcome =
      metadata?.bestKnownOutcome || getTaskBestKnownOutcome(existingTask);
    const canonicalTaskIntent = buildCanonicalTaskIntentQuery({
    ...
    for (const event of historicalEvents) {
      const payload =
        ...
        !Array.isArray(event.payload)
          ? (event.payload as Record<string, unknown>)
          : null;
      if (!payload || typeof payload.copiedFrom !== "string") continue;
      const emittedPath =
        typeof payload.path === "string" ? payload.path.trim() : "";
      if (emittedPath) copiedInputArtifactPaths.add(path.normalize(emittedPath));
    }
    const deliveredArtifactPaths = new Set<string>();
    const collectOutputSummaryPaths = (
      ...
      for (const candidate of [
        ...
        if (typeof candidate === "string" && candidate.trim()) {
          const normalizedCandidate = path.normalize(candidate.trim());
          if (!copiedInputArtifactPaths.has(normalizedCandidate)) {
    ...
    for (const artifact of this.artifactRepo?.findByTaskId?.(taskId) || []) {
      if (typeof artifact.path === "string" && artifact.path.trim()) {
        const normalizedArtifactPath = path.normalize(artifact.path.trim());
        if (!copiedInputArtifactPaths.has(normalizedArtifactPath)) {
    ...
    const waivedNormalizedStepIds = new Set(
      Array.from(waivedFailedStepIds.values()).map((id) =>
    ...
    const nonBlockingFailedStepIds = new Set(
      (this.verificationOutcomeV2Enabled
        ? metadata?.nonBlockingFailedStepIds || []
        : []
      )
        .map((id) => String(id || "").trim())
        .filter((id) => id.length > 0),
    ...
    const isVerificationFailureStep = (rawStepId: string): boolean => {
      ...
      for (let i = historicalEvents.length - 1; i >= 0; i -= 1) {
        ...
        const stepObj =
          payloadObj.step &&
          typeof payloadObj.step === "object" &&
          !Array.isArray(payloadObj.step)
        ...
        const normalizedEventStepId =
          normalizeStepIdForComparison(eventStepIdRaw);
        if (
          normalizedEventStepId !== normalizedStepId &&
          eventStepIdRaw !== stepId
        )
          continue;
    ...
    const isBudgetConstrainedFailureStep = (rawStepId: string): boolean => {
      ...
      const isMatchingStep = (
        ...
        const stepObjIdRaw =
          typeof candidateStepObj.id === "string" ? candidateStepObj.id : "";
        const payloadStepIdRaw =
          typeof payloadObj.stepId === "string" ? payloadObj.stepId : "";
        const options = [eventStepIdRaw, stepObjIdRaw, payloadStepIdRaw].filter(
          Boolean,
        );
      ...
      const reasonText = [
        typeof latestFailurePayload.reason === "string"
          ...
          : "",
        typeof latestFailurePayload.error === "string"
          ? latestFailurePayload.error
          : "",
        typeof latestFailurePayload.message === "string"
          ? latestFailurePayload.message
          : "",
    ...
    const mutationContractBlockers = Array.from(
      failedMutationRequiredStepIds.values(),
    );
    ...
    const allowEvidenceBackedAutoWaive =
      metadata?.terminalStatus === "partial_success" &&
      // A prose progress note or a copied input file is not enough to waive a
      // failed mutation-required step. Require at least one genuine output.
      (mutationContractBlockers.length === 0 ||
        deliveredArtifactPaths.size > 0) &&
      hasSubstantiveOutcomeEvidence({
    ...
    let blockingFailedSteps = unresolvedFailedSteps.filter((id) => {
      ...
      if (
        waivedFailedStepIds.has(id) ||
        waivedNormalizedStepIds.has(normalized)
      ) {
        return false;
      }
    ...
    if (autoWaivedEvidenceBackedStepIds.size > 0) {
      const autoWaivedStepIds = Array.from(
        autoWaivedEvidenceBackedStepIds.values(),
      );
    ...
    if (blockingFailedSteps.length > 0) {
      ...
      const terminalFailureClass: Task["failureClass"] =
        hasMutationContractBlockers
          ? "contract_unmet_write_required"
          : "contract_error";
      const terminalFailureStatus: NonNullable<Task["terminalStatus"]> =
        hasMutationContractBlockers ? "failed" : "partial_success";
      if (metadata?.terminalStatus === "partial_success") {
        this.logEvent(taskId, "log", {
          metric: "completion_gate_blocked_partial_success",
          blocked: true,
          blockingFailedSteps,
          failedMutationRequiredStepIds: Array.from(
            failedMutationRequiredStepIds.values(),
          ),
          terminalStatusReason: metadata?.terminalStatusReason,
        });
    ...
    const reviewPolicy = resolveReviewPolicy(
      existingTask.agentConfig?.reviewPolicy,
    );
    const reviewDecision = deriveReviewGateDecision({
      policy: reviewPolicy,
      riskLevel: risk.level,
      isMutatingTask:
        inferMutationFromSummary(metadata?.outputSummary) ||
        risk.signals.changedFileCount > 0,
    });

    let terminalStatus: NonNullable<Task["terminalStatus"]> =
      metadata?.terminalStatus || "ok";
    let failureClass: Task["failureClass"] | undefined =
      metadata?.failureClass || undefined;
    ...
    const completionTelemetry = {
      ...this.computeTimelineTelemetryFromEvents(
    ...
    const updates: Partial<Task> = {
      ...
      error:
        resolvedOutcome.status === "completed"
          ? null
          : existingTask.error ||
            (resolvedOutcome.terminalStatus === "resume_available"
              ? "Resume available"
              : null),
      terminalStatus,
      failureClass,
    ...
    this.logEvent(
      taskId,
      isCompletedOutcome ? "task_completed" : "task_status",
      {
        ...
        ...(metadata?.verificationOutcome
          ...
          : {}),
        ...(metadata?.verificationScope
          ? { verificationScope: metadata.verificationScope }
          : {}),
        ...(metadata?.verificationEvidenceMode
          ? { verificationEvidenceMode: metadata.verificationEvidenceMode }
          : {}),
        ...(Array.isArray(metadata?.pendingChecklist) &&
        metadata?.pendingChecklist.length > 0
        ...
        metadata.waivedVerificationStepIds.length > 0
          ...
          : {}),
        ...(metadata?.terminalKind
          ? { terminalKind: metadata.terminalKind }
          : {}),
        ...(Array.isArray(metadata?.failedStepIds) &&
        metadata.failedStepIds.length > 0
          ? { failedStepIds: metadata.failedStepIds }
          : {}),
        ...(Array.isArray(metadata?.incompleteStepIds) &&
        metadata.incompleteStepIds.length > 0
    ...
    if (quality) {
      this.logEvent(
        taskId,
        quality.passed ? "review_quality_passed" : "review_quality_failed",
        {
          policy: reviewPolicy,
    ...
    if (isCompletedOutcome && reviewDecision.runVerificationAgent) {
      ...
      void this.runPostCompletionVerification(
        ...
        console.warn(
          "[AgentDaemon] Post-completion verification failed to launch:",
          error,
        );
      });
    }
    ...
    if (isCompletedOutcome && entropyDecision.runEntropySweep) {
      ...
      void this.runPostTaskEntropySweep(existingTask, {
        ...
        console.warn(
          "[AgentDaemon] Post-task entropy sweep failed to launch:",
          error,
        );
      });
    }
      ...
      if (worktreeSettings.autoCommitOnComplete) {
        void (async () => {
          try {
            ...
            console.error(
              `[AgentDaemon] Auto-commit failed for task ${taskId}:`,
              error,
            );
            this.logEvent(taskId, "log", {
    ...
    if (
      isCompletedOutcome &&
      existingTask?.comparisonSessionId &&
      comparisonSvc
    ) {
      void (async () => {
        try {
          ...
          console.error(
            `[AgentDaemon] Comparison callback failed for task ${taskId}:`,
            error,
          );
        }
      })();
    ...
    try {
      ...
      if (isCompletedOutcome && isTopLevelTask) {
        const workspaceName = this.workspaceRepo.findById(
          existingTask.workspaceId,
        )?.name;
        PersonalityManager.recordTaskCompleted(workspaceName);
        const gatewayContext =
          existingTask.agentConfig?.gatewayContext ?? "private";
        ...
        if (canCaptureRelationshipMemory) {
          RelationshipMemoryService.recordTaskCompletion(
            existingTask.title,
            typeof updates.resultSummary === "string"
              ? updates.resultSummary
              : undefined,
            taskId,
      ...
      console.warn(
        "[AgentDaemon] Failed to record relationship milestone:",
        error,
      );
    }
    if (this.teamOrchestrator) {
  ...
  private buildActiveArtifactFollowUpContext(
    ...
    const kindLabels: Record<
      NonNullable<TaskFollowUpInput["activeArtifactContext"]>["kind"],
      string
    > = {
  ...
  updateQueuedFollowUp(
    ...
    const currentSuffixIndex = current.message.lastIndexOf(
      current.displayMessage,
    );
    const nextExecutionMessage =
      currentSuffixIndex >= 0 &&
      currentSuffixIndex + current.displayMessage.length ===
  ...
  reorderQueuedFollowUps(
    ...
    const byId = new Map(
      queued.map((followUp) => [followUp.queueId, followUp]),
    );
  ...
  async sendMessage(
    ...
    const sideChatAgentConfigOverride =
      this.buildSideChatTurnAgentConfigOverride(task, message);
    ...
    if (overrideResult.changed) {
      this.taskRepo.update(taskId, {
        agentConfig: overrideResult.task.agentConfig,
      });
    }
    ...
    const accessAdjustedTask =
      cached?.executor.isRunning && !accessPolicyChanged
        ? overrideResult.task
        : this.applyTaskAccessSnapshot(overrideResult.task);
    const { task: roleAdjustedTask } =
      this.applyAgentRoleOverrides(accessAdjustedTask);
    ...
    if (annotationContext.annotations.length > 0) {
      ...
      this.logEvent(taskId, "annotation_addressing_started", {
        annotationIds: annotationContext.annotations.map(
    ...
    if (!cached) {
      ...
      const events = this.getTaskEventsForResume(
        taskId,
   ...
   */
  private processOrphanedFollowUps(
    taskId: string,
    executor: TaskExecutor,
  ): void {
    ...
    if (legacyFollowUps.length > 0) {
      const queued = this.deferredUserFollowUps.get(taskId) || [];
      const migrated = legacyFollowUps.map<DeferredUserFollowUp>(
        (followUp, index) => ({
          ...followUp,
          queueId: crypto.randomUUID(),
    ...
    for (const taskId of runningTaskIds) {
      ...
      if (cached) {
        try {
          ...
          console.error(
            `[AgentDaemon] Error cancelling task ${taskId}:`,
            error,
          );
        }
      }
  ...
  async handleTaskTimeout(taskId: string): Promise<void> {
    ...
    if (cached) {
      try {
        ...
        console.error(
          `[AgentDaemon] Error cancelling timed out task ${taskId}:`,
          error,
        );
      }
    }
    ...
    this.logEvent(taskId, "step_timeout", {
      message:
        "Task exceeded maximum execution time and was automatically cancelled",
    });
  }
  ...
  private emitQueueUpdate(status: QueueStatus): void {
    ...
    windows.forEach((window) => {
      try {
        ...
        console.error(
          `[AgentDaemon] Error sending queue update to window:`,
          error,
        );
      }
    });
  ...
  async shutdown(): Promise<void> {
    ...
    const shutdownInterruptedTaskIds = new Set<string>();
    this.activeTasks.forEach((cached, taskId) => {
      const currentTask = this.taskRepo.findById(taskId);
      if (!this.shouldInterruptCachedTaskOnShutdown(cached, currentTask)) {
        return;
      }
      shutdownInterruptedTaskIds.add(taskId);
      ...
      try {
        const interruptedOutcome = decideTaskOutcome({
          ...
          error:
            "Application shutdown while task was running - will resume on restart",
        });
        this.taskRepo.update(taskId, {
        ...
        this.logEvent(taskId, "task_interrupted", {
          message:
            "Task interrupted by application shutdown. Will resume on restart.",
          terminalStatus: interruptedOutcome.terminalStatus,
        });
      } catch (err) {
        console.error(
          `[AgentDaemon] Failed to update task ${taskId} status on shutdown:`,
          err,
        );
      }
    });
    ...
    this.activeTasks.forEach((cached, taskId) => {
      if (!shutdownInterruptedTaskIds.has(taskId)) return;
