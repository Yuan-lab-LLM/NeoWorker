...
export type CompletionContract = {
  ...
  /**
   * A terse continuation such as "继续" may finish verification of an
   * artifact produced immediately before the follow-up. In that narrow case,
   * existing artifact evidence is valid; explicit repair/regeneration turns
   * must still create or modify the requested artifact during the new turn.
   */
  allowExistingArtifactEvidence?: boolean;
...
// Normal tasks should either produce a result or switch to best-effort
// finalization promptly. Long autonomous work uses the separate deep-work
// deadline below.
export const STEP_TIMEOUT_MS = 8 * 60 * 1000;

// Deep work remains intentionally larger, but still has a bounded step window
// so one broken provider/tool lane cannot occupy the task indefinitely.
...
// One initial attempt plus two retries is enough to distinguish a transient
// failure from a broken route without spending many minutes on the same call.
export const MAX_TOOL_FAILURES = 3;

// Initial plans should stay compact so each task does not spend most of its
// runtime crossing LLM/tool boundaries between tiny steps. Keep one extra slot
// in the total budget for a bounded recovery revision.
export const MAX_INITIAL_PLAN_STEPS = 7;
export const MAX_TOTAL_STEPS = 8;
