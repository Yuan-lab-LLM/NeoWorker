...
function isPackagingRelatedToolCall(toolName: string, input: unknown): boolean {
  ...
  return /\b(pdf|epub|docx|ebook|manuscript|cover|fpdf|pdfkit|pandoc)\b/.test(
    serializedInput,
  );
}
...
export function appendMaxTokensRecoveryUserMessage(
  ...
  truncationClassification?:
    "visible_partial_output" | "reasoning_exhausted" | null,
): void {
  const recoveryText =
    truncationClassification === "reasoning_exhausted"
      ? "The previous attempt exhausted its completion allowance before producing visible content. " +
        "Do not resume, repeat, or expand internal analysis. Use the existing context and immediately emit exactly one next required tool call. " +
        "If no tool is needed, return a concise usable answer first. Keep tool arguments bounded, do not re-read or re-search known inputs, " +
        "and verify the requested deliverable before claiming completion."
...
export function handleMaxTokensRecovery(opts: {
  ...
  opts.emitMaxTokensRecovery({
    ...
    message:
      nextRecoveryCount <= opts.maxRecoveries
        ? opts.truncationClassification === "reasoning_exhausted"
          ? "本轮额度已用完但尚无可见结果，正在切换为直接行动模式自动恢复。"
          : "已达到 NeoWorker 本轮输出预算，已有内容已保留，正在从中断位置继续。"
        : "已达到 NeoWorker 本轮输出预算，自动续写次数已用完。",
    recoveryMode:
      opts.truncationClassification === "reasoning_exhausted"
        ? "action_first"
        : "continue_from_cutoff",
    ...opts.eventPayload,
  });

  if (nextRecoveryCount > opts.maxRecoveries) {
    opts.log(
      `${messagePrefix}max_tokens recovery exhausted after ${opts.maxRecoveries} attempts`,
    );
  ...
  const remainingTurns =
    ...
    Number.isFinite(opts.remainingTurns)
      ? opts.remainingTurns
      : Number.POSITIVE_INFINITY;
  if (opts.allowRetry === false) {
    opts.log(
      `${messagePrefix}max_tokens retry skipped: adaptive continuation disabled`,
    );
    appendRecoveryAssistantMessage(opts.messages, opts.response, false);
    return { action: "exhausted", recoveryCount: nextRecoveryCount };
  }
  ...
  appendMaxTokensRecoveryUserMessage(
    opts.messages,
    opts.truncationClassification,
  );
...
export function computeToolFailureDecision(opts: {
  ...
  const shouldStopFromHardFailure =
    opts.hasHardToolFailureAttempt && allToolsFailed;
...
export function maybeInjectToolLoopBreak(opts: {
  ...
  for (const content of opts.responseContent || []) {
    ...
    const isLoop = opts.detectToolLoop(
      opts.recentToolCalls,
      content.name,
      content.input,
      5,
    );
    ...
    opts.messages.push({
      ...
      content: [
        {
          type: "text",
          text: opts.sanitizeMessageText
            ? opts.sanitizeMessageText(messageText)
            : messageText,
        },
      ],
    });
      ...
      content: [
        {
          ...
          text: (() => {
            if (opts.mutationRequired && !opts.mutationSatisfied) {
              ...
              return opts.sanitizeMessageText
                ? opts.sanitizeMessageText(message)
                : message;
            }
            const message =
              `${escalationTag}\n` +
...
export function maybeInjectStopReasonNudge(opts: {
  ...
  const toolUseStreakTriggered =
    stopReason === "tool_use" &&
    (opts.consecutiveToolUseStops >= minToolUseStreak ||
      opts.remainingTurns <= 1);
  const maxTokenStreakTriggered =
    stopReason === "max_tokens" &&
    opts.consecutiveMaxTokenStops >= minMaxTokenStreak;
  ...
  const consecutiveCount =
    stopReason === "tool_use"
      ? opts.consecutiveToolUseStops
      : opts.consecutiveMaxTokenStops;
  opts.log(
  ...
  if (stopReason === "tool_use") {
    if (opts.suppressToolUseStopNudge) {
      const requiredTools =
        Array.isArray(opts.requiredToolNames) &&
        opts.requiredToolNames.length > 0
...
export function shouldLockFollowUpToolCalls(opts: {
  ...
  const allowImmediateTurnBudgetLock =
    opts.allowImmediateTurnBudgetLock ?? true;
  const repeatedPackagingFailureThreshold =
    opts.repeatedPackagingFailureThreshold ?? 2;
  ...
  if (
    opts.stopReason === "tool_use" &&
    (opts.repeatedPackagingFailureCount || 0) >=
      repeatedPackagingFailureThreshold
  ) {
    return true;
  }
  return (
