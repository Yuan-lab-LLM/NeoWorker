...
describe("TaskStrategyService deriveLlmProfile", () => {
  ...
  it("reserves extra quality passes for explicit deep work", () => {
    const strategy = TaskStrategyService.derive(makeRoute({ intent: "deep_work" }));
    expect(strategy.deepWorkMode).toBe(true);
    expect(strategy.qualityPasses).toBe(2);
  });
  ...
  it("promotes a Chinese Excel creation request without using attachment text", () => {
    const strategy = TaskStrategyService.derive(
      makeRoute({
        intent: "chat",
        conversationMode: "chat",
    ...
    expect(strategy.executionMode).toBe("execute");
    expect(strategy.conversationMode).toBe("task");
    expect(strategy.taskDomain).toBe("general");
    expect(strategy.qualityPasses).toBe(1);
  });
...
describe("TaskStrategyService getRelevantToolSet", () => {
  ...
  it("keeps chat on the explicit attachment and web lookup allow-list", () => {
    const chat = TaskStrategyService.getRelevantToolSet("chat");
    expect(chat.has("read_file")).toBe(true);
    expect(chat.has("parse_document")).toBe(true);
    expect(chat.has("analyze_image")).toBe(true);
    expect(chat.has("read_pdf_visual")).toBe(true);
    expect(chat.has("web_search")).toBe(true);
    expect(chat.has("web_fetch")).toBe(true);
    expect(chat.has("http_request")).toBe(true);
    expect(chat.has("tool_search")).toBe(false);
    expect(chat.has("list_directory")).toBe(false);
    expect(chat.has("task_history")).toBe(false);
