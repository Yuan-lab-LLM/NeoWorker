...
describe("tool failure normalization", () => {
  ...
  it("classifies explicitly non-retryable attachment failures as hard failures", () => {
    expect(
      isHardToolFailure(
        "analyze_image",
        {
  ...
  it("preserves re-upload guidance in normalized tool failures", () => {
    const normalized = buildNormalizedToolResult({
      toolName: "analyze_image",
      toolUseId: "tool-missing-image",
      result: {
        success: false,
        error: "Attachment is no longer available",
        ...
        actionHint: {
          ...
          label: "重新上传图片",
          target: ".neoworker/uploads/123/missing.png",
        },
      },
      rawResult: "{}",
    ...
    expect(normalized.toolResult.content).toContain("ATTACHMENT_UNAVAILABLE");
    expect(normalized.toolResult.content).toContain('"retryable":false');
    expect(normalized.toolResult.content).toContain("reupload_attachment");
  });
