...
describe("tool-policy-engine request_user_input gating", () => {
  it("allows only attachment-reading and web lookup tools in chat mode", () => {
    ...
    const documentDecision = evaluateToolPolicy("parse_document", {
      executionMode: "chat",
      taskDomain: "auto",
    });
    expect(readDecision.decision).toBe("allow");
    expect(webDecision.decision).toBe("allow");
    expect(documentDecision.decision).toBe("allow");
  });

  it.each(["write_file", "list_directory", "git_status", "browser_navigate", "task_history"])(
    "blocks non-whitelisted tool %s in chat mode",
    (toolName) => {
      ...
      expect(decision.reason).toContain(
        "web lookup and read-only attachment tools",
      );
    },
  );
