...
describe("AgentDaemon automated task model selection", () => {
  it("repairs an attachment-derived stale code strategy for an Excel retry", () => {
...
- meeting.docx (.neoworker/uploads/123/meeting.docx)
  ...
  [[ATTACHMENT_EXTRACTED_CONTENT_START]]
    API implementation notes and a PowerPoint follow-up.
  [[ATTACHMENT_EXTRACTED_CONTENT_END]]`;
    ...
    expect(result.agentConfig.taskDomain).toBe("general");
    expect(result.agentConfig.qualityPasses).toBe(1);
