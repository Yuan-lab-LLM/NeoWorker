...
describe("AgentDaemon.completeTask", () => {
  ...
  it("does not treat a copied source PPTX as the requested PDF deliverable", () => {
    const daemonLike = createDaemonLike();
    daemonLike.taskRepo.findById.mockReturnValue({
      id: "task-1",
      title: "基于材料内容，生动PDF文档",
      prompt: "基于材料内容，生动PDF文档",
      rawPrompt: "基于材料内容，生动PDF文档",
      status: "executing",
      workspaceId: "workspace-1",
      parentTaskId: "parent-task",
      agentType: "sub",
    ...
    daemonLike.eventRepo.findByTaskId.mockReturnValue([
      {
        ...
        payload: {
          path: "材料原稿.pptx",
          copiedFrom: ".neoworker/uploads/source.pptx",
          legacyType: "file_created",
        },
      },
    ]);
    ...
    AgentDaemon.prototype.completeTask.call(
      ...
      "已经读取材料，准备生成 PDF。",
      {
        terminalStatus: "partial_success",
        failureClass: "contract_unmet_write_required",
        failedMutationRequiredStepIds: ["1"],
        outputSummary: {
          created: ["材料原稿.pptx"],
          primaryOutputPath: "材料原稿.pptx",
          outputCount: 1,
          folders: ["."],
        },
      },
    ...
    expect(daemonLike.logEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        gate: "completion_required_artifact_gate",
        missingRequiredArtifactExtensions: [".pdf"],
      }),
    );
  });

  it("ignores PPT mentions inside attachment extracts for an Excel-only task", () => {
    const daemonLike = createDaemonLike();
...
- meeting.docx (.neoworker/uploads/meeting.docx)
  ...
  上周已经完成产品方案 PPT，客户还讨论了 PowerPoint 演示。
  [[ATTACHMENT_EXTRACTED_CONTENT_END]]`;
    ...
    AgentDaemon.prototype.completeTask.call(
      ...
      "Excel 台账已经生成。",
      {
        terminalStatus: "ok",
        outputSummary: {
          created: ["AI-HPC例会台账.xlsx"],
          primaryOutputPath: "AI-HPC例会台账.xlsx",
          outputCount: 1,
          folders: ["."],
        },
      },
  ...
  it("completes a PPT task when DOCX files are inputs rather than requested outputs", () => {
    const daemonLike = createDaemonLike();
    const prompt = `基于内容，生成PPT文件

Attached files (relative to workspace):
- AI&HPC 产品线例会纪要-褚福州-20260510.docx (.neoworker/uploads/meeting-1.docx)
  Attachment metadata: mime=application/vnd.openxmlformats-officedocument.wordprocessingml.document
  Extracted content:
  [[ATTACHMENT_EXTRACTED_CONTENT_START]]
  第一份例会纪要。
  [[ATTACHMENT_EXTRACTED_CONTENT_END]]
- AI&HPC 产品线例会纪要-褚福州-20260622.docx (.neoworker/uploads/meeting-2.docx)
  Attachment metadata: mime=application/vnd.openxmlformats-officedocument.wordprocessingml.document
  Extracted content:
  [[ATTACHMENT_EXTRACTED_CONTENT_START]]
  第二份例会纪要。
  [[ATTACHMENT_EXTRACTED_CONTENT_END]]`;
    ...
    AgentDaemon.prototype.completeTask.call(
      daemonLike,
      "task-1",
      "PPTX 已生成并通过结构与视觉质量检查。",
      {
        terminalStatus: "ok",
        outputSummary: {
          created: [".neoworker/基于内容_生成ppt文件.pptx"],
          primaryOutputPath: ".neoworker/基于内容_生成ppt文件.pptx",
          outputCount: 1,
          folders: [".neoworker"],
        },
      },
