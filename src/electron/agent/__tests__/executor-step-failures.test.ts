...
describe("TaskExecutor executeStep failure handling", () => {
  ...
  it("does not require delete_file when removing placeholders from HTML content", () => {
    executor = createExecutorWithStubs([], {});

    const step: Any = {
      id: "html-without-placeholders",
      description:
        "Create the final standalone HTML experience `dashboard.html` with all requested content. Remove every staging placeholder before delivery and produce exactly one usable .html file.",
      status: "pending",
    };
  ...
  it("completes the HTML placeholder-cleanup step after write_file succeeds", async () => {
    executor = createExecutorWithStubs(
      [
        toolUseResponse("write_file", {
          path: "dashboard.html",
        ...
        textResponse("Created dashboard.html without placeholders."),
      ],
      {
        write_file: { success: true, path: "dashboard.html" },
      },
    );
    const tempDir = fs.mkdtempSync("/tmp/neoworker-html-contract-");
    (executor as Any).workspace.path = tempDir;
    fs.writeFileSync(
      path.join(tempDir, "dashboard.html"),
      "<!doctype html><title>Dashboard</title>",
    );

    const step: Any = {
      id: "html-placeholder-cleanup",
      description:
        "Create the final standalone HTML experience `dashboard.html` with all requested content. Remove every staging placeholder before delivery and produce exactly one usable .html file.",
      status: "pending",
    };
    ...
    try {
      await (executor as Any).executeStep(step);
      expect(step.status, String(step.error || "")).toBe("completed");
      expect(String(step.error || "")).not.toContain("delete_file");
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
  });

  it("still requires delete_file for an explicit file deletion step", () => {
    executor = createExecutorWithStubs([], {});

    const step: Any = {
      id: "delete-obsolete-file",
      description: "Delete the obsolete file `draft.html` before delivery.",
      status: "pending",
    };

    const contract = (executor as Any).resolveStepExecutionContract(step);

    expect(Array.from(contract.requiredTools)).toContain("delete_file");
  });
...
[/AGENT_STRATEGY_CONTEXT_V1]`;
  ...
  it("reconciles create_directory-only contract misses when equivalent artifacts were written later", async () => {
    ...
    expect((executor as Any).plan.steps[0].status).toBe("completed");
    expect((executor as Any).plan.steps[0].error).toBeUndefined();
    expect((executor as Any).daemon.logEvent).toHaveBeenCalledWith(
      "task-1",
    ...
    expect((executor as Any).daemon.logEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        recovered: true,
        recoveredBy: "alternate_plan_step",
      }),
    );
  });

  it("reconciles a timed-out HTML step when its final write is complete", async () => {
    executor = createExecutorWithStubs([textResponse("done")], {});
    const workspacePath = fs.mkdtempSync(
      path.join("/tmp", "neoworker-timeout-complete-html-"),
    );
    const artifactPath = path.join(workspacePath, "dashboard.html");
    const step: Any = {
      id: "plan-html-timeout-1",
      description:
        "Create the final interactive standalone HTML page `dashboard.html`.",
      status: "pending",
    };
    (executor as Any).workspace.path = workspacePath;
    (executor as Any).task.title = "Create an interactive HTML dashboard";
    (executor as Any).task.prompt =
      "Create an interactive standalone HTML dashboard with a canvas chart and button.";
    (executor as Any).plan = { description: "Plan", steps: [step] };
    (executor as Any).executeStep = vi.fn(async (target: Any) => {
      fs.writeFileSync(
        artifactPath,
        `<!doctype html><html><head><title>Dashboard</title></head><body>
          <section><canvas id="chart"></canvas><button id="refresh">Refresh</button></section>
          <script>
      ...
      (executor as Any).recordArtifactMutationLedgerEntry("dashboard.html", {
        stepId: target.id,
        tool: "write_file",
        evidence: {
          tool_success: true,
          canonical_tool: "write_file",
          reported_path: artifactPath,
          artifact_registered: true,
          fs_exists: true,
          mtime_after_step_start: true,
      ...
      target.status = "failed";
      target.error = "Step timed out after 480s";
      target.completedAt = Date.now();
    });

    try {
      await expect((executor as Any).executePlan()).resolves.toBeUndefined();
      const executedStep = (executor as Any).plan.steps.find(
        (candidate: Any) => candidate.id === step.id,
      );
      expect(executedStep.status).toBe("completed");
      expect(executedStep.error).toBeUndefined();
      expect((executor as Any).daemon.logEvent).toHaveBeenCalledWith(
        "task-1",
        "step_contract_reconciled_posthoc",
        expect.objectContaining({
          stepId: step.id,
          reconciledBy: "complete_artifact_evidence_after_timeout",
        }),
      );
    } finally {
  ...
  it("still emits step_timeout when the abort came from an execution timeout", async () => {
    ...
    expect(executedStep.status).toBe("failed");
    expect((executor as Any).daemon.logEvent).toHaveBeenCalledWith(
      "task-1",
      "step_timeout",
      expect.objectContaining({
  ...
  it("reports the real model timeout, removes its unchanged HTML stub, and skips dependent verification", async () => {
    executor = createExecutorWithStubs([textResponse("done")], {});
    const workspacePath = fs.mkdtempSync(
      path.join("/tmp", "neoworker-timeout-chain-"),
    );
    (executor as Any).workspace.path = workspacePath;
    (executor as Any).task.title = "生成中文股票分析网页";
    (executor as Any).task.prompt = "生成一个中文股票实时分析系统网页";
    const artifactPath = path.join(workspacePath, ".neoworker", "final.html");
    ...
    const mutationStep: Any = {
      id: "plan-html-create-1",
      description:
        "Create the final standalone HTML experience `.neoworker/final.html` with complete requested content and styling.",
      kind: "primary",
      status: "pending",
    };
    const verificationStep: Any = {
      id: "plan-html-verify-2",
      description:
        "Verify the final HTML file `.neoworker/final.html` in a browser and inspect its rendered content.",
      kind: "verification",
      status: "pending",
    };
    (executor as Any).plan = {
      description: "Plan",
      steps: [mutationStep, verificationStep],
    };
    (executor as Any).executeStep = vi.fn(async (target: Any) => {
      if (target.id === mutationStep.id) {
        throw new LLMRequestTimeoutError("Step execution", 150_000);
      }
      target.status = "completed";
      target.completedAt = Date.now();
    });
    ...
    try {
      ...
      const executedMutationStep = (executor as Any).plan.steps.find(
        (candidate: Any) => candidate.id === mutationStep.id,
      );
      const executedVerificationStep = (executor as Any).plan.steps.find(
        (candidate: Any) =>
          candidate.kind === "verification" ||
          String(candidate.description).includes("Verify the final HTML file"),
      );
      expect(executedMutationStep.status).toBe("failed");
      expect(executedMutationStep.error).toBe(
        "模型生成请求在 150 秒后超时",
      );
      expect(executedVerificationStep.status).toBe("skipped");
      expect(String(executedVerificationStep.error)).toContain(
        "前置文件生成失败",
      );
      expect((executor as Any).executeStep).toHaveBeenCalledTimes(1);
      ...
      expect((executor as Any).daemon.logEvent).toHaveBeenCalledWith(
        ...
        "step_failed",
        expect.objectContaining({
          errorCode: "NEOWORKER_LLM_TIMEOUT",
        }),
      );
    } finally {
      fs.rmSync(workspacePath, { recursive: true, force: true });
  ...
  it("localizes internal mutation-contract failures for Chinese tasks", () => {
    executor = createExecutorWithStubs([textResponse("done")], {});
    (executor as Any).task.title = "生成中文网页";
    (executor as Any).task.prompt = "请生成一个中文 HTML 网页";

    const message = (executor as Any).buildTaskFailureMessage(
      new Error(
        "Task failed: mutation-required contract unmet - Create final HTML",
      ),
      "contract_unmet_write_required",
    );

    expect(message).toContain("最终文件没有成功写入");
    expect(message).toContain("后续验证已停止");
    expect(message).not.toContain("mutation-required contract unmet");
  });
  ...
  it("does not treat reported artifact path alone as successful mutation evidence", async () => {
    ...
    (executor as Any).task.prompt =
      "Create contract_negotiation_training.pptx as a PowerPoint presentation.";
  ...
  it("does not bootstrap unrelated internal Markdown paths for an XLSX contract", async () => {
    executor = createExecutorWithStubs([textResponse("OK")], {});
    const tempDir = fs.mkdtempSync("/tmp/neoworker-bootstrap-xlsx-");
    (executor as Any).workspace.path = tempDir;
    (executor as Any).task.prompt =
      "Generate report.xlsx. Internal context path: agent.md/soul.md/user.md";

    const step: Any = {
      id: "bootstrap-xlsx-no-internal-md",
      description: "Create the final spreadsheet `report.xlsx`.",
      status: "pending",
    };
    const stepContract: Any = {
      requiredTools: new Set(["create_spreadsheet"]),
      mode: "mutation_required",
      requiresMutation: true,
      requiresArtifactEvidence: true,
      targetPaths: ["report.xlsx"],
  ...
  it("never bootstraps the injected agent.md/soul.md/user.md context path", async () => {
    executor = createExecutorWithStubs([textResponse("OK")], {});
    const tempDir = fs.mkdtempSync("/tmp/neoworker-bootstrap-internal-context-");
    (executor as Any).workspace.path = tempDir;
    (executor as Any).task.prompt =
      "Inspect how agent.md/soul.md/user.md are used as internal agent context.";

    const step: Any = {
      id: "bootstrap-no-internal-context",
      description: "Prepare the requested Markdown output.",
      status: "pending",
    };
    const stepContract: Any = {
      requiredTools: new Set(["write_file"]),
    ...
    try {
      ...
      expect(bootstrap.attempted).toBe(false);
      expect(fs.existsSync(path.join(tempDir, "agent.md"))).toBe(false);
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
  ...
  it("keeps intermediate presentation plans scoped to their own JSON write", () => {
    executor = createExecutorWithStubs([], {});
    ...
    const step: Any = {
      id: "write-presentation-plan",
      description:
        "Write the narrative plan to presentation-plan.json for later slide compilation",
      status: "pending",
    };
    ...
    expect(Array.from(contract.requiredTools)).toContain("write_file");
    expect(Array.from(contract.requiredTools)).not.toContain("run_command");
    expect(Array.from(contract.requiredTools)).not.toContain(
      "create_presentation",
    );
    expect(contract.requiredExtensions).toContain(".json");
    expect(contract.requiredExtensions).not.toContain(".pptx");
  });

  it("does not require an input DOCX as output for a PPTX creation step", () => {
    executor = createExecutorWithStubs([], {});
    (executor as Any).task.title = "Build a PowerPoint deck";
    (executor as Any).task.prompt =
      "Use the attached meeting-notes.docx to create a PowerPoint presentation.";
    (executor as Any).appliedSkills = [
      {
        skillId: "presentation-studio",
        skillName: "Presentation Studio",
    ...
    const step: Any = {
      id: "compile-final-deck",
      description:
        "Create final-deck.pptx from the input meeting-notes.docx and verify the deck",
      status: "pending",
    };
    ...
    expect(contract.requiredExtensions).not.toContain(".docx");
  });

  it("accepts a newly written artifact reported through a filePath event", () => {
    executor = createExecutorWithStubs([], {});
    const workspace = fs.mkdtempSync("/tmp/neoworker-presentation-plan-");
    const outputPath = path.join(workspace, "presentation-plan.json");
    fs.writeFileSync(outputPath, '{"slides":[]}');
    (executor as Any).workspace.path = workspace;
    (executor as Any).daemon.getTaskEvents.mockReturnValue([
      {
        type: "file_created",
        timestamp: Date.now(),
    ...
    try {
      const evidence = (executor as Any).collectMutationEvidence({
        ...
        input: {},
        result: { success: true },
        stepStartedAt: Date.now() - 100,
        toolStartedAt: Date.now() - 50,
        stepContract: { requiredExtensions: [".json"] },
      });
      ...
      expect(
        (executor as Any).mutationEvidenceSatisfiesWriteContract(evidence),
      ).toBe(true);
    } finally {
  ...
  it("completes an intermediate presentation-plan JSON write without requiring PPT generation", async () => {
    const workspace = fs.mkdtempSync("/tmp/neoworker-presentation-plan-step-");
    executor = createExecutorWithStubs(
      [
        toolUseResponse("write_file", {
          path: "presentation-plan.json",
          content: '{"slides":[]}',
        }),
        textResponse("Saved presentation-plan.json."),
      ],
      {},
    );
    (executor as Any).workspace.path = workspace;
    (executor as Any).task.title = "Create a PowerPoint deck";
    (executor as Any).task.prompt = "Create a product launch PPT.";
    (executor as Any).toolRegistry.executeTool = vi.fn(
      async (name: string, input: Any) => {
        if (name === "write_file") {
          fs.writeFileSync(path.join(workspace, input.path), input.content);
          return { success: true };
        }
        return { success: true };
      },
    ...
    try {
      await (executor as Any).executeStep(step);
      expect(step.status, String(step.error || "")).toBe("completed");
      expect(fs.existsSync(path.join(workspace, "presentation-plan.json"))).toBe(
        true,
      );
      expect(String(step.error || "")).not.toContain("create_presentation");
    } finally {
      fs.rmSync(workspace, { recursive: true, force: true });
    }
  });
  ...
  it("downgrades only browser infrastructure failures after HTML integrity passes", () => {
    executor = createExecutorWithStubs([], {});
    const internal = executor as Any;
    internal.buildCompletionContract = vi.fn().mockReturnValue({
      requiresArtifactEvidence: true,
      requiredArtifactExtensions: [".html"],
    });
    internal.fileOperationTracker = {
      getCreatedFiles: vi.fn().mockReturnValue(["dashboard.html"]),
    };
    internal.buildTaskOutputSummary = vi.fn().mockReturnValue({
      created: ["dashboard.html"],
      primaryOutputPath: "dashboard.html",
      outputCount: 1,
      folders: ["."],
    });
    internal.getHtmlArtifactIntegrityError = vi.fn().mockReturnValue(null);
    const step: Any = {
      id: "verify-html-browser",
      description: "Browser verification for generated dashboard.html",
      status: "failed",
      kind: "verification",
    };

    expect(
      internal.canDowngradeHtmlBrowserInfrastructureFailure(
        step,
        "",
    ...
    internal.getHtmlArtifactIntegrityError.mockReturnValue(
      "HTML artifact is incomplete: unresolved staging placeholder",
    );
    expect(
      internal.canDowngradeHtmlBrowserInfrastructureFailure(
        step,
    ...
    expect(
      internal.canDowngradeHtmlBrowserInfrastructureFailure(
        step,
        "",
        "The rendered chart is blank and the button throws TypeError",
      ),
    ).toBe(false);
  });
  ...
  it("completes image analysis when recoverable Kimi vision output is followed by a direct answer", async () => {
    executor = createExecutorWithStubs(
      [
        toolUseResponse("analyze_image", {
          path: ".neoworker/uploads/avatar.webp",
          prompt: "Identify the product logo and describe its visual style.",
        }),
        textResponse(
          "这是一枚 3D 卡通女性头像，采用圆形浅蓝背景、柔和渐变和高亲和力表情，适合作为 AI 助手产品形象。",
        ),
      ],
      {
        analyze_image: {
          success: false,
          error: 'Active vision model "kimi-k2.6" returned no text.',
          nonBlocking: true,
          recoverableFallback: true,
          fallbackHint:
            "Continue from the image already attached to the conversation.",
        },
      },
    );
    ...
    const step: Any = {
      id: "image-analysis-empty-kimi-recovery",
      description: "使用 analyze_image 分析附件图片，提取视觉特征并评价形象",
      status: "pending",
      kind: "primary",
    };
    (executor as Any).plan = { description: "Plan", steps: [step] };
