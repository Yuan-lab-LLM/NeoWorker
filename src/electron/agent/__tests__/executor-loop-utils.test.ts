...
describe("executor-loop-utils guardrails", () => {
  ...
  it("uses an action-first recovery when reasoning consumed all visible output", () => {
    const messages: LLMMessage[] = [];
    const emitMaxTokensRecovery = vi.fn();
    const result = handleMaxTokensRecovery({
      response: {
        stopReason: "max_tokens",
      ...
      log: vi.fn(),
      emitMaxTokensRecovery,
    });

    expect(result.action).toBe("retry");
    expect(
      String((messages.at(-1)?.content as Any[])[0]?.text || ""),
    ).toContain("immediately emit exactly one next required tool call");
    expect(emitMaxTokensRecovery).toHaveBeenCalledWith(
      expect.objectContaining({
        recoveryMode: "action_first",
        message: expect.stringContaining("直接行动模式"),
      }),
    );
  });
  ...
  it("injects low-progress nudge for repeated mixed-tool probing on same target", () => {
    const calls: ToolLoopCall[] = [
      ...
      {
        tool: "read",
        target: "/tmp/a.html:200-400",
        baseTarget: "/tmp/a.html",
      },
      {
      ...
      {
        tool: "read",
        target: "/tmp/a.html:400-600",
        baseTarget: "/tmp/a.html",
      },
      { tool: "search", target: "/tmp/a.html", baseTarget: "/tmp/a.html" },
      {
        tool: "read",
        target: "/tmp/a.html:600-800",
        baseTarget: "/tmp/a.html",
      },
    ];
    const messages: LLMMessage[] = [];
