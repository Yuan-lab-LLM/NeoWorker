...
describe("TaskExecutor provider failover retry semantics", () => {
  ...
  it("hands malformed tool arguments to the corrective turn loop without an identical retry", async () => {
    const executor = createRetryExecutor() as Any;
    executor.llmCallSequence = 0;
    executor.recordObservedOutputThroughput = vi.fn();
    ...
    const malformed = Object.assign(
      new Error("The model returned malformed tool arguments"),
      {
    ...
    const requestFn = vi.fn().mockRejectedValue(malformed);

    await expect(
      executor.callLLMWithRetry(requestFn, "Spreadsheet creation"),
    ).rejects.toThrow(/malformed tool arguments/i);
