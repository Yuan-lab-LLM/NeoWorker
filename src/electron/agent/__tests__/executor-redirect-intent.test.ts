...
describe("TaskExecutor — isRedirectIntent", () => {
  ...
  describe("Pattern 1 — ignore-and-pivot", () => {
    ...
    it("detects 'ignore X and work on Y'", () => {
      expect(
        isRedirect("ignore the bug fixes and work on the dashboard instead"),
      ).toBe(true);
    });

    it("detects 'ignore X, do Y'", () => {
      expect(isRedirect("ignore the auth module, do the payment flow")).toBe(
        true,
      );
    });
    ...
    it("detects 'ignore X and build Y'", () => {
      expect(
        isRedirect(
          "ignore the refactoring tasks and build the new onboarding flow",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Pattern 2 — explicit pivot language", () => {
    it("detects 'pivot to X'", () => {
      expect(
        isRedirect("let's pivot to building the authentication flow"),
      ).toBe(true);
    });
    ...
    it("detects 'change direction'", () => {
      expect(
        isRedirect("change direction and focus on the payment module"),
      ).toBe(true);
    });

    it("detects 'change focus'", () => {
      expect(
        isRedirect("change focus to the frontend performance issues"),
      ).toBe(true);
    });

    it("detects 'change approach'", () => {
      expect(
        isRedirect("let's change approach and try a different architecture"),
      ).toBe(true);
    });

    it("detects 'new direction'", () => {
      expect(isRedirect("new direction: focus on the mobile app instead")).toBe(
        true,
      );
    });
  });
  ...
  describe("Pattern 3 — contrast pivot", () => {
    ...
    it("detects 'instead of X, work on Y'", () => {
      expect(
        isRedirect(
          "instead of the database migration, work on the UI redesign",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Pattern 4 — negate-and-pivot", () => {
    it("detects 'forget that and focus on X'", () => {
      expect(
        isRedirect("forget that approach and instead focus on the API layer"),
      ).toBe(true);
    });
    ...
    it("detects 'skip X and work on Y'", () => {
      expect(
        isRedirect("skip the tests for now and work on the deployment script"),
      ).toBe(true);
    });
    ...
    it("detects 'drop X, concentrate on Y'", () => {
      expect(
        isRedirect(
          "drop the OpenClaw integration, concentrate on the new features",
        ),
      ).toBe(true);
    });
    ...
    it("detects 'abandon X, focus instead'", () => {
      expect(
        isRedirect(
          "abandon the current plan and focus instead on delivering the MVP",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("Pattern 5 — scope narrowing", () => {
    it("detects 'focus only on Y'", () => {
      expect(
        isRedirect("focus only on the new features, not the old bugs"),
      ).toBe(true);
    });
    ...
    it("detects 'focus solely on Y'", () => {
      expect(isRedirect("focus solely on the performance improvements")).toBe(
        true,
      );
    });
  ...
  describe("non-redirect messages — should NOT be detected", () => {
    ...
    it("does not flag plain execution tasks", () => {
      expect(isRedirect("build a REST API for user authentication")).toBe(
        false,
      );
      expect(isRedirect("create a new React component for the dashboard")).toBe(
        false,
      );
    });
    ...
    it("does not flag extend follow-ups", () => {
      ...
      expect(isRedirect("can you also look at the payment service?")).toBe(
        false,
      );
    });

    it("does not flag correction follow-ups", () => {
      expect(isRedirect("you used the wrong API, please use v2 instead")).toBe(
        false,
      );
      expect(isRedirect("actually use the postgres database not sqlite")).toBe(
        false,
      );
    });
...
describe("TaskExecutor — compactHistoryForRedirect", () => {
  it("replaces full conversation history with a user→assistant stub pair", () => {
    ...
    executor.conversationHistory = [
      ...
      {
        role: "assistant",
        content: [{ type: "text", text: "Done, fixed 3 bugs." }],
      },
      {
        role: "user",
      ...
      {
        role: "assistant",
        content: [{ type: "text", text: "Logs look clean." }],
      },
    ];
...
describe("TaskExecutor — contextual terminal follow-up isolation", () => {
  const executor = Object.create(TaskExecutor.prototype) as Any;
  const isStateUpdate = (text: string): boolean =>
    (executor as any).isTerminalContextualStateUpdate(text);

  it("recognizes short installation and configuration completion replies", () => {
    expect(isStateUpdate("安装完了")).toBe(true);
    expect(isStateUpdate("icalBuddy配置好了！")).toBe(true);
    expect(isStateUpdate("已经授权了")).toBe(true);
    expect(isStateUpdate("I've installed it successfully.")).toBe(true);
  });

  it("does not treat new artifact work or an explicit continuation as a state update", () => {
    expect(isStateUpdate("生成一个PDF文件")).toBe(false);
    expect(isStateUpdate("继续完成之前的Excel")).toBe(false);
    expect(isStateUpdate("帮我分析这三份文档")).toBe(false);
  });

  it("retains only the latest text exchange and removes old attachment bodies", () => {
    const scopedExecutor = Object.create(TaskExecutor.prototype) as Any;
    scopedExecutor.task = { title: "基于材料生成PDF和Excel" };
    scopedExecutor._runtime = null;
    scopedExecutor.workspace = { id: "workspace-1" };
    scopedExecutor.conversationHistory = [
      {
        role: "user",
        content: [
          {
      ...
      {
        role: "assistant",
        content: [{ type: "text", text: "Excel 已生成。" }],
      },
      {
        role: "user",
        content: [{ type: "text", text: "接着帮我查一下系统日历" }],
      },
      {
        role: "assistant",
        content: [
          {
            type: "text",
            text: "需要安装 icalBuddy 才能继续查询日历。需要我帮你安装吗？",
          },
        ],
      },
    ];
    ...
    const compacted = (
      scopedExecutor as any
    ).compactHistoryForContextualFollowUp();

    expect(compacted).toBe(true);
    expect(scopedExecutor.conversationHistory).toHaveLength(2);
    expect(
      scopedExecutor.conversationHistory.map((message: Any) => message.role),
    ).toEqual(["user", "assistant"]);
    const retainedText = scopedExecutor.conversationHistory
      .map((message: Any) => message.content[0].text)
      .join("\n");
    expect(retainedText).toContain("系统日历");
    expect(retainedText).toContain("icalBuddy");
    expect(retainedText).not.toContain("旧文档正文");
    expect(retainedText).not.toContain("Excel 已生成");
  });
});
...
describe("TaskExecutor — sendMessageLegacy redirect wiring", () => {
  it("sets redirectRequested=true for pivot messages and false for plain ones", () => {
    ...
    expect(check("ignore the openclaw fixes and focus on new features")).toBe(
      true,
    );
