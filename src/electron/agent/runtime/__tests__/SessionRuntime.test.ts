...
function createHarness() {
  ...
  const verifySuccessCriteria = vi.fn(async () => ({
    success: true,
    message: "ok",
  }));
  ...
  const deps: SessionRuntimeDeps = {
    ...
    getEffectiveExecutionMode: () =>
      task.agentConfig?.executionMode ?? "execute",
...
describe("SessionRuntime", () => {
  ...
  it("preserves visible partial output for cutoff continuation instead of replaying the request", async () => {
    ...
    try {
      ...
      expect(harness.createMessageWithTimeout.mock.calls[0][0].maxTokens).toBe(
        8_000,
      );
      ...
      expect(result.outputBudget.continuationBudget).toBe(16_000);
  ...
  it("keeps one bounded recovery available after thinking-only truncation", async () => {
    const previousPolicy = process.env.NEOWORKER_LLM_OUTPUT_POLICY;
    try {
      process.env.NEOWORKER_LLM_OUTPUT_POLICY = "adaptive";
      const harness = createHarness();
      ...
      expect(result.response.stopReason).toBe("max_tokens");
      expect(result.outputBudget.escalationAttempted).toBe(false);
      expect(result.outputBudget.truncationClassification).toBe(
        "reasoning_exhausted",
      );
      expect(result.outputBudget.continuationAllowed).toBe(true);
      expect(result.outputBudget.continuationBudget).toBe(8_000);
      expect(result.outputBudget.guidanceMessage).toBeUndefined();
    } finally {
      if (previousPolicy == null) {
        delete process.env.NEOWORKER_LLM_OUTPUT_POLICY;
      } else {
  ...
  it("refreshes base tool discovery and still invalidates on catalog or workspace changes", () => {
    ...
    expect(updatedTools.map((tool: Any) => tool.name)).toEqual([
      "browser_navigate",
    ]);
  });
  ...
  it("restores a legacy snapshot payload and backfills usage totals from llm_usage events", () => {
    ...
    const restoredFirstMessage =
      harness.runtime.getOutputState().conversationHistory[0];
    expect(typeof restoredFirstMessage?.content).toBe("string");
    expect(String(restoredFirstMessage?.content || "")).toContain(
  ...
  it("prefers a V2 snapshot over a legacy checkpoint payload when restoring", () => {
    ...
    expect(
      harness.runtime.getOutputState().conversationHistory[0]?.content,
    ).toBe("v2 snapshot");
  });
  ...
  it("owns verification and worker session-local state", () => {
    ...
    expect(
      harness.runtime.getVerificationState().verificationEvidenceEntries,
    ).toHaveLength(1);
    expect(
      harness.runtime
        .getVerificationState()
        .blockingVerificationFailedStepIds.has("step-1"),
    ).toBe(true);
    expect(
      harness.runtime.getVerificationState().dispatchedMentionedAgents,
    ).toBe(true);
    expect(
      harness.runtime.getVerificationState().verificationAgentState,
    ...
    expect(
      harness.runtime.getVerificationState().blockingVerificationFailedStepIds
        .size,
    ).toBe(0);
    expect(
      harness.runtime.getVerificationState().dispatchedMentionedAgents,
  ...
  it("creates, updates, and lists a session checklist while preserving ids", () => {
    ...
    expect(harness.runtime.getTaskListState().items[1]?.status).toBe(
      "in_progress",
    );
  ...
  it("rejects invalid checklist mutations", () => {
    ...
    expect(() => harness.runtime.createTaskList([])).toThrow(
      /at least one item/i,
    );
  ...
  it("restores checklist state from snapshot payloads and checklist events", () => {
    ...
    expect(harness.runtime.getTaskListState().items[0]?.title).toBe(
      "Recovered",
    );
    ...
    expect(harness.runtime.getTaskListState().items[0]?.kind).toBe(
      "verification",
    );
  });
  ...
  it("reuses rendered available tools when the render context is stable and invalidates on context change", () => {
    ...
    const renderToolsForContext = vi.fn((tools: Any[]) =>
      tools.map((tool) => ({
        ...tool,
        description: `${tool.description} rendered`,
      })),
    );
    harness.setToolRegistry({
      getTools: vi.fn(() => [
  ...
  it("triggers and clears the verification nudge under the expected conditions", () => {
    ...
    expect(
      harness.emittedEvents.some(
        (event) => event.type === "task_list_verification_nudged",
      ),
    ).toBe(true);
  ...
  it("suppresses the verification nudge when plan verification or verified mode already covers it", () => {
    ...
    harness.setPlan({
      description: "Plan",
      steps: [
        { id: "verify", description: "Verify: run tests", status: "pending" },
      ],
    });
