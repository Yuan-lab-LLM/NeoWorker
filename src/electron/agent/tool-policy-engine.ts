...
// Chat is a deliberately small, read-only product surface. Keep this as an
// allow-list rather than relying on mutation-name heuristics: an unfamiliar
// read-only tool can still expose workspace, memory, browser, or integration
// capabilities that Chat does not promise to use.
...
function applyModeGate(toolName: string, mode: ExecutionMode): string | null {
  if (mode === "chat" && !CHAT_MODE_ALLOWED_TOOLS.has(toolName)) {
    return `Tool "${toolName}" is blocked in chat mode because chat mode only allows web lookup and read-only attachment tools.`;
  }
