...
export class ToolRegistry {
  ...
  setActiveVisionRouteResolver(
    ...
    this.visionTools.setActiveVisionRouteResolver(resolver);
  }
  ...
  getTools(): LLMTool[] {
    ...
    // Repository download uses NeoWorker's bundled JavaScript Git runtime, so
    // it remains available without shell access or system developer tools.
    allTools.push(...GitTools.getCloneToolDefinitions());
  ...
  private getApprovalTypeForTool(toolName: string, input?: Any): ApprovalType | null {
    ...
    if (canonicalToolName === "web_fetch" || canonicalToolName === "git_clone") {
  ...
  private registerRuntimeHandlers(): void {
    ...
    register("git_clone", async ({ request }) => this.gitTools.gitClone(request.input));
...
Repository Download:
- git_clone: Download a repository with NeoWorker's built-in Git runtime. Always use this instead of shell git clone; it does not require system Git or Xcode Command Line Tools.`;
...
${skillDescriptions}`;
  ...
  async executeTool(name: string, input: Any, _runtime?: Record<string, unknown>): Promise<Any> {
    ...
    if (name === "git_clone") return await this.gitTools.gitClone(input);
      ...
      },
      {
        ...
        input_schema: {
          ...
          properties: {
            ...
            title: {
              type: "string",
              description: "Optional report title. When omitted, the first heading or snapshot title is used.",
            },
            subtitle: {
              type: "string",
              description: "Optional report subtitle shown on the professional cover.",
            },
            author: {
              type: "string",
              description: "Optional author or analyst name shown in metadata and on the cover.",
            },
            organization: {
              type: "string",
              description: "Optional organization or department shown on the cover and in the running header.",
            },
            reportDate: {
              type: "string",
              description: "Optional display date for the cover, for example 2026年8月23日.",
            },
            subject: {
              type: "string",
              description: "Optional document subject or one-line purpose statement.",
            },
            officeProfile: {
              type: "string",
