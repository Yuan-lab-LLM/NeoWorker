...
const TOOL_PROMPT_METADATA_BY_NAME: Record<string, LLMToolPromptMetadata> = {
  ...
  git_clone: createPromptMetadata(() => ({
    appendDescription:
      "Use this for repository downloads instead of run_command with `git clone`. It uses NeoWorker's built-in Git runtime and works even when system Git or Xcode Command Line Tools are unavailable. For public GitHub repositories it automatically falls back to a source archive.",
    compactDescription:
      "Download repositories with NeoWorker's built-in Git; always prefer over shell `git clone`.",
  })),
