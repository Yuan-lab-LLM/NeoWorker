...
type LegacyVisionProvider =
  "azure" | "openai" | "anthropic" | "bedrock" | "deepseek";

type VisionProvider = LLMProviderType;

export interface ActiveVisionRoute {
...
const VISION_REFUSAL_PATTERNS = [
  ...
  /(?:没有|未)(?:接收|收到|检测到).{0,20}(?:图片|图像|照片|图像数据)/i,
  /(?:不具备|不支持|缺少).{0,24}(?:图片|图像|视觉).{0,16}(?:能力|理解|输入|分析)?/i,
  /(?:cannot|can't|unable to)\s+(?:see|view|read|process|access|analy[sz]e).{0,60}(?:image|photo|visual)/i,
  /(?:no|not)\s+(?:image|photo|visual).{0,30}(?:received|provided|attached|available)/i,
  /(?:lack|do not have|don't have).{0,40}(?:ability|capability).{0,30}(?:image|photo|visual)/i,
  /does not support.{0,30}(?:image|photo|visual)/i,
];

export function isVisionRefusalResponse(text: string): boolean {
  const normalized = String(text || "")
    .replace(/\s+/g, " ")
    .trim();
  return (
    normalized.length > 0 &&
    VISION_REFUSAL_PATTERNS.some((pattern) => pattern.test(normalized))
  );
}
...
function buildSetupHint(provider: LegacyVisionProvider): {
  type: string;
  label: string;
  target: string;
} {
...
export class VisionTools {
  ...
  private activeVisionRouteResolver?: () => ActiveVisionRoute | null;
  ...
  /**
   * Route visual tool calls through the provider/model already selected for
   * this task. This is what lets OpenAI-compatible gateways and custom Kimi,
   * GLM, MiniMax, Qwen-VL, and aliased multimodal models analyze images.
   */
  setActiveVisionRouteResolver(
    resolver: (() => ActiveVisionRoute | null) | undefined,
  ): void {
    this.activeVisionRouteResolver = resolver;
  }
  ...
  > {
    ...
    const workspacePath = safeResolveWithinWorkspace(
      this.workspace.path,
      relPath,
    );
    if (!workspacePath) {
    ...
    try {
      ...
      const restoredPath =
        restoreTaskAttachmentSync({
          ...restoreOptions,
          taskId: this.taskId,
        }) || restoreTempWorkspaceArtifactSync(restoreOptions);
      if (!restoredPath) {
        return {
          success: false,
          error:
            `Attachment is no longer available: ${relPath}. ` +
            "Do not retry analyze_image; ask the user to upload the image again.",
          code: "ATTACHMENT_UNAVAILABLE",
          kind: "attachment_unavailable",
          display: "附件已失效，请重新上传图片后再试。",
          retryable: false,
          nonRetryable: true,
          actionHint: {
            type: "reupload_attachment",
            label: "重新上传图片",
            target: relPath,
          },
        };
      }
      absPath = restoredPath;
      try {
        stat = await fs.stat(absPath);
      } catch {
        return {
          success: false,
          error:
            `Attachment restore failed: ${relPath}. ` +
            "Do not retry analyze_image; ask the user to upload the image again.",
          code: "ATTACHMENT_RESTORE_FAILED",
          kind: "attachment_unavailable",
          display: "附件恢复失败，请重新上传图片后再试。",
          retryable: false,
          nonRetryable: true,
          actionHint: {
            type: "reupload_attachment",
  ...
  > {
    ...
    // The task's selected model is authoritative. Provider/model fields can
    // still arrive from an older tool schema or a hallucinated tool call, but
    // they must never redirect a Kimi/GLM/MiniMax/Qwen-VL task to an unrelated
    // provider. Explicit overrides remain a legacy fallback only when the
    // active task model cannot accept images.
    if (this.activeVisionRouteResolver) {
      const activeRoute = this.activeVisionRouteResolver();
      if (activeRoute?.provider && activeRoute.modelId) {
        activeRouteResolved = true;
        const activeModel = activeRoute.modelId;
        const activeCaps = LLMProviderFactory.getImageCapabilities(
          activeRoute.provider.type,
          activeModel,
        );
        if (activeCaps.supportsImages) {
          try {
            const supportedMimeTypes = new Set<LLMImageMimeType>([
              "image/jpeg",
              "image/png",
              "image/gif",
              "image/webp",
            ]);
            if (!supportedMimeTypes.has(mimeType as LLMImageMimeType)) {
              throw new Error(
                `Image type "${mimeType}" is not supported by the active model.`,
              );
            }

            // Reasoning-heavy multimodal models can consume a very small
            // vision budget entirely in hidden reasoning and return no final
            ...
            const response = await activeRoute.provider.createMessage({
              ...
              system:
                "Analyze the attached image carefully and answer the user's question using only visible evidence. Return a concise final answer; do not stop after internal reasoning.",
              messages: [
                {
                  role: "user",
                  content: [
                    { type: "text", text: prompt },
                    {
                      type: "image",
                      data: base64,
                      mimeType: mimeType as LLMImageMimeType,
                    },
                  ],
                },
              ],
            ...
            if (!text) {
              throw new Error(
                `Active vision model "${activeModel}" returned no text.`,
              );
            }
            if (isVisionRefusalResponse(text)) {
              throw new Error(
                `Active model "${activeModel}" returned a vision-refusal response even though an image payload was sent.`,
              );
            }
            this.daemon.logEvent(this.taskId, "tool_result", {
              tool: toolName,
              success: true,
              provider: activeRoute.provider.type,
              model: activeModel,
              route: "active_task_model",
              ignoredProviderOverride: providerOverride || undefined,
              ignoredModelOverride: modelOverride || undefined,
            });
            return {
              success: true,
              provider: activeRoute.provider.type,
            ...
            activeRouteError = error;
          }
        }

        // A custom OpenAI-compatible endpoint can expose both text-only and
        // multimodal models. If the selected model refuses an image that was
        // actually sent, try another *user-configured* model on that same
        // endpoint whose ID is a high-confidence visual family (Kimi K2.6,
        ...
        if (activeRouteError) {
          ...
          try {
            configuredModels =
              LLMProviderFactory.getSelectableProviderModelStatus(
                settings,
                activeRoute.provider.type,
              ).models;
          } catch (error) {
            logger.warn(
              `[VisionTools] Failed to resolve configured visual fallbacks for ${activeRoute.provider.type}: ${
                error instanceof Error ? error.message : String(error)
              }`,
            );
          }
          const fallbackModelIds = configuredModels
            ...
            .filter(
              (modelId, index, all) =>
                ...
                isHighConfidenceVisionModelId(modelId),
            );
          ...
          for (const fallbackModel of fallbackModelIds) {
            try {
              ...
              if (!text) {
                throw new Error(
                  `Configured vision fallback "${fallbackModel}" returned no text.`,
                );
              }
              if (isVisionRefusalResponse(text)) {
                throw new Error(
                  `Configured vision fallback "${fallbackModel}" also refused the image payload.`,
                );
              }
    ...
    const hasDirectOpenAIVisionConfig = Boolean(
      settings.openai?.apiKey?.trim(),
    );
    const hasOpenAIOAuthVisionConfig = Boolean(
      settings.openai?.accessToken?.trim() &&
      settings.openai?.refreshToken?.trim(),
    );
    const hasOpenAIVisionConfig =
      hasDirectOpenAIVisionConfig || hasOpenAIOAuthVisionConfig;
    const normalizedProviderOverride =
      ...
      !hasOpenAIVisionConfig
        ? "azure"
        : providerOverride;
    const preferred: LegacyVisionProvider | undefined =
      ...
      normalizedProviderOverride === "deepseek"
        ? (normalizedProviderOverride as LegacyVisionProvider)
        : undefined;

    const tryOrder: LegacyVisionProvider[] = activeRouteResolved
      ? []
      : preferred
        ? [preferred]
        : (() => {
            const order: LegacyVisionProvider[] = [];
    ...
    let lastError =
      activeRouteError instanceof Error
        ? activeRouteError.message
        : activeRouteError
          ? String(activeRouteError)
          : undefined;
    let lastErrorRaw: unknown = activeRouteError;
    ...
    for (const provider of tryOrder) {
      try {
        ...
        if (provider === "openai") {
          ...
          const useOAuth =
            settings.openai?.authMethod === "oauth" && canUseOAuth;
          const model =
            modelOverride ||
            settings.openai?.model ||
            (useOAuth ? "gpt-5.5" : "gpt-4o-mini");
          let text: string;
          if (useOAuth) {
    ...
    const actionHint = fallbackProvider
      ? buildSetupHint(fallbackProvider)
      : undefined;
    ...
    const emptyModelOutput =
      /returned no text|returned an empty response|reasoning(?:-only| only).*response/i.test(
        fallbackError,
      );
    const recoverableFailure = configurationMissing || emptyModelOutput;
    ...
    return {
      ...
      error: fallbackError,
      retryable,
      nonBlocking: recoverableFailure,
      recoverableFallback: recoverableFailure,
      fallbackHint: recoverableFailure
        ? emptyModelOutput
          ? "The selected vision model produced no final text. Continue from the image already attached to the conversation, or use web_search when product identification needs external confirmation."
  ...
  private async analyzeWithOpenAIOAuth(args: {
    ...
    const provider = new OpenAIProvider({
      ...
      openaiOAuthTokenUpdater: (tokens) =>
        this.persistOpenAIOAuthTokens(tokens),
    });
  ...
  private async analyzeWithAzureOpenAI(args: {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Azure OpenAI vision error ${response.status}: ${errorBody}`,
      );
    }
