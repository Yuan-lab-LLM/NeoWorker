...
describe("ToolRegistry tool catalog versioning", () => {
  ...
  it("exposes built-in repository cloning even when shell access is disabled", () => {
    const workspace = createWorkspace();
    workspace.permissions.shell = false;
    const registry = new ToolRegistry(workspace, createDaemon(), "task-built-in-git");

    const tool = registry.getTools().find((entry) => entry.name === "git_clone");
    expect(tool).toBeDefined();
    expect(tool?.description).toContain("built-in Git runtime");
    expect(tool?.runtime?.readOnly).toBe(false);
    expect(tool?.runtime?.concurrencyClass).toBe("exclusive");
    expect((registry as Any).getApprovalTypeForTool("git_clone")).toBe(
      "network_access",
    );
  });
