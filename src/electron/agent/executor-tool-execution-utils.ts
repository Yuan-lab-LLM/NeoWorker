    ...
    toolResult: {
      ...
      content: resultIsError && !advisoryFallbackFailure
        ? JSON.stringify({
            ...
            ...(typeof opts.result?.retryable === "boolean"
              ? { retryable: opts.result.retryable }
              : {}),
            ...(typeof opts.result?.nonRetryable === "boolean"
              ? { nonRetryable: opts.result.nonRetryable }
              : {}),
            ...(opts.result?.actionHint
              ? { actionHint: opts.result.actionHint }
              : {}),
...
export function isHardToolFailure(toolName: string, result: Any, failureReason = ""): boolean {
  ...
  if (result.retryable === false || result.nonRetryable === true) {
    return true;
  }
