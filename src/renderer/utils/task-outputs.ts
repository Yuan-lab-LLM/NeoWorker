...
function isProvisionalArtifactEvent(event: TaskEvent): boolean {
  return (
    String(event.payload?.source || "").trim().toLowerCase() ===
      "artifact_bootstrap" || event.payload?.provisional === true
  );
}
...
export function deriveTaskOutputSummaryFromEvents(
  ...
  for (const event of events) {
    if (isProvisionalArtifactEvent(event)) continue;
...
export function resolveTaskOutputSummaryFromCompletionEvent(
  ...
  // Persisted timeline-v2 completions use a `timeline_step_finished` envelope
  // with legacyType=task_completed. Checking the raw type made their verified
