...
describe("localized skill composer prompts", () => {
  ...
  it("localizes the complete Manim parameter form while preserving option values", () => {
    const skill = loadBundledJsonSkills().find(
      (candidate) => candidate.id === "manim-video",
    );
    expect(skill).toBeDefined();
    ...
    const localized = Object.fromEntries(
      (skill!.parameters || []).map((parameter) => [
        parameter.name,
        getLocalizedSkillParameterText(skill!, parameter, "zh-CN"),
      ]),
    );

    expect(localized.mode).toMatchObject({
      name: "模式",
      description: "选择要搭建的视频讲解结构。",
    });
    expect(localized.mode.options).toEqual([
      "自动选择",
      "概念讲解",
      "公式推导",
      "算法可视化",
    ...
    expect(localized.audience.name).toBe("目标受众");
    expect(localized.target_length_seconds.name).toBe("目标时长（秒）");
    expect(localized.voiceover.options).toEqual([
      "自动选择",
      "生成旁白",
      "不生成旁白",
    ]);
