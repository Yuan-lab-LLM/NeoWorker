...
export function UnifiedTaskComposer({
  ...
  submitLabel = translate(
    "generated.components.unifiedtaskcomposer.71.0",
    "send",
  ),
  autoFocus = false,
  showLabel = true,
  ...
  const [stage, setStage] = useState<
    "idle" | "selecting" | "importing" | "reading" | "sending"
  >("idle");
  ...
  useEffect(() => {
    ...
    if (value.length > 0)
      inlineDraftCache.set(activeCacheKeyRef.current, value);
  ...
  const attachFiles = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to add inline composer attachments:",
        attachmentError,
      );
      setError(
        translate(
  ...
  const submit = async () => {
    ...
    try {
      ...
      const composed = await composeMessageWithAttachments(
        workspace.path,
        draft,
        imported,
      );
  ...
  const stageLabel =
    stage === "selecting"
      ? translate(
          "generated.components.unifiedtaskcomposer.242.6",
          "Select attachment…",
        )
      : stage === "importing"
        ? translate(
            "generated.components.unifiedtaskcomposer.244.7",
            "Import attachments…",
          )
        : stage === "reading"
          ? translate(
              "generated.components.unifiedtaskcomposer.246.8",
              "Parse attachment…",
            )
          : stage === "sending"
            ? translate(
                "generated.components.unifiedtaskcomposer.248.9",
                "Submitting…",
              )
            : null;
  ...
  return (
    <section className="unified-task-composer" aria-label={label}>
      <label className={showLabel ? "unified-task-composer-label" : "sr-only"}>
        {label}
      </label>
        ...
        <div className="unified-task-composer-toolbar">
          <div className="unified-task-composer-toolbar-primary">
            <button
              ...
              title={translate(
                "generated.components.unifiedtaskcomposer.298.12",
                "Upload files",
              )}
            >
            ...
            <div
              className="unified-task-composer-permission"
            ...
            >
              ...
              >
                ...
                <span>
                  {permissionMode === "bypass_permissions"
                    ? translate(
                        "generated.components.unifiedtaskcomposer.316.14",
                        "full access",
                      )
                    : translate(
              ...
              {permissionMenuOpen ? (
                <div
                  className="unified-task-composer-permission-menu"
                  role="menu"
                >
                  <button
                    type="button"
                    className={
                      permissionMode !== "bypass_permissions" ? "selected" : ""
                    }
                    onClick={() => {
            ...
            {onChangeWorkspace ? (
              <button
                ...
                title={
                  workspace?.path ||
                  translate(
                    "generated.components.unifiedtaskcomposer.355.20",
                    "Select workspace",
                  )
                }
              >
          ...
          <div className="unified-task-composer-toolbar-secondary">
            ...
            <button
              ...
              onClick={() => void submit()}
              disabled={
                stage !== "idle" || (!value.trim() && attachments.length === 0)
              }
