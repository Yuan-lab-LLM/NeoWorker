...
describe("ActionBlock V2 projection", () => {
  ...
  it("explains non-blocking action errors without calling the whole task unfinished", () => {
    ...
    expect(recoveringHtml).toContain("部分工具操作未成功，任务仍在继续");
    expect(recoveringHtml).toContain("本组记录了 1 个工具或内部操作错误");
    expect(recoveringHtml).toContain("不代表整个任务未完成");
    ...
    expect(historicalHtml).toContain("本轮已结束，有操作未成功");
    expect(historicalHtml).toContain("部分工具操作未成功，本轮已经结束");
    ...
    expect(completedHtml).toContain("本轮已结束，有操作未成功");
    expect(completedHtml).toContain("部分工具操作未成功，本轮已经结束");
  });
