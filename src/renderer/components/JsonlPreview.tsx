...
export function JsonlPreview({
  ...
  return (
    <div
      className="jsonl-preview"
      role="region"
      aria-label={t("jsonlPreview.aria", "JSONL preview")}
    >
      <div className="jsonl-preview-header">
        <div className="jsonl-preview-title">
          <span className="jsonl-preview-kicker">JSONL</span>
          <span>
            {t("jsonlPreview.recordCount", "{count} records", {
              count: records.length,
            })}
          </span>
        </div>
        {truncated ? (
          <span className="jsonl-preview-note">
            {t("jsonlPreview.truncated", "Preview truncated")}
          </span>
        ) : null}
      </div>
        ...
        {records.map((record) => {
          ...
          const details = objectRecord
            ? Object.entries(objectRecord).filter(
                ([key]) => !SUMMARY_KEYS.has(key.toLowerCase()),
              )
            : [];
            ...
            <div className="jsonl-preview-row" key={record.lineNumber}>
              ...
              <div className="jsonl-preview-row-body">
                <div className="jsonl-preview-meta">
                  {level ? (
                    <span
                      className="jsonl-preview-level"
                      data-tone={getLevelTone(level)}
                    >
                      {level}
                    </span>
                  ) : null}
                  {timestamp ? (
                    <span className="jsonl-preview-time">{timestamp}</span>
                  ) : null}
                  {component ? (
                    <span className="jsonl-preview-component">{component}</span>
                  ) : null}
                </div>
                  ...
                  <div className="jsonl-preview-fields">
                    ...
                    {details.length > 6 ? (
                      <span className="jsonl-preview-field jsonl-preview-field-more">
                        {t("jsonlPreview.moreFields", "+{count} more", {
                          count: details.length - 6,
                        })}
                      </span>
                    ) : null}
                  </div>
