...
export type PromptComposerInputHandle = {
  ...
  getValue: () => string;
  getSnapshot: () => PromptComposerInputSnapshot;
  resize: (shrink?: boolean) => void;
};

export type PromptComposerInputSnapshot = {
  value: string;
...
export function replaceComposerDraftText(
  snapshot: PromptComposerInputSnapshot,
  start: number,
  end: number,
  replacement: string,
): PromptComposerInputSnapshot & { cursor: number } {
  const baseValue = snapshot.value;
  const baseMentions = sortedValidMentions(baseValue, snapshot.mentions);
  const safeStart = Math.max(0, Math.min(start, baseValue.length));
  const safeEnd = Math.max(safeStart, Math.min(end, baseValue.length));
  const protectedSpans = [
    ...baseMentions,
    ...parseMarkdownLinks(baseValue, baseMentions),
  ];
  const expandedStart = Math.min(
    safeStart,
    ...protectedSpans
      .filter((span) => span.start < safeEnd && span.end > safeStart)
      .map((span) => span.start),
  );
...
>(function PromptComposerInput(
  ...
  const hasPendingProgrammaticEditRef = useRef(false);
  if (!hasPendingProgrammaticEditRef.current) {
  ...
  const reportDraftPresence = useCallback(
    (nextValue: string) => {
      ...
      // Keep a synchronous DOM signal for CSS. React state can trail a native
      // contenteditable/IME mutation by one frame, which previously left the
      // send arrow looking disabled even though clicking it already worked.
  ...
  const latestMentionsRef = useRef(validMentions);
  if (!hasPendingProgrammaticEditRef.current) {
  ...
  const getLatestSnapshot = useCallback((): PromptComposerInputSnapshot => {
    if (hasPendingProgrammaticEditRef.current) {
      return {
        value: latestValueRef.current,
        mentions: latestMentionsRef.current,
      };
    }
    ...
    latestMentionsRef.current = snapshot.mentions;
    return { value: snapshot.value, mentions: snapshot.mentions };
  }, [mentionsById]);
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      getSelectionStart: () =>
        hasPendingProgrammaticEditRef.current && pendingSelectionRef.current
          ? pendingSelectionRef.current.start
          : rootRef.current
          ? getSelectionIndex(rootRef.current, mentionsById)
          : value.length,
      // Paste/beforeinput edits update this atomic snapshot synchronously,
      // before React commits. Submission must consume the snapshot rather than
      // mixing a fresh DOM value with stale render-time mention state.
      getValue: () => getLatestSnapshot().value,
      getSnapshot: getLatestSnapshot,
      resize,
    }),
    [applySelection, getLatestSnapshot, mentionsById, resize, value.length],
  );
  ...
  const applyTextReplacement = useCallback(
    (start: number, end: number, replacement: string) => {
      ...
      const next = replaceComposerDraftText(
        {
          value: baseValue,
          mentions: latestMentionsRef.current,
        },
      ...
      latestMentionsRef.current = next.mentions;
      ...
      onChange(
        ...
        next.cursor,
        next.mentions,
        next.value.length < baseValue.length,
      );
    },
    [onChange, reportDraftPresence],
  );
    ...
    (direction: "backward" | "forward") => {
      ...
      if (start === end) {
        if (direction === "backward") {
          ...
          if (end >= latestValueRef.current.length) return;
          end += 1;
        }
      }
  ...
  const handleCopy = (event: ReactClipboardEvent<HTMLDivElement>) => {
    ...
    event.clipboardData.setData(
      "text/plain",
      latestValueRef.current.slice(range.start, range.end),
    );
    event.preventDefault();
  };
