...
type BuilderStage = "idle" | "designing" | "choosing" | "creating" | "created";
...
type AgentDirectoryFilter =
  "all" | "build" | "insight" | "content" | "coordination";
...
const AGENT_DIRECTORY_FILTERS: Array<{
...
}> = [
  {
    id: "all",
    label: translate(
      "generated.components.simpleagentbuilderpanel.54.0",
      "All",
    ),
    capabilities: [],
  },
  ...
  },
  {
    id: "content",
    label: translate(
      "generated.components.simpleagentbuilderpanel.67.3",
      "Content and growth",
    ),
...
const AGENT_SUGGESTIONS = [
  {
    ...
    icon: MessageSquare,
    title: translate(
      "generated.components.simpleagentbuilderpanel.86.5",
      "Team Chat Q&A",
    ),
...
const AGENT_CREATION_STEPS = [
  ...
  translate(
    "generated.components.simpleagentbuilderpanel.106.15",
    "Matching ability",
  ),
  translate(
...
export function applySimpleBuilderName(
  ...
  const name =
    ...
    (isGenericAgentName(currentName)
      ? deriveAgentNameFromPrompt(
          prompt,
          translate(
            "generated.components.simpleagentbuilderpanel.130.17",
            "new agent",
          ),
        )
      : currentName);
  ...
  const replaceName = (value: string): string =>
    currentName && currentName !== name
      ? value.split(currentName).join(name)
      : value;
...
export function getSimpleAgentDirectoryFilter(
  ...
  const primaryMatch = AGENT_DIRECTORY_FILTERS.find(
    (filter) =>
      filter.id !== "all" &&
      primaryCapability &&
      filter.capabilities.includes(primaryCapability),
  );
  ...
  const fallbackMatch = AGENT_DIRECTORY_FILTERS.find(
    (filter) =>
      filter.id !== "all" &&
      filter.capabilities.some((capability) => capabilities.has(capability)),
  );
...
export function filterSimpleAgentDirectory(
  ...
  return roles.filter((role) => {
    if (filter !== "all" && getSimpleAgentDirectoryFilter(role) !== filter)
      return false;
    ...
    const searchable = [
      ...
      ...(role.capabilities || []).map((capability) =>
        getLocalizedAgentCapability(capability),
      ),
    ]
      .filter(Boolean)
      .join(" ")
...
export function getLocalizedSimpleBuilderRequirement(
  ...
  if (requirement.kind === "skill") {
    return {
      title: translate(
        "generated.components.simpleagentbuilderpanel.286.18",
        "Choose a skill",
      ),
      reason: translate(
  ...
  return {
    title: translate(
      "generated.components.simpleagentbuilderpanel.297.22",
      "Choose a tool",
    ),
    reason: translate(
...
export function SimpleAgentBuilderPanel({
  ...
  const [directoryFilter, setDirectoryFilter] =
    useState<AgentDirectoryFilter>("all");
  const [agentName, setAgentName] = useState("");
  const [prompt, setPrompt] = useState("");
  const [plan, setPlan] = useState<AgentBuilderPlan | null>(null);
  const [stage, setStage] = useState<BuilderStage>("idle");
  ...
  const requiredSelections = useMemo(
    () => getRequiredSimpleBuilderSelections(plan),
    [plan],
  );
  ...
  const loadOverview = useCallback(async () => {
    try {
      ...
      const [managedAgents, availableWorkspaces, availableRoles] =
        await Promise.all([
      ...
      const routines = await Promise.all(
        agents.map((agent) =>
          window.electronAPI.listManagedAgentRoutines(agent.id),
        ),
      );
      const scheduled = routines.reduce(
        (count, entries) =>
          count + entries.filter((routine) => routine.enabled).length,
        0,
      );
    ...
    try {
      ...
      setError(
        createError instanceof Error
          ? createError.message
          : t(
              "agents.simple.createError",
              "Agent creation failed, please try again.",
            ),
      );
      setStage(
        getRequiredSimpleBuilderSelections(resolvedPlan).length > 0
          ? "choosing"
          : "idle",
      );
    }
  ...
  const handleCreate = async () => {
    ...
    if (!normalizedPrompt || stage === "designing" || stage === "creating")
      return;
    ...
    try {
      ...
      const resolvedPlan = applySimpleBuilderName(
        generatedPlan,
        agentName,
        normalizedPrompt,
      );
  ...
  return (
    <main className="simple-agent-builder">
      ...
      <section
        className="simple-agent-page-header"
        aria-labelledby="simple-agent-page-title"
      >
        <div>
          <h1 id="simple-agent-page-title">
            {translate(
              "generated.components.simpleagentbuilderpanel.502.25",
              "Agent team",
            )}
          </h1>
          <p>
        ...
        <div className="simple-agent-page-actions">
          <span className="simple-agent-page-stat">
            ...
            {translate(
              "generated.components.simpleagentbuilderpanel.509.27",
              "agent",
            )}
          </span>
          {overview.scheduled > 0 ? (
            <span className="simple-agent-page-stat">
              ...
              {translate(
                "generated.components.simpleagentbuilderpanel.515.28",
                "plan",
              )}
            </span>
          ) : null}
          ...
          >
            ...
            {translate(
              "generated.components.simpleagentbuilderpanel.525.29",
              "Browse agents",
            )}
          </button>
        </div>
      ...
      <section
        className="simple-agent-create"
        aria-labelledby="simple-agent-create-title"
      >
        ...
        <label className="simple-agent-name-field" htmlFor="simple-agent-name">
          ...
          <span>
            {translate(
              "generated.components.simpleagentbuilderpanel.543.32",
              "Name",
            )}
          </span>
          <input
        ...
        >
          ...
          <button
            ...
            disabled={
              !prompt.trim() || stage === "designing" || stage === "creating"
            }
          >
        ...
        {plan && stage === "choosing" ? (
          <div className="simple-agent-choices">
            ...
            {requiredSelections.map((requirement) => {
              const requirementCopy = getLocalizedSimpleBuilderRequirement(
                requirement,
                language,
              );
              return (
                <fieldset key={requirement.id}>
                  ...
                  {requirementCopy.reason ? (
                    <p>{requirementCopy.reason}</p>
                  ) : null}
                  <div>
                    {requirement.options.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            handleSelection(requirement, option.id)
                          }
                        >
            ...
            >
              {translate(
                "generated.components.simpleagentbuilderpanel.683.43",
                "Create an agent",
              )}
            </button>
          </div>
        ...
        {stage === "created" ? (
          <div className="simple-agent-success" role="status">
            ...
            <button type="button" onClick={resetBuilder}>
              {translate(
                "generated.components.simpleagentbuilderpanel.696.46",
                "Create another",
              )}
            </button>
          </div>
        ...
        <div className="simple-agent-directory-heading">
          <div>
            <h2 id="simple-agent-directory-title">
              {translate(
                "generated.components.simpleagentbuilderpanel.735.47",
                "Existing agents",
              )}
            </h2>
            <p>
          ...
          <span>
            {roles.length}{" "}
            {translate(
              "generated.components.simpleagentbuilderpanel.738.49",
              "agent",
            )}
          </span>
        </div>
        ...
        <div className="simple-agent-directory-toolbar">
              ...
              const count =
                filter.id === "all"
                  ...
                  : queryMatchedRoles.filter(
                      (role) =>
                        getSimpleAgentDirectoryFilter(role) === filter.id,
                    ).length;
          ...
          <label className="simple-agent-directory-search">
            ...
            <span className="sr-only">
              {translate(
                "generated.components.simpleagentbuilderpanel.767.51",
                "Search agent",
              )}
            </span>
            <input
        ...
        {directoryLoading ? (
          ...
          >
            {Array.from({ length: 8 }, (_, index) => (
              <div
                key={index}
                className="simple-agent-card simple-agent-card-skeleton"
              />
            ))}
          </div>
        ) : visibleRoles.length ? (
          <div className="simple-agent-directory-grid">
            {visibleRoles.map((role) => {
              ...
              const linkedSkills = getAgentRoleLinkedSkillLabels(
                role,
                [],
                language,
              );
