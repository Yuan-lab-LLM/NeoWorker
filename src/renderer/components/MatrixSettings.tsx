...
export function MatrixSettings({ onStatusChange }: MatrixSettingsProps) {
  ...
  const loadChannel = useCallback(async () => {
    try {
      ...
      const matrixChannel = channels.find(
        (c: ChannelData) => c.type === "matrix",
      );
  ...
  const handleAddChannel = async () => {
    if (!homeserver.trim() || !userId.trim() || !accessToken.trim()) {
      setTestResult({
        success: false,
        error: t(
          "matrix.error.required",
          "Homeserver, User ID, and Access Token are required",
        ),
      });
      return;
    }
  ...
  const handleRemoveChannel = async () => {
    ...
    if (
      !confirm(
        t(
          "matrix.confirm.remove",
          "Are you sure you want to remove the Matrix channel?",
        ),
      )
    ) {
      return;
  ...
  if (loading) {
    return (
      <div className="settings-loading">
        {t("matrix.loading", "Loading Matrix settings...")}
      </div>
    );
  }
    ...
    return (
      <div className="matrix-settings">
        <div className="settings-section">
          <h3>{t("matrix.connect.title", "Connect Matrix")}</h3>
          ...
          <div className="settings-field">
            <label>{t("matrix.field.homeserver", "Homeserver URL")}</label>
            <input
              type="text"
              className="settings-input"
          ...
          <div className="settings-field">
            <label>{t("matrix.field.userId", "User ID")}</label>
            <input
              type="text"
              className="settings-input"
          ...
          <div className="settings-field">
            <label>{t("matrix.field.accessToken", "Access Token")}</label>
            <input
              type="password"
              className="settings-input"
          ...
          <div className="settings-field">
            <label>
              {t("matrix.field.deviceIdOptional", "Device ID (Optional)")}
            </label>
            <input
              type="text"
              className="settings-input"
              placeholder={t(
                "matrix.placeholder.deviceId",
          ...
          <div className="settings-field">
            <label>
              {t("matrix.field.roomIdsOptional", "Room IDs (Optional)")}
            </label>
            <input
              type="text"
              className="settings-input"
            ...
            <p className="settings-hint">
              ...
              {securityMode === "allowlist" &&
                t(
                  ...
                  "Only pre-approved Matrix user IDs can use the bot",
                )}
              {securityMode === "open" &&
                t(
        ...
        <div className="settings-section">
          ...
          <ol className="setup-instructions">
            <li>
              {t(
                "matrix.setup.openClient",
                "Open Element or your Matrix client",
              )}
            </li>
            <li>
              {t(
                "matrix.setup.goAdvanced",
                "Go to Settings > Help & About > Advanced",
              )}
            </li>
            <li>{t("matrix.setup.copyToken", "Copy your Access Token")}</li>
            <li>
              {t(
                "matrix.setup.findUserId",
                "Find your User ID in your profile settings",
              )}
            </li>
            <li>
              {t(
                "matrix.setup.enterCredentials",
                "Enter the homeserver URL and credentials above",
              )}
            </li>
          </ol>
        </div>
