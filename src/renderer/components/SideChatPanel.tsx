...
function isForkedParentTranscriptEvent(event: TaskEvent): boolean {
  ...
  return (
    typeof payload.forkedFromTaskId === "string" ||
    typeof payload.forkedFromEventId === "string"
  );
}
    ...
    return [
      {
        id:
          event.id ||
          event.eventId ||
          `${event.taskId}:${event.timestamp}:${type}`,
...
function getTaskStatusLabel(task?: Task | null): string {
  const t = translate;
  if (!task) return t("sideChat.status.unavailable", "Unavailable");
  switch (task.status) {
    case "executing":
    case "planning":
      return t("sideChat.status.running", "Running");
    case "paused":
      return t("sideChat.status.paused", "Paused");
    case "blocked":
    case "interrupted":
      return t("sideChat.status.needsInput", "Needs input");
    case "completed":
      return t("sideChat.status.completed", "Completed");
    case "failed":
      return t("sideChat.status.failed", "Failed");
    case "cancelled":
      return t("sideChat.status.cancelled", "Cancelled");
    case "pending":
    case "queued":
      return t("sideChat.status.queued", "Queued");
    default:
      return task.status;
  }
}
...
export const SideChatPanel = memo(function SideChatPanel({
  ...
  const headerStatus = sideTask
    ? sideStatus
    : loading
      ? t("sideChat.status.opening", "Opening")
      : t("sideChat.status.unavailable", "Unavailable");
  ...
  return (
    <aside
      ...
      aria-label={t("sideChat.aria", "Side conversation")}
    >
      <header className="side-chat-header">
        <div className="side-chat-title-row">
          <div className="side-chat-title">
            ...
            <div className="side-chat-title-copy">
              <span>{t("sideChat.title", "Side chat")}</span>
              {parentTask?.title ? (
                <span
                  className="side-chat-parent-title"
          ...
          <div className="side-chat-header-actions">
            {sideTask?.id && onOpenSideTask ? (
              <button
                ...
                title={t(
                  "sideChat.openFullThread",
                  "Open side conversation as a full thread",
                )}
                aria-label={t(
                  "sideChat.openFullThread",
                  "Open side conversation as a full thread",
            ...
            <button
              ...
              className="side-chat-icon-btn"
              onClick={onClose}
              title={t("sideChat.close", "Close side conversation")}
              aria-label={t("sideChat.close", "Close side conversation")}
            >
        ...
        <div className="side-chat-meta-row">
          <span
            className={`side-chat-status side-chat-status-${sideTask?.status || "unknown"}`}
          >
            {headerStatus}
          </span>
          ...
          >
            {t("sideChat.parentStatus", "Parent {status}", {
              status: parentStatus,
            })}
          </span>
        </div>
      ...
      <div className="side-chat-messages" ref={scrollRef}>
        {loading ? (
          <div className="side-chat-empty">
            ...
            <span>{t("sideChat.opening", "Opening side conversation...")}</span>
          </div>
        ) : messages.length === 0 ? (
          <div className="side-chat-empty">
            ...
            <span>
              {t(
                "sideChat.empty",
                "Ask about this session without changing the active task.",
              )}
            </span>
          </div>
        ) : (
          messages.map((message) => (
            ...
            >
              <div className="side-chat-message-role">
                {message.role === "user"
                  ? t("sideChat.role.you", "You")
                  : t("sideChat.role.side", "Side")}
              </div>
        ...
        {sending ? (
          <div className="side-chat-thinking">
            ...
            <span>{t("sideChat.thinking", "Thinking")}</span>
          </div>
        ) : null}
      </div>
      ...
      <footer className="side-chat-composer">
        <textarea
          ...
          placeholder={t("sideChat.placeholder", "Ask a side question")}
          aria-label={t("sideChat.placeholder", "Ask a side question")}
          rows={2}
          disabled={!sideTask}
        />
        <button
          ...
          onClick={submit}
          disabled={!canSend}
          title={t("sideChat.send", "Send side question")}
          aria-label={t("sideChat.send", "Send side question")}
        >
          {sending ? (
