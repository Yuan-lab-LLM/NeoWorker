...
function formatRelativeTime(timestamp?: number): string {
  if (!timestamp) return translate("devices.time.justNow", "Just now");
  const diff = Date.now() - timestamp;
  const minutes = Math.max(1, Math.round(diff / 60000));
  if (minutes < 60)
    return translate("devices.time.minutesAgo", "{count}m ago", {
      count: minutes,
    });
  const hours = Math.round(minutes / 60);
  if (hours < 24)
    return translate("devices.time.hoursAgo", "{count}h ago", { count: hours });
  const days = Math.round(hours / 24);
  if (days < 7)
    return translate("devices.time.daysAgo", "{count}d ago", { count: days });
  return new Date(timestamp).toLocaleDateString();
}

function formatBytes(value?: number): string {
  if (!value || !Number.isFinite(value) || value <= 0)
    return translate("devices.unknown", "Unknown");
...
function purposeLabel(purpose?: ManagedDevice["purpose"]): string {
  switch (purpose) {
    case "primary":
      return translate("devices.purpose.primary", "Primary");
    case "work":
      return translate("devices.purpose.work", "Work");
    case "personal":
      return translate("devices.purpose.personal", "Personal");
    case "automation":
      return translate("devices.purpose.automation", "Automation");
    case "archive":
      return translate("devices.purpose.archive", "Archive");
    case "general":
    default:
      return translate("devices.purpose.general", "General");
  }
}

function deviceConnectionLabel(device: ManagedDevice): string {
  if (device.role === "local")
    return translate("devices.status.local", "Local");
  switch (device.status) {
    case "connected":
      return translate("devices.status.connected", "Connected");
    case "connecting":
      return translate("devices.status.connecting", "Connecting");
    case "authenticating":
      return translate("devices.status.authenticating", "Authenticating");
    case "reconnecting":
      return translate("devices.status.reconnecting", "Reconnecting");
    case "error":
      return translate("devices.status.needsAttention", "Needs attention");
    default:
      return translate("devices.status.saved", "Saved");
  }
}

function deviceDisplayName(device?: ManagedDevice | null): string {
  if (!device) return translate("devices.selectDevice", "Select a device");
  return device.role === "local"
    ? translate("devices.thisDevice", "This device")
    : device.name;
}

function deviceAttentionLabel(
  level?: ManagedDevice["attentionState"],
): string | null {
  switch (level) {
    case "critical":
      return translate("devices.attention.critical", "Critical");
    case "warning":
      return translate("devices.attention.warning", "Attention");
    case "info":
      return translate("devices.attention.info", "Info");
    default:
      return null;
  }
}
...
function reconcileRemoteDeviceNamesFromSettings(
  ...
  return devices.map((device) => {
    ...
    const matchesUrl =
      normalizeGatewayUrl(device.config?.url) === normalizedRemoteUrl;
...
function toSavedRemoteDevice(
  device: ManagedDevice,
): SavedRemoteGatewayDevice | null {
...
export function DevicesPanel({
  ...
  const [overflowSubmenu, setOverflowSubmenu] = useState<
    "mode" | "domain" | null
  >(null);
  ...
  const loadSummaries = useCallback(async (managedDevices: ManagedDevice[]) => {
    ...
    const nextEntries = results.filter(
      (entry): entry is [string, ManagedDeviceSummary] => !!entry,
    );
    ...
    setDevices((current) =>
      current.map((device) => {
        const summary = nextEntries.find(
    ...
    try {
      ...
      setSelectedDeviceId((current) => {
        if (current && nextDevices.some((device) => device.id === current))
          return current;
        const preferredId =
          (settingsData as ControlPlaneSettingsData | null)
            ?.activeManagedDeviceId || null;
        if (
          preferredId &&
          nextDevices.some((device) => device.id === preferredId)
        ) {
          return preferredId;
        }
        return (
  ...
  useEffect(() => {
    const handleNavigate = (event: Event) => {
      ...
      const refMap: Record<
        string,
        { current: HTMLElement | HTMLDivElement | null }
      > = {
  ...
  const loadTaskFeed = useCallback(async () => {
    ...
    const targetDevices =
      taskFilter === "selected"
        ? activeDevice
          ? [activeDevice]
          : []
        : devices.filter((device) => !!device.taskNodeId);
    ...
    const results = await Promise.all(
      targetDevices.map(async (device) => {
        ...
        try {
          const result = await window.electronAPI?.deviceListTasks?.(
            device.taskNodeId,
          );
          return result?.ok && Array.isArray(result.tasks)
    ...
    const visible = sorted.filter((task) => {
      const taskNodeId =
        typeof task.targetNodeId === "string" ? task.targetNodeId : "";
      const taskDevice = devices.find(
        (device) => device.taskNodeId === taskNodeId,
      );
    ...
    setDeviceTasks(
      taskFilter === "attention"
        ? visible.filter((task) => isTaskAttention(task))
        : visible,
    );
  }, [activeDevice, devices, taskFilter]);
  ...
  const handleAttachFiles = useCallback(async () => {
    ...
    if (activeDevice.role === "local") {
      ...
      try {
        const res = await window.electronAPI?.deviceListRemoteWorkspaces?.(
  ...
  const taskCountLabel =
    taskFilter === "selected"
      ? activeDevice
        ? deviceDisplayName(activeDevice)
        : t("devices.filter.selected", "Selected")
      : taskFilter === "attention"
        ? t("devices.filter.needsAttention", "Needs attention")
        : t("devices.filter.allDevices", "All devices");
  ...
  return (
    <div className="devices-panel">
      <div className="dp-header">
        <h1 className="dp-title">{t("devices.title", "Devices")}</h1>
      </div>
      ...
      >
        <div className="dp-input-toolbar">
          <div className="dp-toolbar-primary">
            <span className="dp-device-pill">
              ...
              {activeDevice
                ? deviceDisplayName(activeDevice)
                : t("devices.noDeviceSelected", "No device selected")}
            </span>
            <span className="dp-purpose-chip">
              {purposeLabel(activeDevice?.purpose)}
            </span>
            <span className="dp-run-chip">
              {activeDevice?.role === "local"
                ? t("devices.runHere", "Run here")
                : t("devices.runOnSelectedRemote", "Run on selected remote")}
            </span>
          </div>
          <select
          ...
          >
            <option value="" disabled>
              {t("devices.selectManagedDevice", "Select managed device")}
            </option>
            {devices.map((device) => (
              <option key={device.id} value={device.id}>
                {deviceDisplayName(device)} ({purposeLabel(device.purpose)})
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="dp-input-row">
          ...
          <input
            ...
            placeholder={
              !activeDevice
                ? t(
                    "devices.prompt.selectDevice",
                    "Select a device to start a task...",
                  )
                : submittingTask
                  ? t("devices.prompt.startingTask", "Starting task...")
                  : t(
                      "devices.prompt.startOnDevice",
                      "Start a task on {device}...",
                      {
                        device: deviceDisplayName(activeDevice),
                      },
                    )
            }
            value={taskPrompt}
        ...
        <div className="dp-input-actions">
          <button
            ...
            aria-label={t("devices.actions.attachFiles", "Attach files")}
            title={t(
              "devices.actions.attachFilesFromDevice",
              "Attach files from this device",
            )}
          >
            <Plus size={20} />
          </button>
          <div
            className="overflow-menu-container dp-overflow-menu"
            ref={overflowMenuRef}
          >
            <button
              type="button"
              ...
              aria-label={t("devices.taskOptions", "Task options")}
            >
              ...
              >
                ...
                <div className="overflow-menu-item" role="none">
                  <button
                    ...
                    aria-label={t(
                      ...
                      {
                        state: shellAccess
                          ? t("composer.shellOn", "on")
                          : t("composer.shellOff", "off"),
                      },
                    )}
                  >
                    <svg
                    ...
                    <span>{t("composer.shell", "Shell")}</span>
                    <span
                      className={`goal-mode-switch-track ${shellAccess ? "on" : ""}`}
                      aria-hidden="true"
                    >
                    ...
                    <span className="goal-mode-toggle-switch-content">
                      <span className="goal-mode-toggle-text">
                        <span className="goal-mode-label">
                          {t("composer.autonomous", "Autonomous")}
                        </span>
                      </span>
                      <span
                ...
                <div className="overflow-menu-item" role="none">
                  <button
                    ...
                    onClick={() =>
                      setOverflowSubmenu((c) => (c === "mode" ? null : "mode"))
                    }
                    role="menuitem"
                    aria-haspopup="menu"
                  ...
                  >
                    <span className="overflow-submenu-trigger-content">
                      <span className="goal-mode-toggle-text">
                        <span className="goal-mode-label">
                          {t("composer.modePrefix", "Mode: {mode}", {
                            mode: t(
                ...
                <div className="overflow-menu-item" role="none">
                  <button
                    ...
                    onClick={() =>
                      setOverflowSubmenu((c) =>
                        c === "domain" ? null : "domain",
                      )
                    }
                    role="menuitem"
                    aria-haspopup="menu"
                  ...
                  >
                    <span className="overflow-submenu-trigger-content">
                      <span className="goal-mode-toggle-text">
                        <span className="goal-mode-label">
                          {t("composer.domainPrefix", "Domain: {domain}", {
                            domain: t(
                              `composer.domain.${taskDomain}`,
                    ...
                    <div className="overflow-submenu-header">
                      <span className="overflow-submenu-title">
                        {overflowSubmenu === "mode"
                          ? t("composer.executionMode", "Mode")
                          : t("composer.taskDomain", "Domain")}
                      </span>
                    </div>
                    {(overflowSubmenu === "mode"
          ...
          <button
            ...
            aria-label={t("devices.sendTask", "Send task")}
            title={t("devices.sendTask", "Send task")}
          >
            <Send size={20} />
          </button>
        </div>
      ...
      <div ref={tasksRef} className="dp-section">
        <div className="dp-section-header">
          <span className="dp-section-label">
            {t("devices.runningTasks", "Running Tasks")}
          </span>
        ...
        {deviceTasks.length > 0 ? (
          <div className="dp-tasks-container expanded">
            <div className="dp-task-feed-meta">
              {t("devices.showingFilter", "Showing {filter}", {
                filter: taskCountLabel,
              })}
            </div>
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      onOpenTask(
                        task.id,
                        taskDevice?.role === "remote"
                          ? {
                              deviceId: taskDevice.id,
                              deviceName: taskDevice.name,
                            }
                          : undefined,
                      )
                    }
                  >
                    <span className="dp-task-title">
                      {task.title || task.prompt}
                    </span>
                    <div className="dp-task-detail-row">
                      <span className="dp-task-device-badge">
                        {taskDevice
                          ? deviceDisplayName(taskDevice)
                      ...
                      {isTaskAttention(task) ? (
                        <span className="dp-task-inline-alert">
                          {task.terminalStatus === "awaiting_approval"
                            ? t(
                                "devices.task.approvalBlocked",
                                "Approval blocked",
                              )
                            : t(
                                "devices.task.waitingForInput",
                                "Waiting for input",
                              )}
                        </span>
                      ) : null}
          ...
          <div className="dp-placeholder">
            {taskFilter === "attention"
              ? t(
                  "devices.empty.noAttentionTasks",
                  "No tasks need attention right now.",
                )
              : activeDevice
                ? t(
                    "devices.empty.noTasksForSelection",
                    "No tasks have been run for this selection yet.",
                  )
                : t(
                    "devices.empty.selectDeviceForTasks",
      ...
      <div ref={devicesRef} className="dp-section">
        <div className="dp-section-header">
          ...
          <button
            ...
            onClick={() => setOverlay({ type: "pairing" })}
          >
            {t("devices.addNewDevice", "Add new device")} &gt;
          </button>
        </div>
        {devices.length > 0 ? (
          <div className="dp-devices-list">
            {devices.map((device) => {
              ...
              const attentionLabel = deviceAttentionLabel(
                device.attentionState,
              );
              return (
                <div
                  key={device.id}
                  ...
                  <div className="dp-device-content">
                    <div className="dp-device-topline">
                      <div className="dp-device-heading">
                        ...
                        <div className="dp-device-chip-row">
                          ...
                          <span className="dp-device-chip">
                            {device.transport}
                          </span>
                          {attentionLabel ? (
                            <span
                              className={`dp-device-chip attention-${device.attentionState}`}
                            >
                              {attentionLabel}
                            </span>
                          ) : null}
                    ...
                    <div className="dp-device-stats">
                      <div className="dp-device-stat">
                        ...
                        <span>
                          {t("devices.stats.activeRuns", "Active runs")}
                        </span>
                      </div>
                      <div className="dp-device-stat">
                        <strong>
                          {device.storageSummary?.workspaceCount || 0}
                        </strong>
                        <span>
                          {t("devices.stats.workspaces", "Workspaces")}
                        </span>
                      </div>
                      <div className="dp-device-stat">
                        <strong>
                          {device.appsSummary?.channelsEnabled || 0}
                        </strong>
                        <span>{t("devices.stats.appsOn", "Apps on")}</span>
                      </div>
                      <div className="dp-device-stat">
                        <strong>
                          {device.lastSeenAt
                            ? formatRelativeTime(device.lastSeenAt)
                            : t("devices.stats.noSync", "No sync")}
                        </strong>
                        <span>{t("devices.stats.lastSeen", "Last seen")}</span>
                      </div>
                    </div>
                    <div className="dp-device-footer">
                      {summary?.alerts?.length ? (
                        <div className="dp-device-alert-strip">
                          {summary.alerts[0].title}
                        </div>
                      ) : (
                        <div className="dp-device-hint">
                          {t(
                            "devices.card.selectHint",
                      ...
                      <div className="dp-device-actions">
                        {device.role === "remote" ? (
                          device.status === "connected" ? (
                            ...
                            >
                              <Unplug size={14} />
                              {t("devices.actions.disconnect", "Disconnect")}
                            </button>
                          ) : (
                            <button
                              type="button"
                            ...
                            >
                              <Plug2 size={14} />
                              {t("devices.actions.connect", "Connect")}
                            </button>
                          )
                        ) : null}
                        <button
                        ...
                        >
                          <AppWindow size={14} />
                          {t("devices.actions.openApps", "Open Apps")}
                        </button>
                        <button
                          type="button"
                          className="dp-ghost-btn"
                        ...
                        >
                          <HardDrive size={14} />
                          {t("devices.actions.storage", "Storage")}
                        </button>
                        <button
                          type="button"
                          className="dp-ghost-btn"
                        ...
                        >
                          <Activity size={14} />
                          {t("devices.actions.details", "Details")}
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      <div className="dp-support-grid">
        <section ref={appsRef} className="dp-support-card">
          <div className="dp-support-header">
            <div>
              <span className="dp-support-eyebrow">
                {t("devices.support.appsAccounts", "Apps & Accounts")}
              </span>
              <h3>{deviceDisplayName(activeDevice)}</h3>
            </div>
            <button
              className="dp-section-link"
              disabled={!activeDevice}
              onClick={() =>
                activeDevice &&
                setOverlay({ type: "apps", deviceId: activeDevice.id })
              }
            >
              {t("devices.actions.manage", "Manage")} &gt;
            </button>
          </div>
          {activeSummary ? (
            <>
              <div className="dp-support-stats">
                <div>
                  ...
                  <span>{t("devices.stats.enabledApps", "Enabled apps")}</span>
                </div>
                <div>
                  <strong>{activeSummary.apps.channelsTotal}</strong>
                  <span>{t("devices.stats.totalApps", "Total apps")}</span>
                </div>
                <div>
                  <strong>{activeSummary.apps.accounts?.length || 0}</strong>
                  <span>{t("devices.stats.accounts", "Accounts")}</span>
                </div>
              </div>
              <div className="dp-support-list">
                {(activeSummary.apps.channels || [])
                  .slice(0, 3)
                  .map((channel: any) => (
                    <div key={channel.id} className="dp-support-row">
                      ...
                      <span
                        className={`dp-inline-status ${channel.enabled ? "ok" : "muted"}`}
                      >
                        {channel.status ||
                          (channel.enabled
                            ? t("devices.status.enabled", "enabled")
                            : t("devices.status.disabled", "disabled"))}
                      </span>
                    </div>
                  ))}
                {(activeSummary.apps.channels || []).length === 0 ? (
                  <div className="dp-placeholder compact">
                    {t(
                      "devices.empty.noAppsConfigured",
            ...
            <div className="dp-placeholder compact">
              {t(
                "devices.empty.loadSummaryApps",
                "Load a device summary to inspect its apps.",
              )}
            </div>
          )}
        </section>
        ...
        <section ref={storageRef} className="dp-support-card">
          <div className="dp-support-header">
            <div>
              <span className="dp-support-eyebrow">
                {t("devices.support.alertsUpdates", "Alerts & Updates")}
              </span>
              <h3>{deviceDisplayName(activeDevice)}</h3>
            </div>
            <button
              ...
              onClick={() =>
                activeDevice &&
                setOverlay({ type: "observer", deviceId: activeDevice.id })
              }
            >
              {t("devices.actions.monitor", "Monitor")} &gt;
            </button>
          </div>
          {activeSummary ? (
            <div className="dp-support-list">
              {activeSummary.alerts.slice(0, 4).map((alert) => (
                <div key={alert.id} className="dp-support-row">
                  ...
                  <span className={`dp-inline-status level-${alert.level}`}>
                    {alert.level}
                  </span>
                </div>
              ))}
              {activeSummary.alerts.length === 0 ? (
                <div className="dp-placeholder compact">
                  {t(
                    "devices.empty.noDiagnosticIssues",
            ...
            <div className="dp-placeholder compact">
              {t(
                "devices.empty.loadSummaryAlerts",
                "Load a device summary to inspect alerts.",
              )}
            </div>
          )}
        </section>
        ...
        <section ref={alertsRef} className="dp-support-card">
          <div className="dp-support-header">
            <div>
              <span className="dp-support-eyebrow">
                {t("devices.support.storageFiles", "Storage & Files")}
              </span>
              <h3>{deviceDisplayName(activeDevice)}</h3>
            </div>
            <button
              ...
              onClick={() =>
                activeDevice &&
                setOverlay({ type: "storage", deviceId: activeDevice.id })
              }
            >
              {t("devices.actions.inspect", "Inspect")} &gt;
            </button>
          </div>
          {activeSummary ? (
            <>
              <div className="dp-support-stats">
                <div>
                  ...
                  <span>{t("devices.stats.workspaces", "Workspaces")}</span>
                </div>
                <div>
                  <strong>{activeSummary.storage.artifactCount}</strong>
                  <span>{t("devices.stats.artifacts", "Artifacts")}</span>
                </div>
                <div>
                  <strong>
                    {formatBytes(activeSummary.storage.freeBytes)}
                  </strong>
                  <span>{t("devices.stats.freeSpace", "Free space")}</span>
                </div>
              </div>
              <div className="dp-support-list">
                {activeSummary.storage.workspaceRoots
                  .slice(0, 3)
                  .map((root) => (
                ...
                {activeSummary.storage.workspaceRoots.length === 0 ? (
                  <div className="dp-placeholder compact">
                    {t(
                      "devices.empty.noWorkspaceRoots",
                      "No workspace roots reported yet.",
                    )}
                  </div>
                ) : null}
              </div>
            ...
            <div className="dp-placeholder compact">
              {t(
                "devices.empty.loadSummaryStorage",
                "Load a device summary to inspect storage.",
              )}
            </div>
          )}
        </section>
        ...
        <section className="dp-support-card">
          <div className="dp-support-header">
            <div>
              <span className="dp-support-eyebrow">
                {t("devices.support.resourceHealth", "Resource Health")}
              </span>
              <h3>{deviceDisplayName(activeDevice)}</h3>
            </div>
            <button
              ...
              onClick={() =>
                activeDevice &&
                setOverlay({ type: "details", deviceId: activeDevice.id })
              }
            >
              {t("devices.actions.hardware", "Hardware")} &gt;
            </button>
          </div>
          {activeSummary ? (
            <div className="dp-support-stats">
              <div>
                <strong>
                  {(activeSummary.device as any).cpuUsage?.toFixed(0) || 0}%
                </strong>
                <span>{t("devices.stats.cpuLoad", "CPU Load")}</span>
              </div>
              <div>
                <strong>
                  {(activeSummary.device as any).memoryUsage?.toFixed(0) || 0}%
                </strong>
                <span>{t("devices.stats.memory", "Memory")}</span>
              </div>
              <div>
                <strong>
                  {(activeSummary.device as any).loadAverage?.[0]?.toFixed(1) ||
                    0}
                </strong>
                <span>{t("devices.stats.loadAvg", "Load Avg")}</span>
              </div>
            </div>
          ) : (
            <div className="dp-placeholder compact">
              {t(
                "devices.empty.loadSummaryResources",
      ...
      <div className="dp-section">
        <div className="dp-section-header">
          <span className="dp-section-label">
            {t("devices.dispatch.title", "Dispatch")}
          </span>
        </div>
        {dispatchLoading ? (
          <div className="dispatch-loading">
            {t("devices.dispatch.loading", "Loading...")}
          </div>
        ) : (
          (() => {
            const connectedChannels = dispatchChannels.filter(
            ...
            if (!hasConnections) {
              return (
                <div className="dispatch-onboarding">
                  ...
                  <h2 className="dispatch-onboarding-title">
                    {t("devices.dispatch.onTheGo", "{app} on the go", {
                      app: APP_NAME,
                    })}
                  </h2>
                  <p className="dispatch-onboarding-subtitle">
                    {t(
                      ...
                      "Dispatch tasks to {app} from WhatsApp, Telegram, Slack, and other messaging apps in one continuous conversation.",
                      { app: APP_NAME },
                    )}
                  </p>
                  <div className="dispatch-feature-cards">
                    <div className="dispatch-feature-card">
                      <MessageCircle
                        size={20}
                        strokeWidth={2}
                        className="dispatch-feature-icon"
                      />
                      <p>
                        {t(
                          "devices.dispatch.feature.messaging",
                          "Your messaging apps can communicate with {app} on your computer.",
                          { app: APP_NAME },
                        )}
                      </p>
                    </div>
                    <div className="dispatch-feature-card">
                      ...
                      <p>
                        {t(
                          ...
                          "Send {app} a message from WhatsApp, Telegram, or Slack, and it will work on tasks using your computer.",
                          { app: APP_NAME },
                        )}
                      </p>
                    </div>
                    <div className="dispatch-feature-card">
                      ...
                      <p>
                        {t(
                          ...
                          "{app} can also run tasks on a schedule or whenever you need them.",
                          { app: APP_NAME },
                        )}
                      </p>
                    </div>
                    <div className="dispatch-feature-card">
                      ...
                      <p>
                        {t(
                          ...
                          "Remember to keep your computer awake so {app} can keep working.",
                          { app: APP_NAME },
                        )}{" "}
                        <button
                          type="button"
                  ...
                  <span className="dispatch-section-label">
                    {t(
                      "devices.dispatch.connectChannel",
                      "Connect at least one channel to get started",
                    )}
                  </span>
                  <div className="dispatch-setup-cards">
                    {DISPATCH_CHANNELS.map(
                      ({ type, label, icon: Icon, settingsTab }) => {
                        const ch = dispatchChannels.find(
                          (c) => c.type === type,
                        );
                        ...
                        return (
                          ...
                          >
                            <Icon
                              size={20}
                              strokeWidth={2}
                              className="dispatch-setup-icon"
                            />
                            <div className="dispatch-setup-card-content">
                              <strong>
                                {isConnected
                                  ? t(
                                      "devices.dispatch.connectedTo",
                                      "Connected to {label}",
                                      { label },
                                    )
                                  : t(
                                      "devices.dispatch.connectTo",
                                      "Connect to {label}",
                                      { label },
                                    )}
                              </strong>
                              <span>
                                {isConnected
                                  ? t(
                                      "devices.dispatch.sendFromAnytime",
                                      "Send tasks from {label} anytime",
                                      { label },
                                    )
                                  : t(
                                      "devices.dispatch.linkAccount",
                            ...
                            {isConnected ? (
                              <span className="dispatch-setup-badge connected">
                                ●
                              </span>
                            ) : (
                  ...
                  >
                    {t("devices.dispatch.getStarted", "Get started")}
                  </button>
                  <p className="dispatch-disclaimer">
                    {t(
                      "devices.dispatch.disclaimer",
                      "{app} will access your desktop (files, apps, and browser) to complete tasks you send from messaging apps. This may have security risks. Only connect devices and accounts that you own and trust.",
                      { app: APP_NAME },
                    )}{" "}
                    <button
                      type="button"
            ...
            return (
              <div className="dispatch-connected">
                <div className="dispatch-info-card">
                  <p>
                    {t("devices.dispatch.connectedInfoPrefix", "Dispatch")}{" "}
                    {t(
                      "devices.dispatch.connectedInfo",
                      "from your connected apps: seamless task handoff from WhatsApp, Telegram, Slack, and more.",
                    )}
                  </p>
                </div>
                <div className="dispatch-settings-list">
                  <div className="dispatch-settings-item">
                    ...
                    <div>
                      <strong>
                        {t(
                          "devices.dispatch.keepAwakeTitle",
                          "Keep this computer awake",
                        )}
                      </strong>
                      <span>
                        {t(
                          "devices.dispatch.keepAwakeDescription",
                          "Prevents sleep while Dispatch is running.",
                        )}
                      </span>
                    </div>
                ...
                <div className="dp-section">
                  <span className="dp-section-label">
                    {t("devices.outputs", "Outputs")}
                  </span>
                  <div className="dp-placeholder">
                    {t(
                      "devices.outputsPlaceholder",
                      "Files {appName} shares will appear here.",
                      { appName: APP_NAME },
                    )}
                  </div>
                </div>
...
function ModalShell({
  ...
  return (
    <div className="devices-onboarding-overlay" onClick={onClose}>
      <div
        className="devices-remote-modal"
      ...
      >
        <div className="devices-remote-header">
          ...
          <button
            className="devices-remote-close"
            onClick={onClose}
...
function DeviceConnectionOverlay({
  ...
  const populateForm = useCallback((device: ManagedDevice) => {
    ...
    setRemoteDeviceName(
      device.name || device.config?.deviceName || "NeoWorker Remote Client",
    );
  ...
  const buildConfig = useCallback(
    (): RemoteGatewayConfig => ({
      ...
      deviceName: remoteDeviceName.trim() || "NeoWorker Remote Client",
      autoReconnect: true,
  ...
  const buildManagedDevice = useCallback(
    (id: string): ManagedDevice => ({
      id,
      name: remoteDeviceName.trim() || "NeoWorker Remote Client",
      role: "remote",
      purpose,
  ...
  const saveDevice = useCallback(
    async (connectAfterSave: boolean) => {
      ...
      try {
        ...
        const nextManagedDevices = upsertManagedRemoteDevice(
          existingManaged,
          nextDevice,
        );
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const result =
        await window.electronAPI?.testRemoteGatewayConnection?.(buildConfig());
      setTestResult(
        result?.ok
          ? {
              success: true,
  ...
  return (
    <ModalShell
      title={translate("devices.addDevice.title", "Add new device")}
      subtitle={translate(
        "devices.addDevice.subtitle",
        "Pair a remote NeoWorker once, then manage it from here.",
      )}
      onClose={onClose}
    >
      {loadingConfig ? (
        <div className="devices-remote-loading">
          ...
          <span>
            {translate(
              "devices.addDevice.loadingSaved",
        ...
        <>
          ...
          {remoteDevices.length > 0 ? (
            <div className="devices-remote-saved">
              <div className="devices-remote-saved-header">
                <h3>
                  {translate("devices.addDevice.savedDevices", "Saved devices")}
                </h3>
                <button
                  type="button"
                ...
                >
                  {translate("devices.addDevice.newDevice", "New device")}
                </button>
              </div>
          ...
          <div className="devices-remote-form-card">
            <div className="devices-remote-saved-header">
              <h3>
                {selectedDeviceId && !isCreatingNewDevice
                  ? translate("devices.details.title", "Device details")
                  : translate("devices.addDevice.newDevice", "New device")}
              </h3>
            </div>

            <div className="devices-remote-field">
              <label htmlFor="devices-remote-name">
                {translate("devices.addDevice.deviceName", "Device name")}
              </label>
              <input
                ...
                placeholder={translate(
                  "devices.addDevice.deviceNamePlaceholder",
                  "Work Mac Mini",
                )}
              />
            </div>
            ...
            <div className="devices-remote-field">
              <label htmlFor="devices-purpose">
                {translate("devices.addDevice.purpose", "Purpose")}
              </label>
              <select
                id="devices-purpose"
                value={purpose}
                onChange={(event) =>
                  setPurpose(event.target.value as ManagedDevice["purpose"])
                }
              >
                <option value="general">
                  {translate("devices.purpose.general", "General")}
                </option>
                <option value="work">
                  {translate("devices.purpose.work", "Work")}
                </option>
                <option value="personal">
                  {translate("devices.purpose.personal", "Personal")}
                </option>
                <option value="automation">
                  {translate("devices.purpose.automation", "Automation")}
                </option>
                <option value="archive">
                  {translate("devices.purpose.archive", "Archive")}
                </option>
                <option value="primary">
                  {translate("devices.purpose.primary", "Primary")}
                </option>
              </select>
            </div>

            <div className="devices-remote-field">
              <label htmlFor="devices-remote-url">
                {translate("devices.addDevice.gatewayUrl", "Gateway URL")}
              </label>
              <input
            ...
            <div className="devices-remote-field">
              <label htmlFor="devices-remote-token">
                {translate("devices.addDevice.token", "Token")}
              </label>
              <div className="devices-remote-token-row">
                <input
                  ...
                  placeholder={translate(
                    "devices.addDevice.tokenPlaceholder",
                    "Remote auth token",
                  )}
                />
                <button
            ...
            {testResult ? (
              ...
              >
                {testResult.success
                  ? translate(
                      "devices.addDevice.connectionSuccessful",
                      "Connection successful{latency}",
                      {
            ...
            <div className="devices-remote-actions">
              ...
              >
                {testing
                  ? translate("common.testing", "Testing...")
                  : translate(
                      "devices.addDevice.testConnection",
                      "Test Connection",
                    )}
              </button>
              <button
              ...
              >
                {saving
                  ...
                  : selectedDeviceId
                    ...
                    : translate("devices.addDevice.addDevice", "Add Device")}
              </button>
              <button
              ...
              >
                {saving
                  ...
                  : translate(
                      "devices.addDevice.connectDevice",
                      "Connect Device",
                    )}
              </button>
            </div>
      ...
      <aside className="dp-drawer" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="dp-drawer-actions">
          {device.role === "remote" ? (
            device.status === "connected" ? (
              <button
                className="dp-secondary-btn"
                onClick={() => void onDisconnect(device.id)}
              >
                <Unplug size={14} />
                {translate("devices.actions.disconnect", "Disconnect")}
              </button>
            ) : (
              <button
                className="dp-primary-btn"
                onClick={() => void onConnect(device.id)}
              >
                <Plug2 size={14} />
                {translate("devices.actions.connect", "Connect")}
              </button>
            )
          ) : null}
          {device.role === "remote" ? (
            <button
              className="dp-secondary-btn danger"
              onClick={() => void onRemove(device.id)}
            >
              <Trash2 size={14} />
              {translate("devices.details.removeDevice", "Remove device")}
            </button>
          ) : null}
        </div>
        ...
        <div className="dp-detail-grid">
          ...
          <DetailBlock
            ...
            label={translate("devices.addDevice.purpose", "Purpose")}
            value={purposeLabel(device.purpose)}
          />
          <DetailBlock
            icon={<Server size={14} />}
            label={translate("devices.details.transport", "Transport")}
            value={device.transport}
          />
          <DetailBlock
            icon={<Activity size={14} />}
            label={translate("devices.details.version", "Version")}
            value={device.version || translate("common.unknown", "Unknown")}
          />
          <DetailBlock
            icon={<Cpu size={14} />}
            label={translate("devices.details.platform", "Platform")}
            value={device.platform}
          />
          <DetailBlock
            icon={<Bell size={14} />}
            label={translate("devices.stats.lastSeen", "Last seen")}
            value={
              device.lastSeenAt
                ? formatRelativeTime(device.lastSeenAt)
                : translate("devices.notYet", "Not yet")
            }
          />
        </div>

        <div className="dp-drawer-section">
          <h3>{translate("devices.details.runtime", "Runtime")}</h3>
          <div className="dp-code-list">
            <code>
              {summary.runtime?.cwd ||
                translate(
                  "devices.details.noRuntimeSnapshot",
                  "No runtime snapshot yet",
                )}
            </code>
            <code>
              {summary.runtime?.userDataDir ||
                translate(
                  "devices.details.noUserDataPath",
                  "No user data path reported",
                )}
            </code>
            <code>{`profile:${summary.runtime?.activeProfileId || "default"}`}</code>
            <code>
              {device.config?.url ||
                translate("devices.status.local", "Local device")}
            </code>
          </div>
        </div>

        <div className="dp-drawer-section">
          <h3>{translate("devices.details.diagnostics", "Diagnostics")}</h3>
          <div className="dp-support-list">
            <div className="dp-support-row">
              <div>
                <strong>{summary.tasks.active}</strong>
                <span>
                  {translate("devices.details.activeRuns", "Active runs")}
                </span>
              </div>
            </div>
            <div className="dp-support-row">
              <div>
                ...
                <span>
                  {translate(
                    "devices.details.approvalsPending",
                    "Approvals pending",
                  )}
                </span>
              </div>
            ...
            <div className="dp-support-row">
              <div>
                ...
                <span>
                  {translate(
                    "devices.details.inputRequestsPending",
                    "Input requests pending",
                  )}
                </span>
              </div>
            </div>
...
function getChannelSetupHint(
  type: AppsChannelType,
...
function validateChannelDraft(
  ...
  switch (type) {
    case "telegram":
      return draft.telegramBotToken.trim()
        ? null
    ...
    case "teams":
      ...
      if (!draft.teamsAppPassword.trim())
        return "Teams app password is required.";
      return null;
    case "email":
      if (draft.emailProtocol === "loom") {
        if (!draft.emailLoomBaseUrl.trim()) return "Loom base URL is required.";
        if (!draft.emailLoomAccessToken.trim())
          return "Loom access token is required.";
        return null;
      }
...
function buildChannelCreateParams(
  type: AppsChannelType,
  draft: ChannelDraftState,
) {
...
function AppsManagerModal({
  ...
  const [newChannelType, setNewChannelType] =
    useState<AppsChannelType>("whatsapp");
  const [draft, setDraft] = useState<ChannelDraftState>(() =>
    buildChannelDraft("whatsapp"),
  );
  ...
  const loadApps = useCallback(async () => {
    ...
    try {
      ...
      if (channelsResult?.ok) {
        const payload = channelsResult.payload as
          { channels?: any[] } | undefined;
        setChannels(Array.isArray(payload?.channels) ? payload.channels : []);
      }
      if (accountsResult?.ok) {
        const payload = accountsResult.payload as
  ...
  return (
    <ModalShell
      ...
      subtitle={translate(
        "devices.apps.subtitle",
        "Manage channels, connectors, and linked accounts without leaving Devices.",
      )}
      onClose={onClose}
    >
      <div className="dp-modal-section">
        <div className="devices-remote-saved-header">
          <h3>{translate("devices.apps.connections", "Connections")}</h3>
          <button
            className="devices-remote-link-btn"
            onClick={() => void loadApps()}
          >
            {loading
              ? translate("common.refreshing", "Refreshing...")
        ...
        <div className="dp-channel-builder">
          <div className="dp-inline-form dp-channel-builder-top">
            ...
            >
              ...
              <option value="teams">
                {translate("devices.channel.teams", "Teams")}
              </option>
              <option value="email">
                {translate("devices.channel.email", "Email")}
              </option>
            </select>
            <input
              type="text"
            ...
            >
              <option value="pairing">
                {translate("devices.apps.security.pairing", "Pairing mode")}
              </option>
              <option value="allowlist">
                {translate("devices.apps.security.allowlist", "Allowlist")}
              </option>
              <option value="open">
                {translate("devices.apps.security.open", "Open")}
              </option>
            </select>
          </div>

          <p className="dp-channel-builder-hint">
            {getChannelSetupHint(newChannelType, draft)}
          </p>
          ...
          {newChannelType === "whatsapp" ? (
            <div className="dp-channel-fields">
              <label className="devices-remote-field">
                <span>
                  {translate("devices.apps.allowedNumbers", "Allowed numbers")}
                </span>
                <textarea
              ...
              <label className="devices-remote-field">
                <span>
                  {translate("devices.apps.responsePrefix", "Response prefix")}
                </span>
                <input
                  type="text"
          ...
          {newChannelType === "telegram" ? (
            <div className="dp-channel-fields">
              <label className="devices-remote-field">
                <span>{translate("devices.apps.botToken", "Bot token")}</span>
                <input
                  type="password"
          ...
          {newChannelType === "slack" ? (
            <div className="dp-channel-fields">
              ...
              <label className="devices-remote-field">
                <span>{translate("devices.apps.appToken", "App token")}</span>
                <input
                  type="password"
              ...
              <label className="devices-remote-field">
                <span>
                  {translate("devices.apps.signingSecret", "Signing secret")}
                </span>
                <input
                  type="password"
          ...
          {newChannelType === "teams" ? (
            <div className="dp-channel-fields">
              <label className="devices-remote-field">
                <span>{translate("devices.apps.appId", "App ID")}</span>
                <input
                  type="text"
              ...
              <label className="devices-remote-field">
                <span>
                  {translate("devices.apps.appPassword", "App password")}
                </span>
                <input
                  type="password"
                  ...
                  placeholder={translate(
                    "devices.apps.microsoftAppPassword",
                    "Microsoft app password",
                  )}
                />
              </label>
              <label className="devices-remote-field">
                <span>{translate("devices.apps.tenantId", "Tenant ID")}</span>
                <input
                  type="text"
              ...
              <label className="devices-remote-field">
                <span>
                  {translate("devices.apps.webhookPort", "Webhook port")}
                </span>
                <input
                  type="text"
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      teamsWebhookPort: event.target.value,
                    }))
                  }
                  placeholder="3978"
                />
          ...
          {newChannelType === "email" ? (
            <div className="dp-channel-fields">
              <label className="devices-remote-field">
                <span>{translate("devices.apps.protocol", "Protocol")}</span>
                <select
              ...
              <label className="devices-remote-field">
                <span>
                  {translate("devices.apps.displayName", "Display name")}
                </span>
                <input
                  type="text"
                  ...
                  onChange={(event) =>
                    setDraft((current) => ({
                      ...current,
                      emailDisplayName: event.target.value,
                    }))
                  }
                  placeholder={translate("common.optional", "Optional")}
                />
              ...
              {draft.emailProtocol === "imap-smtp" ? (
                <>
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.emailAddress", "Email address")}
                    </span>
                    <input
                      type="text"
                  ...
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.password", "Password")}
                    </span>
                    <input
                      type="password"
                  ...
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.imapHost", "IMAP host")}
                    </span>
                    <input
                      type="text"
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          emailImapHost: event.target.value,
                        }))
                      }
                  ...
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.imapPort", "IMAP port")}
                    </span>
                    <input
                      type="text"
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          emailImapPort: event.target.value,
                        }))
                      }
                      placeholder="993"
                    />
                  ...
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.smtpHost", "SMTP host")}
                    </span>
                    <input
                      type="text"
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          emailSmtpHost: event.target.value,
                        }))
                      }
                  ...
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.smtpPort", "SMTP port")}
                    </span>
                    <input
                      type="text"
                      ...
                      onChange={(event) =>
                        setDraft((current) => ({
                          ...current,
                          emailSmtpPort: event.target.value,
                        }))
                      }
                      placeholder="587"
                    />
                ...
                <>
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.loomBaseUrl", "Loom base URL")}
                    </span>
                    <input
                      type="text"
                  ...
                  <label className="devices-remote-field">
                    <span>
                      {translate("devices.apps.loomIdentity", "Loom identity")}
                    </span>
                    <input
                      type="text"
          ...
          {formError ? (
            <div className="devices-remote-feedback error">{formError}</div>
          ) : null}
          ...
          <div className="devices-remote-actions">
            ...
            >
              {workingKey === "channel:create"
                ? translate("devices.apps.adding", "Adding...")
                : translate("devices.apps.addConnection", "Add connection")}
            </button>
          </div>
        </div>
            ...
            <div key={channel.id} className="dp-channel-row">
              ...
              <div className="dp-channel-actions">
                ...
                <button
                  className="dp-ghost-btn"
                  onClick={() =>
                    void runChannelAction(channel.id, "channel.test")
                  }
                  disabled={workingKey === `channel.test:${channel.id}`}
                >
                  {translate("common.test", "Test")}
                </button>
                <button
                  className="dp-ghost-btn danger"
                  onClick={() =>
                    void runChannelAction(channel.id, "channel.remove")
                  }
      ...
      <div className="dp-modal-section">
        <h3>{translate("devices.apps.accounts", "Accounts")}</h3>
        <div className="dp-support-list">
          {accounts.map((account) => (
            <div key={account.id} className="dp-support-row">
              ...
              <span
                className={`dp-inline-status ${account.status === "active" ? "ok" : "muted"}`}
              >
          ...
          {accounts.length === 0 ? (
            <div className="dp-placeholder compact">
              {translate(
                "devices.apps.noAccounts",
                "No managed accounts linked to this device.",
              )}
            </div>
          ) : null}
        </div>
...
function StorageModal({
  ...
  return (
    <ModalShell
      ...
      subtitle={translate(
        "devices.storage.subtitle",
        "Workspace roots, artifact volume, and safe storage indicators.",
      )}
      onClose={onClose}
    >
      <div className="dp-support-stats">
        <div>
          ...
          <span>
            {translate("devices.storage.workspaceRoots", "Workspace roots")}
          </span>
        </div>
        <div>
          <strong>{summary.storage.artifactCount}</strong>
          <span>{translate("devices.stats.artifacts", "Artifacts")}</span>
        </div>
        <div>
          <strong>{formatBytes(summary.storage.freeBytes)}</strong>
          <span>{translate("devices.storage.freeDisk", "Free disk")}</span>
        </div>
      </div>
      <div className="dp-support-list">
...
function ObserverModal({
  ...
  return (
    <ModalShell
      ...
      subtitle={translate(
        "devices.observer.subtitle",
        "Alerts, approvals, and connection events for the selected device.",
      )}
      onClose={onClose}
    >
      <div className="dp-modal-section">
        <h3>{translate("devices.observer.alerts", "Alerts")}</h3>
        <div className="dp-support-list">
          ...
          {summary.alerts.length === 0 ? (
            <div className="dp-placeholder compact">
              {translate(
                "devices.observer.noActiveAlerts",
                "No active alerts.",
              )}
            </div>
          ) : null}
      ...
      <div className="dp-modal-section">
        <h3>{translate("devices.observer.feed", "Observer feed")}</h3>
        <div className="dp-observer-list">
            ...
            <div key={entry.id} className="dp-observer-row">
              ...
              <div>
                ...
                <span>
                  {entry.detail ||
                    translate(
                      "devices.observer.noAdditionalDetail",
          ...
          {summary.observer.length === 0 ? (
            <div className="dp-placeholder compact">
              {translate(
                "devices.observer.noEvents",
                "No observer events recorded yet.",
              )}
            </div>
          ) : null}
        </div>
