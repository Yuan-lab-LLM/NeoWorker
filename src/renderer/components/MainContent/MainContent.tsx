...
function getAttachmentFilePresentation(fileName: string, _mimeType?: string) {
  const extension = String(fileName || "")
    ...
    .toLowerCase();
  const format =
    extension && extension !== fileName.toLowerCase() ? extension.toUpperCase() : "FILE";
...
function BubbleAttachmentCard({
  ...
  const { format } = getAttachmentFilePresentation(attachment.name, attachment.mimeType);
  ...
  if (
    ...
    return (
      <AttachmentImagePreview
        ...
        variant="message"
        onOpenFallback={canOpen ? () => onOpen?.(attachment.relativePath!) : undefined}
      />
    );
  }
  const content = (
...
function cacheComposerAttachmentDraft(key: string, attachments: PendingAttachment[]): void {
...
function readRecentSlashSkillIds(): string[] {
  try {
    const parsed = JSON.parse(window.localStorage.getItem(RECENT_SLASH_SKILLS_STORAGE_KEY) || "[]");
...
function getLocalizedWorkspaceStatusFolderLabel(workspace?: Workspace | null): string {
  const label = getWorkspaceStatusFolderLabel(workspace);
  if (label === "Work in a folder") return translate("composer.workInFolder", label);
  if (label === "No folder selected") return translate("composer.noFolderSelected", label);
  return label;
}
...
function hasVisibleAssistantReplyText(event: TaskEvent): boolean {
  ...
  const message = event.payload?.message;
  return typeof message === "string" && cleanAssistantMessageForDisplay(message).trim().length > 0;
}

function isUserFacingPlanStepResult(event: TaskEvent): boolean {
  const stepId = event.payload?.stepId;
  const normalizedStepId =
    typeof stepId === "string" || typeof stepId === "number" ? String(stepId).trim() : "";
  return (
...
function localizeReasoningProgressLine(text: string, language: string): string {
  ...
  if (/^(?:analyz|assess|evaluate)/.test(normalized))
    return translate("generated.components.maincontent.maincontent.950.1", "Analyzing task");
  ...
  if (/^(?:plan|outline|organize)/.test(normalized))
    return translate("generated.components.maincontent.maincontent.953.3", "Working on solutions");
  if (/^(?:write|edit|update|implement|apply)/.test(normalized))
    return translate("generated.components.maincontent.maincontent.955.4", "Processing content");
...
export function TaskSessionLineageFooter({
  ...
  const content = (
    <>
      ...
      <span>{translate("task.lineage.forkedFromConversation", "Forked from conversation")}</span>
    </>
  );
    ...
    >
      ...
      {onSelectTask ? (
        <button
          ...
          title={translate("task.lineage.openSource", "Open source conversation")}
        >
          {content}
        </button>
      ) : (
        <span
          ...
          title={translate("task.lineage.sourceUnavailable", "Source conversation unavailable")}
        >
          {content}
        </span>
      )}
...
function getConversationTurnPreview(events: TaskEvent[], startIndex: number): string {
  for (let index = Math.max(0, startIndex + 1); index < events.length; index += 1) {
    ...
    const preview = cleanAssistantMessageForDisplay(rawMessage).replace(/\s+/g, " ").trim();
...
function ConversationTurnRail({
  ...
  useEffect(() => {
    ...
    ).find((element) => element.dataset.conversationTurnMarkerId === activeTurnId);
    if (!activeRow) return;

    if (activeRow.offsetTop < list.scrollTop) {
      list.scrollTop = activeRow.offsetTop;
    } else if (activeRow.offsetTop + activeRow.offsetHeight > list.scrollTop + list.clientHeight) {
      list.scrollTop = activeRow.offsetTop + activeRow.offsetHeight - list.clientHeight;
    }
  }, [activeTurnId]);
  ...
  const previewTurn =
    previewState == null ? null : (turns.find((turn) => turn.id === previewState.turnId) ?? null);

  const revealPreview = (turnId: string, button: HTMLButtonElement) => {
    const list = railListRef.current;
    if (!list) return;
    const markerCenter = button.offsetTop - list.scrollTop + button.offsetHeight / 2;
    setPreviewState({
      turnId,
  ...
  return (
    <nav
      ...
      aria-label={translate("task.conversationNavigation", "Conversation navigation")}
      style={railPlacement}
    ...
    >
      ...
      >
        {turns.map((turn, index) => {
          ...
          const turnLabel = translate("task.conversationTurn", "Conversation {number}", {
            number: index + 1,
          });
          ...
          return (
            <button
              ...
              onMouseEnter={(event) => revealPreview(turn.id, event.currentTarget)}
      ...
      {previewTurn && previewState ? (
        <div className="conversation-turn-preview" role="tooltip" style={{ top: previewState.top }}>
          <div className="conversation-turn-preview-title">{previewTurn.label}</div>
          {previewTurn.preview ? (
            <div className="conversation-turn-preview-text">{previewTurn.preview}</div>
          ) : null}
        </div>
      ) : null}
    </nav>
...
const TaskConversationFlow = memo(function TaskConversationFlow(props: any) {
  ...
  const onForkTaskSessionFromEvent = props.onForkTaskSessionFromEvent as
    ...
    | undefined;
  const renderLatestUserMessageActionRow = props.renderLatestUserMessageActionRow as
  ...
  const latestUserMessageEventId =
  ...
  const feedRows = useMemo<TaskFeedRow[]>(() => {
    ...
    const artifactStacksByAnchorEventId = new Map<string, EndOfTaskArtifactStack[]>();
    for (const stack of artifactStacks) {
      ...
      const anchorEventId = getEndOfTaskArtifactStackAnchorEventId(stack, events);
      if (anchorEventId) {
        const eventStacks = artifactStacksByAnchorEventId.get(anchorEventId) || [];
    ...
    const pushArtifactStacksForTimelineItem = (item: any) => {
      ...
      for (const anchor of eventAnchors) {
        const rawEventIndex = anchor.event
          ? getEarliestTaskEventStreamIndex(events, [anchor.event], fullEventIndexById)
      ...
      feedRows.filter((row) => {
        ...
        if (isAssistantMessage || isCompletionSummaryMessage) {
          return (
            ...
            shouldRenderAssistantMessageInTranscript(transcriptEvents, item.eventIndex)
          );
        }
    ...
    (message: string, sourceEvent: TaskEvent) => {
      ...
      const deliveryArtifactKeys = new Set(
        collectLatestEndOfTaskArtifactCards(
          ...
          8,
          resolveTaskOutputSummaryFromTask(task, events),
        ).map((artifact) => getArtifactCardDisplayKey(artifact.path, artifact.kind)),
      );
      const artifactPaths = resolveArtifactPathsAgainstTaskEvents(
        ...
        if (
          ...
          // The end-of-task delivery stack owns final Office/HTML artifacts.
          // Rendering the same path again from the assistant summary produced
          // two identical download cards after a successful follow-up.
          return false;
        }
        ...
        {(events.length > 0 || (collaborativeRun && childTasks.length > 0) || isTaskWorking) &&
          (() => {
            const artifactProjectionRevision = visibleFeedRows
              .filter((row) => row.kind === "artifact-stack")
            ...
            const getRowRenderSignature = (row: TaskFeedRow): string => {
              ...
              if (item.kind === "action_block") {
                const isActive = shouldMarkActionBlockActiveForCurrentTurn({
                  isLatestActionBlock: timelineIndex === lastActionBlockTimelineIndex,
                  isTaskWorking,
                  isReplayMode,
                  actionBlockEventIndices: item.eventIndices,
                  latestUserMessageEventIndex,
            ...
            const renderFeedRow = (row: TaskFeedRow) => {
                ...
                return (
                  <div className="conversation-artifact-stack assistant-artifact-cards">
                    ...
                    {hiddenCount > 0 && (
                      ...
                      >
                        <span>
                          {translate("mainContent.artifacts.showMore", "Show {count} more", {
                            count: hiddenCount,
                          })}
                        </span>
              ...
              if (item.kind === "action_block") {
                ...
                // Exclude sessions already shown inline inside an expanded
                // run_command frame. Command sessions are present only while
                // execution records are enabled.
              ...
              if (isUserMessage) {
                ...
                const messageAttachments =
                  typeof rawMessage === "string" ? extractAttachmentDetails(rawMessage) : [];
                const showCurrentQueryControls = Boolean(
                  renderLatestUserMessageActionRow &&
                    event.id &&
                    event.id === latestUserMessageEventId,
                );
                const renderCurrentQueryActionRow = showCurrentQueryControls
                  ? ({ defaultActions }: { defaultActions: ReactNode }) =>
                      renderLatestUserMessageActionRow(defaultActions)
                  : undefined;
                return (
                  <Fragment key={event.id || `event-${item.eventIndex}`}>
                    <div
                    ...
                    >
                      {quotedAssistantMessage?.message ? (
                        ...
                        <CollapsibleUserBubble
                          ...
                          renderActionRow={renderCurrentQueryActionRow}
                        >
                          <UserMessageText
                            text={messageText}
                            integrationMentions={messageIntegrationMentions}
                          ...
                          <div className="chat-bubble-header">
                            {isFailedFollowUpMessage && (
                              <span className="chat-status failed-follow-up">
                                {translate(
                                  "app.toast.followUpFailed.title",
                                  "本轮未完成",
                                )}
                              </span>
                            )}
                            {!isFailedFollowUpMessage && task.status === "completed" && (
                              ...
                              <span className="chat-status">
                                {task.terminalStatus === "awaiting_approval"
                                  ? agentContext.getMessage("taskBlocked") ||
                                    translate("task.status.needsApproval", "Needs approval")
                                  : translate("task.status.waitingInput", "Waiting for your input")}
                              </span>
                            )}
                        ...
                        <div className="message-actions">
                                  ...
                                  <div className="message-feedback-menu">
                                    {(
                                      [
                                        [
                                          "incorrect",
                                          translate("feedback.reason.incorrect", "Incorrect"),
                                        ],
                                        [
                                          "too_verbose",
                                          translate("feedback.reason.tooVerbose", "Too verbose"),
                                        ],
                                        [
                                        ...
                                        [
                                          "wrong_tone",
                                          translate("feedback.reason.wrongTone", "Wrong tone"),
                                        ],
                                        [
                                          "unsafe",
                                          translate("feedback.reason.unsafe", "Unsafe / unwanted"),
                                        ],
                                      ] as const
                                    ).map(([reason, label]) => (
                                      <button
                          ...
                          {isTaskWorking && (
                            <button
                              ...
                              title={translate("feedback.give", "Give feedback")}
                            >
                              <svg
                                width="14"
                                height="14"
...
function areTaskConversationFlowPropsEqual(prev: any, next: any): boolean {
  return (
    ...
    prev.renderLatestUserMessageActionRow === next.renderLatestUserMessageActionRow &&
...
type FocusedWelcomeDaypart = "morning" | "noon" | "afternoon" | "evening" | "lateNight";
...
const FOCUSED_WELCOME_SLIDES = [
  {
    ...
    descriptionFallback: "Agent-powered work, connected to your tools and knowledge.",
  },
  {
    ...
    descriptionFallback: "Bring planning, collaboration, and delivery into one clear flow.",
  },
] as const;
...
function localizeComposerPlanStep(description: string, language: string, fallback: string): string {
  ...
  if (/^(?:collect|catalog|record).*(?:source|url|reference|evidence)/.test(normalized))
    return translate(
  ...
  if (/^(?:generate|create|build).*(?:html|report|file|deliverable)/.test(normalized))
    return /html/.test(normalized)
      ? translate("generated.components.maincontent.maincontent.4661.17", "Generate HTML report")
      : translate(
...
function ComposerProgress({
  ...
  const failedStepCount = planSteps.filter((step) => step.status === "failed").length;
  ...
  const progressPercent =
    planSteps.length > 0 ? Math.round((finishedStepCount / planSteps.length) * 100) : 0;
  const progressLabel =
    failedStepCount > 0
      ? translate(
      ...
      : translate("composer.progress.completedCount", "{completed} of {total} completed", {
          completed: completedStepCount,
          total: planSteps.length,
        });
  ...
  return (
    <div className="composer-progress" ref={rootRef}>
        ...
        >
          ...
          <div className="composer-progress-list" role="list">
            {planSteps.map((step, index) => {
              const fallback = translate("composer.progress.unnamedStep", "Step {number}", {
                number: index + 1,
              });
              const description = localizeComposerPlanStep(step.description, language, fallback);
              const statusLabel = getComposerPlanStepStatusLabel(step.status);
              return (
                <div
                  key={step.id || `${index}-${description}`}
                  className={`composer-progress-item ${step.status}`}
                  title={description}
                  role="listitem"
                ...
                >
                  ...
                  <span className="composer-progress-description">{description}</span>
                  {statusLabel ? (
                    <span className="composer-progress-status">{statusLabel}</span>
                  ) : null}
                </div>
              );
            })}
      ...
      <button
        ...
        aria-label={translate("composer.progress.toggle", "Show task progress: {progress}", {
...
function getInitialInlineOnboardingStage(): InlineOnboardingStage {
  try {
    return window.sessionStorage.getItem(INLINE_ONBOARDING_STAGE_KEY) === "model"
      ? "model"
      : "profile";
  } catch {
    return "profile";
...
function InlineOnboardingCard({
  ...
  const [stage, setStage] = useState<InlineOnboardingStage>(getInitialInlineOnboardingStage);
  const [preferredName, setPreferredName] = useState("");
  const [workContext, setWorkContext] = useState("");
  const [preferredNameFactId, setPreferredNameFactId] = useState<string | null>(null);
  const [workContextFactId, setWorkContextFactId] = useState<string | null>(null);
    ...
    void Promise.all([
      ...
      .then(([personality, profile]) => {
        ...
        const nameFact = facts.find(
          (fact) => fact.category === "identity" && /^Preferred name:\s*/i.test(fact.value),
        );
        const contextFact = facts.find(
          (fact) =>
            fact.category === "work" && /^(?:Work context|工作背景)[:：]\s*/i.test(fact.value),
        );
        const rememberedName = nameFact?.value.replace(/^Preferred name:\s*/i, "").trim() || "";
        const rememberedContext =
          contextFact?.value.replace(/^(?:Work context|工作背景)[:：]\s*/i, "").trim() || "";

        setPreferredName(personality.relationship?.userName?.trim() || rememberedName);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      if (normalizedContext) {
        const contextFactValue = translate("profile.workContextFact", "Work context: {context}", {
      ...
      >
        <div className="inline-onboarding-copy">
          ...
          <h2 id="inline-onboarding-title">
            {translate("onboarding.inline.profileTitle", "First, tell NeoWorker about you")}
          </h2>
          <p>
            {translate(
        ...
        <div className="inline-onboarding-form">
          <label>
            <span>
              {translate("onboarding.inline.nameLabel", "What should NeoWorker call you?")}
            </span>
            <input
              type="text"
              value={preferredName}
              onChange={(event) => setPreferredName(event.target.value)}
              placeholder={translate("onboarding.inline.namePlaceholder", "Your name or nickname")}
              autoComplete="name"
              maxLength={80}
          ...
          <label>
            <span>
              ...
              <small>{translate("onboarding.inline.optional", "Optional")}</small>
            </span>
            <textarea
              value={workContext}
          ...
          <div className="inline-onboarding-actions">
            ...
            <button
              ...
              disabled={loadingProfile || savingProfile || !preferredName.trim()}
            >
              {savingProfile
                ? translate("onboarding.inline.saving", "Saving...")
                : translate("onboarding.inline.saveAndContinue", "Save and continue")}
            </button>
          </div>
        </div>
      </section>
  ...
  return (
    <section className="inline-onboarding-card" aria-labelledby="inline-onboarding-title">
      <div className="inline-onboarding-copy">
        <span className="inline-onboarding-kicker">
          {translate("onboarding.inline.kicker", "Quick start")}
        </span>
        <h2 id="inline-onboarding-title">
          {translate("onboarding.inline.modelTitle", "Next, configure your model")}
        </h2>
        <p>
          {translate(
      ...
      <div className="inline-onboarding-actions">
        ...
        <button type="button" className="button-primary" onClick={onOpenModelSettings}>
...
function MainContentComponent({
  ...
  const composerDraftValueRef = useRef(composerDraftCache.get(composerDraftCacheKey) || "");
  const [inputValue, setInputValue] = useState(() => composerDraftValueRef.current);
  ...
  const initialAttachmentDraft = composerAttachmentDraftCache.get(composerDraftCacheKey) || [];
  const pendingAttachmentsRef = useRef<PendingAttachment[]>(initialAttachmentDraft);
  const [pendingAttachments, setPendingAttachmentsState] = useState<PendingAttachment[]>(
  ...
  useLayoutEffect(() => {
    ...
    cacheComposerDraft(composerDraftCacheKeyRef.current, composerDraftValueRef.current);
    cacheComposerAttachmentDraft(composerDraftCacheKeyRef.current, pendingAttachmentsRef.current);
    composerDraftCacheKeyRef.current = composerDraftCacheKey;
    const nextDraft = composerDraftCache.get(composerDraftCacheKey) || "";
    const nextAttachments = composerAttachmentDraftCache.get(composerDraftCacheKey) || [];
  ...
  const [isFocusedWelcomeCopySwitching, setIsFocusedWelcomeCopySwitching] = useState(false);
  ...
  const [welcomeVaultExists, setWelcomeVaultExists] = useState<boolean | null>(null);
  ...
  const focusedWelcomeGreeting = useMemo(() => {
    ...
    return translate(key, FOCUSED_WELCOME_GREETING_FALLBACKS[daypart][variant], {
      name: userName,
    });
  }, [agentContext.userName, language, welcomeClock]);
  const focusedWelcomeCopySlide =
    FOCUSED_WELCOME_SLIDES[focusedWelcomeCopyIndex] ?? FOCUSED_WELCOME_SLIDES[0];
  const focusedWelcomeTitle =
    focusedWelcomeCopySlide.titleKey == null
      ? focusedWelcomeGreeting
      : translate(focusedWelcomeCopySlide.titleKey, focusedWelcomeCopySlide.titleFallback);
  ...
  useEffect(() => {
    const intervalId = window.setInterval(() => setWelcomeClock(new Date()), 60_000);
  ...
  const composerPermissionOverrides = buildComposerPermissionOverrides(
    permissionAccessMode,
    shellEnabled,
  );
  ...
  const applyComposerModeSelection = useCallback((selection: ComposerModeSelection) => {
    ...
    setChatModeUpgradePrompt(false);
    if (selection === "chat") {
      setComposerSkillContext(null);
  ...
  const [preferredSlashSkillIds, setPreferredSlashSkillIds] =
    useState<string[]>(readRecentSlashSkillIds);
  ...
  }, [shouldLoadLlmWikiVaultSummary, workspace?.id, workspace?.isTemp, workspace?.path]);
  ...
  const filteredEvents = useMemo(() => {
    ...
    return measureRendererPerf("MainContent.filteredEvents", rendererPerfLoggingEnabled, () => {
      ...
      return dedupedEvents.filter((event) => {
        if (includeExecutionRecordEvents && shouldRevealInternalAssistantMessageInVerbose(event)) {
          return true;
        }
    ...
    for (let i = events.length - 1; i >= 0; i--) {
      ...
      if (e.type === "timeline_step_started" || e.type === "timeline_step_updated") {
        ...
        const rawDescription =
          ...
          "Working";
        const description =
          rawDescription === "Thinking..." ? rawDescription : localizeProgressText(rawDescription);
  ...
  // Build command sessions once; visibility is controlled separately by the
  // execution-record switch.
  ...
  const visibleCommandOutputSessions = useMemo(
    () =>
      selectVisibleCommandOutputSessions({
        sessions: commandOutputSessions,
        verboseSteps: verboseSteps && !isChatTask,
        dismissedSessionIds: dismissedCommandOutputs,
      }),
    [commandOutputSessions, dismissedCommandOutputs, isChatTask, verboseSteps],
  );
  ...
  const loadMessageShortcuts = useCallback(async () => {
    ...
    if (import.meta.env.DEV && typeof window.electronAPI.reloadCustomSkills === "function") {
    ...
    const enabledSkills = skills.filter(
      (skill) => skill.enabled !== false && isSkillVisibleForCurrentProductSupport(skill),
    );
  ...
  const loadIntegrationMentionOptions = useCallback(async () => {
    ...
    const nextOptions = (Array.isArray(options) ? options : []).filter(
      (option) =>
        ...
        (option.id !== INBOX_AGENT_MENTION_ID && option.providerKey !== "inbox-agent"),
    );
    startTransition(() => {
  ...
  const normalizedRoleIndex = useMemo(() => {
    ...
    for (const role of agentRoles) {
      ...
      const haystack = normalizeMentionSearch(
        `${localizedRole.name} ${localizedRole.description} ${role.displayName} ${role.name} ${role.description ?? ""}`,
      );
  ...
  const renderCommandOutputs = useCallback(
    (sessions: CommandOutputSession[] | undefined) => {
      if (!verboseSteps || !sessions || sessions.length === 0) return null;
    ...
    [handleDismissCommandOutput, task?.id, verboseSteps],
  );
  ...
  const renderWelcomeOverflowSubmenu = () => {
    ...
    const optionsLabel = translate("composer.combinedModeOptions", "Work mode options");

    return (
      <div className="overflow-submenu-panel" role="menu" aria-label={optionsLabel}>
        <div className="overflow-submenu-header">
          <span className="overflow-submenu-title">{title}</span>
        </div>
        {COMPOSER_MODE_ORDER.map((value) => {
          const label = translate(`composer.combinedMode.${value}`, COMPOSER_MODE_LABEL[value]);
      ...
      <div className="input-status-mode-wrap" ref={modeDropdownRef}>
        ...
        {showModeDropdown && (
          <div
            ...
            role="listbox"
            aria-label={translate("composer.combinedModeOptions", "Work mode options")}
          >
            {COMPOSER_MODE_ORDER.map((value) => {
              ...
              return (
                ...
                >
                  ...
                  {translate(`composer.combinedMode.${value}`, COMPOSER_MODE_LABEL[value])}
                </button>
              );
            })}
          </div>
  ...
  useEffect(() => {
    ...
    setComposerSkillContext(
      composerDraftRequest.skillId
        ? {
            ...
            skillLabel: composerDraftRequest.skillLabel?.trim() || composerDraftRequest.skillId,
          }
        : null,
    );
    setIntegrationMentionSpans([]);
    ...
    const frame = requestAnimationFrame(() => {
      ...
      input.setSelectionRange(composerDraftRequest.value.length, composerDraftRequest.value.length);
      onComposerDraftConsumed?.(composerDraftRequest.id);
    });
  ...
  const showCliPlaceholder = !inputValue && !isCliInputFocused && !isPromptComposing;
  const showCliEmptyCursor = false;
  const placeholderPlaylist = useMemo(
    () =>
      (rotatingPlaceholders.length > 0 ? rotatingPlaceholders : [personalityPlaceholder]).map(
        (phrase, index) => translate(`welcome.placeholder.${index % 8}`, phrase),
      ),
  ...
  useLayoutEffect(() => {
    ...
    restore();
    resizeObserver = typeof ResizeObserver !== "undefined" ? new ResizeObserver(restore) : null;
    if (!cancelled) {
      resizeObserver?.observe(container);
      const content = container.querySelector<HTMLElement>(".task-content");
      if (content) resizeObserver?.observe(content);
    ...
    const processingCopy =
      composerProcessingStage === "importing"
        ...
        : composerProcessingStage === "reading"
          ? {
              title: translate("composer.attachmentProgress.reading", "Reading file contents..."),
              detail: translate(
        ...
        {pendingAttachments.length > 0 && (
          <div className="attachment-list">
            {pendingAttachments.map((attachment) => {
              ...
              if (isPreviewableImageAttachment(attachment.name, attachment.mimeType)) {
                return (
                  <AttachmentImagePreview
                    key={attachment.id}
                    name={attachment.name}
                    ...
                    removeDisabled={isUploadingAttachments}
                  />
                );
              }
              return (
                <div className="attachment-chip composer-attachment-chip" key={attachment.id}>
                  <ArtifactFileTypeIcon
                    filePath={attachment.name}
                  ...
                  <button
                    ...
                    aria-label={translate("composer.removeAttachment", "Remove attachment")}
        ...
        {processingCopy && (
          <div className="attachment-processing" role="status" aria-live="polite">
            ...
            <span className="attachment-processing-steps" aria-hidden="true">
              ...
              <i
                className={composerProcessingStage !== "importing" ? "is-complete" : "is-active"}
              />
              <i className={composerProcessingStage === "creating" ? "is-active" : ""} />
            </span>
          </div>
        )}
      </div>
    ...
    return (
      <div className="composer-capability-row">
        <div
          ...
          aria-label={translate("composer.selectedCapability", "Selected capability")}
        >
          ...
          <button
            ...
            aria-label={translate("composer.removeCapability", "Remove capability")}
          >
  ...
  const handleSend = async () => {
    ...
    const liveInputValue = liveDraftSnapshot?.value ?? composerDraftValueRef.current;
    const submittedIntegrationMentionSpans = liveDraftSnapshot?.mentions ?? integrationMentionSpans;
    const submittedIntegrationMentions = Array.from(
      new Map(
        submittedIntegrationMentionSpans.map((span) => [span.mention.id, span.mention]),
      ).values(),
    );
    ...
    if (
      ...
      shouldSuggestExecuteForChat(trimmedInput)
    ) {
      setChatModeUpgradePrompt(true);
      setAttachmentError(null);
      return;
    }
    if (
    ...
    const inboxAskQuery = extractInboxAskQuery(liveInputValue, submittedIntegrationMentionSpans);
      ...
      if (shouldCreateFreshTask && onCreateTask) {
        ...
        const modeOptions: CreateTaskOptions = {
          ...
          ...(executionModeDirty || clarifyingCheckinsEnabled || Boolean(composerSkillContext)
            ? {
                agentConfig: {
                  ...(executionModeDirty ? { executionModeSource: "user" as const } : {}),
  ...
  const mentionOptions = useMemo<MentionOption[]>(() => {
    ...
    filteredAgents.forEach((role) => {
      ...
      options.push({
        type: "agent",
        id: role.id,
        label: localizedRole.name,
        description: localizedRole.description,
        icon: role.icon,
        color: role.color,
        portraitSrc: roleVisual.kind === "portrait" ? roleVisual.src : undefined,
      });
    });
  ...
  const slashOptions = useMemo<SlashMenuOption[]>(() => {
    ...
    const visibleSkillIds = new Set(
      visibleOptions.filter((option) => option.kind === "skill").map((option) => option.id),
    );
  ...
  }, [effectiveSlashSelectedIndex, slashOpen, slashOptions.length, slashSelectedIndex]);
  ...
  const updateMentionState = useCallback((value: string, cursor: number | null) => {
    ...
    const shouldResetSelection = !mentionOpenRef.current || queryChanged || targetChanged;
  ...
  const updateSlashState = useCallback((value: string, cursor: number | null) => {
    ...
    const targetChanged =
      !previousTarget || previousTarget.start !== slash.start || previousTarget.end !== slash.end;
    const shouldResetSelection = !slashOpenRef.current || queryChanged || targetChanged;
    ...
    if (option.kind === "skill") {
      setPreferredSlashSkillIds((current) => {
        const next = [
          option.skill.id,
          option.commandName,
          ...current.filter((id) => id !== option.skill.id && id !== option.commandName),
        ].slice(0, 20);
        try {
          window.localStorage.setItem(RECENT_SLASH_SKILLS_STORAGE_KEY, JSON.stringify(next));
        } catch {
  ...
  const handleInputChange = (
    ...
    if (chatModeUpgradePrompt && !shouldSuggestExecuteForChat(value)) {
      setChatModeUpgradePrompt(false);
    }
    ...
    return (
      <div className="mode-suggestion-bar">
        {modeSuggestions.map((s) => {
          const localizedLabel = translate(`modeSuggestion.mode.${s.mode}`, s.label);
          return (
            <button
              key={s.mode}
              className="mode-suggestion-pill"
              onClick={() => handleModeSuggestionClick(s)}
              title={translate(`modeSuggestion.description.${s.mode}`, s.description)}
            >
  ...
  const renderChatModeUpgradePrompt = () => {
    if (!chatModeUpgradePrompt || composerModeSelection !== "chat") return null;

    return (
      <div className="chat-mode-upgrade-prompt" role="status">
        <MessageCircle size={17} aria-hidden="true" />
        <div className="chat-mode-upgrade-copy">
          <strong>
            {translate("composer.chatUpgrade.title", "This request needs Execute mode")}
          </strong>
          <span>
            {translate(
              "composer.chatUpgrade.detail",
              "Chat cannot write files, run code, or operate apps. Switch modes before sending; your draft will be preserved.",
            )}
          </span>
        </div>
        <button
          type="button"
          className="chat-mode-upgrade-action"
          onClick={() => {
            applyComposerModeSelection("execute");
            requestAnimationFrame(() => promptInputRef.current?.focus());
          }}
        >
          {translate("composer.chatUpgrade.switch", "Switch to Execute")}
        </button>
        <button
          type="button"
          className="chat-mode-upgrade-dismiss"
          onClick={() => setChatModeUpgradePrompt(false)}
          title={translate("composer.chatUpgrade.dismiss", "Keep editing")}
          aria-label={translate("composer.chatUpgrade.dismiss", "Keep editing")}
        >
      ...
      return (
        ...
        >
          {isIntegration ? (
            ...
            <span className="mention-autocomplete-icon mention-autocomplete-agent-avatar">
              <img src={option.portraitSrc} alt="" aria-hidden="true" />
            </span>
          ) : (
            <span
    ...
    return (
      <div className="mention-autocomplete-dropdown" ref={mentionDropdownRef}>
        {agentOptions.length > 0 && (
          <div className="mention-autocomplete-section">
            <div className="mention-autocomplete-section-label">
              <span>{translate("mentions.agents", "Agents")}</span>
              <span className="mention-autocomplete-section-count">{agentOptions.length}</span>
            </div>
        ...
        {integrationOptions.length > 0 && (
          <div className="mention-autocomplete-section">
            <div className="mention-autocomplete-section-label">
              <span>{translate("mentions.integrations", "Integrations")}</span>
              <span className="mention-autocomplete-section-count">
        ...
        {slashOptions.map((option, index) => {
            ...
            return (
              ...
              >
                <span className="slash-autocomplete-more-copy">
                  <strong>{translate("slash.menu.moreSkills", "Show more skills")}</strong>
                  <span>
          ...
          return (
            <Fragment key={option.id}>
              {group !== previousGroup && (
                <div className="slash-autocomplete-section-label" role="presentation">
                  {group === "skills"
                ...
                <div className="mention-autocomplete-details">
                  <div className="slash-autocomplete-name-row">
                    <span className="mention-autocomplete-name">
                      {option.kind === "skill" ? option.name : `/${option.commandName}`}
                    </span>
                    {option.kind === "skill" && (
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (slashOpen && slashOptions.length > 0) {
      switch (e.key) {
        case "ArrowDown":
          e.preventDefault();
          setSlashSelectedIndex((effectiveSlashSelectedIndex + 1) % slashOptions.length);
          return;
        case "ArrowUp":
          e.preventDefault();
          setSlashSelectedIndex(
            (effectiveSlashSelectedIndex - 1 + slashOptions.length) % slashOptions.length,
          );
          return;
        case "Enter":
        case "Tab":
  ...
  const handleOutcomeTemplateStart = useCallback(
    (template: OutcomeTemplate, connectors: OutcomeTemplateConnector[]) => {
      const prompt = translate(`outcomeTemplates.${template.id}.prompt`, template.prompt);
      ...
      window.setTimeout(() => {
        promptInputRef.current?.focus();
        promptInputRef.current?.setSelectionRange(draft.text.length, draft.text.length);
      }, 0);
    },
    [language],
  );
  ...
  const handleAddWelcomeSuggestionToTaskCenter = (suggestion: WelcomeTaskSuggestion) => {
      ...
      >
        <div className="welcome-next-actions-header">
          ...
          <h2 className="welcome-next-actions-title">
            {translate("welcome.nextActions", "What deserves your attention now?")}
          </h2>
              ...
              >
                ...
                <div className="welcome-focus-card-body" title={title}>
                  <span className="welcome-focus-card-origin">
                    {translate("welcome.focusOrigin", "From the clues you left recently")}
                  </span>
                  ...
                  {signals.length ? (
                    <div className="welcome-focus-card-signals">
                      <span>
                        {translate("welcome.focusEvidence", "it refers to this information")}
                      </span>
                      <ol>
                        {signals.map((signal, signalIndex) => (
                          <li key={`${suggestion.id}-signal-${signalIndex}`} title={signal}>
                ...
                <div className="welcome-focus-card-dock">
                  <div className="welcome-focus-card-utilities">
                    <button
                      ...
                      aria-label={translate("welcome.snoozeSuggestion", "Snooze {title}", {
                        title: suggestion.title,
                      })}
                      title={translate("welcome.snoozeForDay", "Snooze for a day")}
                    >
                    ...
                    <button
                      ...
                      aria-label={translate("welcome.dismissSuggestion", "Dismiss {title}", {
                        title: suggestion.title,
                      })}
                      title={translate("common.dismiss", "Dismiss")}
                    >
                  ...
                  {suggestion.action.type === "prompt" ? (
                    <>
                      <div className="welcome-focus-card-result">
                        <span>
                          {translate("welcome.focusResultLabel", "NeoWorker will give you")}
                        </span>
                        <strong>
                          {translate("welcome.focusResultTitle", "3 practical directions")}
                        </strong>
                        <small>
                          {translate(
                      ...
                      <div className="welcome-focus-card-actions">
                        ...
                        >
                          ...
                          {translate("welcome.generateOptions", "Help me choose")}
                        </button>
                        <button
                          type="button"
                          className="welcome-focus-card-secondary"
                          onClick={() => handleAddWelcomeSuggestionToTaskCenter(suggestion)}
                        >
                          <ListTodo size={15} aria-hidden="true" />
                          {translate("welcome.addToTaskCenter", "Save for later")}
                        </button>
                      </div>
                    </>
                  ) : (
      ...
      >
        ...
        >
          <button
            ...
            onClick={() => handleQuickAction(translate("vault.prompt.ingest", LLM_WIKI_GUI_PROMPT))}
          >
          ...
          <button
            ...
            onClick={() =>
              handleQuickAction(translate("vault.prompt.query", LLM_WIKI_QUERY_GUI_PROMPT))
            }
          >
          ...
          <button
            ...
            className="vault-browser-action"
            onClick={() =>
              handleQuickAction(translate("vault.prompt.audit", LLM_WIKI_AUDIT_GUI_PROMPT))
            }
          >
          ...
          <button
            ...
            className="vault-browser-action"
            onClick={() =>
              handleQuickAction(translate("vault.prompt.explore", LLM_WIKI_EXPLORE_GUI_PROMPT))
            }
          >
          ...
          <button
            ...
            className="vault-browser-action"
            onClick={() =>
              handleQuickAction(translate("vault.prompt.brief", LLM_WIKI_BRIEF_GUI_PROMPT))
            }
          >
        ...
        {llmWikiVaultLoading ? (
          <div className="vault-browser-loading">
            <Loader2 size={17} className="vault-browser-loading-icon" aria-hidden="true" />
            <span>{translate("vault.loading", "Loading vault summary...")}</span>
          </div>
        ) : summary?.exists ? (
          <>
            <div
            ...
            <div className="vault-browser-groups">
              ...
              {renderVaultEntryGroup(
                ...
                translate("vault.empty.recentOutputs", "No slide decks or charts yet."),
              )}
              {renderVaultEntryGroup(
                translate("vault.group.recentRawCaptures", "Recent raw captures"),
                summary.recentRawSources,
                translate("vault.empty.recentRawCaptures", "No raw source captures yet."),
              )}
            </div>
          </>
        ) : (
            ...
            <div className="vault-browser-empty-message">
              ...
              <div>
                <div className="vault-browser-empty-title">
                  {translate("vault.empty.title", "Start with your first source")}
                </div>
  ...
  const conversationTurnItems = useMemo<ConversationTurnNavItem[]>(() => {
    ...
    return turns;
  }, [cleanedDisplayPrompt, events, initialPromptEventId, task?.createdAt, trimmedPrompt]);
  const [activeConversationTurnId, setActiveConversationTurnId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const updateActiveTurn = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        const readingLine = containerRect.top + Math.min(container.clientHeight * 0.28, 220);
        const anchors = Array.from(
          container.querySelectorAll<HTMLElement>("[data-conversation-turn-id]"),
        ).filter((element) => validTurnIds.has(element.dataset.conversationTurnId || ""));
        let nextActiveId =
          anchors[0]?.dataset.conversationTurnId ?? conversationTurnItems[0]?.id ?? null;
    ...
    const content = container.querySelector<HTMLElement>(".task-content");
    const resizeObserver =
      typeof ResizeObserver !== "undefined" ? new ResizeObserver(updateActiveTurn) : null;
    resizeObserver?.observe(container);
    if (content) resizeObserver?.observe(content);
    updateActiveTurn();
  ...
  const handleSelectConversationTurn = useCallback((turnId: string) => {
    ...
    container
      ...
      .forEach((element) => element.classList.remove("conversation-turn-jump-highlight"));
  ...
  useEffect(() => {
    const handleNavigationRequest = (event: Event) => {
      const detail = (event as CustomEvent<ConversationTurnNavigationDetail>).detail;
      if (!detail || detail.taskId !== task?.id || !detail.turnId) return;
      handleSelectConversationTurn(detail.turnId);
    };

    window.addEventListener(CONVERSATION_TURN_NAVIGATION_EVENT, handleNavigationRequest);
    return () => {
      window.removeEventListener(CONVERSATION_TURN_NAVIGATION_EVENT, handleNavigationRequest);
      if (conversationTurnHighlightTimerRef.current !== null) {
        window.clearTimeout(conversationTurnHighlightTimerRef.current);
        conversationTurnHighlightTimerRef.current = null;
      }
  ...
  const pendingApprovalSummary = useMemo(() => {
    ...
    for (let i = events.length - 1; i >= 0; i -= 1) {
      ...
      if (eventType === "approval_requested" && event.payload?.autoApproved !== true) {
        const approval = event.payload?.approval;
        const approvalId =
          approval && typeof approval === "object" && typeof approval.id === "string"
            ? approval.id
            : "";
  ...
  const showTimelineControls = shouldShowTimelineControls({
      ...
      <div className={`timeline-controls-actions ${extraClassName}`.trim()}>
        ...
        {replayControls &&
          ...
          (task?.status === "completed" ||
            ...
            <button
              ...
              title={translate("composer.replayTitle", "Replay this session step by step")}
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
        ...
        {replayControls?.isReplayMode && !replayControls.areControlsVisible && (
          <button
            ...
            title={translate("composer.replayControlsTitle", "Show replay controls")}
          >
  ...
  const timelineControlsPlacement = resolveTimelineControlsPlacement({
    showTimelineControls,
    hasInitialPrompt: Boolean(trimmedPrompt),
    hasUserFollowUp: hasUserFollowUpAfterInitialPrompt,
  });
  const inlineTimelineControlsWithFollowUp = timelineControlsPlacement === "latest-query";
  const inlineTimelineControlsWithPrompt = timelineControlsPlacement === "initial-query";
  const renderLatestUserMessageActionRow = useCallback(
    (defaultActions: ReactNode) => (
      <div className="prompt-task-control-row current-query-control-row">
        {renderTimelineControlsStatus()}
        <div className="prompt-task-control-actions">
          {defaultActions}
          {renderTimelineControlsActions("timeline-controls-actions-inline")}
        </div>
      </div>
    ),
    [renderTimelineControlsActions, renderTimelineControlsStatus],
  );
  const initialPromptBubble = useMemo(() => {
    if (!trimmedPrompt) return null;
    const initialIntegrationMentions = task?.agentConfig?.integrationMentions;
    return (
      <div className="chat-message user-message" data-conversation-turn-id="initial">
        <CollapsibleUserBubble
              ...
              <div className="welcome-header-focused modern-only">
                ...
                >
                  ...
                  >
                    ...
                    <p className="focused-subtitle">{focusedWelcomeDescription}</p>
                  </div>
                </div>
              </div>
            ) : (
 ...
 ╚██████╗╚██████╔╝╚███╔███╔╝╚██████╔╝██║  ██║██║  ██╗     ╚██████╔╝███████║
              ...
              {uiDensity === "focused" && !outcomeTemplatesEnabled && (
                <div className="focused-cards-section-header">
                  ...
                  <button
                    ...
                    aria-label={translate("welcome.changeCardGroup", "Show another set")}
                  >
                    <RefreshCw size={13} strokeWidth={1.8} aria-hidden="true" />
                    <span>{translate("welcome.changeCardGroup", "Show another set")}</span>
                  </button>
                </div>
              )}
              {uiDensity !== "focused" && (
              ...
              {outcomeTemplatesEnabled ? (
                <OutcomeTemplateGrid
                  ...
                  onConfigureWorkspace={
                    projectId ? () => onOpenProject?.(projectId) : onChangeWorkspace
                  }
                  ...
                  {visibleFocusedCards.map((card) => {
                    ...
                    const handleClick = () => {
                      if (card.action.type === "prompt") {
                        handleQuickAction(
                          translate(`welcome.card.${card.id}.prompt`, card.action.prompt),
                        );
                      } else {
                        onOpenSettings?.(card.action.tab);
                      }
                    };
                    const cardTitle = translate(`welcome.card.${card.id}.title`, card.title);
                    const cardDesc = translate(`welcome.card.${card.id}.desc`, card.desc);
                    return (
                      <button
                        key={card.id}
                ...
                <div className="quick-start-grid">
                  ...
                  >
                    ...
                    <span className="quick-start-title">
                      {translate("welcome.quick.organizeFiles.title", "Organize files")}
                    </span>
                  ...
                  >
                    ...
                    />
                    <span className="quick-start-title">
                      {translate("welcome.quick.writeTogether.title", "Write together")}
                    </span>
                    <span className="quick-start-desc">
                      {translate(
                        "welcome.quick.writeTogether.desc",
                  ...
                  >
                    ...
                    />
                    <span className="quick-start-title">
                      {translate("welcome.quick.analyzeData.title", "Analyze data")}
                    </span>
                    <span className="quick-start-desc">
                      {translate(
                        "welcome.quick.analyzeData.desc",
                  ...
                  >
                    ...
                    />
                    <span className="quick-start-title">
                      {translate("welcome.quick.generateDocs.title", "Generate docs")}
                    </span>
                    <span className="quick-start-desc">
                      {translate(
                        "welcome.quick.generateDocs.desc",
                  ...
                  >
                    ...
                    />
                    <span className="quick-start-title">
                      {translate("welcome.quick.researchCompetitors.title", "Research competitors")}
                    </span>
                    <span className="quick-start-desc">
                      {translate(
                        "welcome.quick.researchCompetitors.desc",
                  ...
                  >
                    ...
                    />
                    <span className="quick-start-title">
                      {translate("welcome.quick.validateIdea.title", "Validate an idea")}
                    </span>
                    <span className="quick-start-desc">
                      {translate(
                        "welcome.quick.validateIdea.desc",
            ...
            >
              {showVoiceNotConfigured && (
                <div className="voice-not-configured-banner">
                  ...
                  <span>
                    {translate("composer.voice.notConfigured", "Voice input is not configured.")}
                  </span>
                  <button
                  ...
                  >
                    {translate("composer.voice.openSettings", "Open Voice Settings")}
                  </button>
                  <button
                ...
                <div className="input-left-actions">
                  ...
                  {composerModeSelection !== "chat" && (
                    ...
                    <button
                      ...
                      aria-label={translate("composer.permissionMode", "Permission access mode")}
                      title={
                        permissionAccessMode === "full"
                          ? translate("composer.fullAccess", "Full access")
                          : translate("composer.defaultPermissions", "Default permissions")
                      }
                    >
                      {permissionAccessMode === "full" ? (
                    ...
                    {showPermissionDropdown && (
                      ...
                      >
                        ...
                        >
                          ...
                          <span>{translate("composer.fullAccess", "Full access")}</span>
                        </button>
                        <div className="permission-access-divider" role="separator" />
                        <button
                          type="button"
                          className={`permission-access-option permission-command-option ${
                            shellEnabled || permissionAccessMode === "full" ? "active" : ""
                          }`}
                          onClick={() => {
                          ...
                          aria-checked={shellEnabled || permissionAccessMode === "full"}
                          disabled={permissionAccessMode === "full"}
                        >
                          <Terminal size={16} aria-hidden="true" />
                          <span>
                            {translate("composer.allowCommands", "Allow command execution")}
                          </span>
                          <span
                            className={`goal-mode-switch-track ${
                              shellEnabled || permissionAccessMode === "full" ? "on" : ""
                            }`}
                            aria-hidden="true"
                          >
                            <span className="goal-mode-switch-thumb" />
                    ...
                    <>
                        ...
                        <button className="folder-selector" onClick={handleWorkspaceDropdownToggle}>
                          ...
                          <span>
                            {workspace?.isTemp || isTempWorkspaceId(workspace?.id)
                              ? translate("composer.workInFolder", "Work in a folder")
                              : workspace?.name ||
                                translate("composer.workInFolder", "Work in a folder")}
                          </span>
                          <svg
                      ...
                      <div className="overflow-menu-container" ref={overflowMenuRef}>
                        <button
                          ...
                          title={translate("composer.moreOptions", "More options")}
                          aria-label={translate("composer.moreOptions", "More options")}
                          aria-haspopup="menu"
                            ...
                            <div className="overflow-menu-item" role="none">
                              ...
                              >
                                <span className="goal-mode-toggle-switch-content">
                                  <span className="goal-mode-toggle-text">
                                    <span className="goal-mode-label">
                                      {translate("composer.autonomous", "Autonomous")}
                                    </span>
                                  </span>
                                  <span
                                    className={`goal-mode-switch-track ${
                              ...
                              <div className="overflow-menu-item" role="none">
                                ...
                                >
                                  <span className="goal-mode-toggle-switch-content">
                                    <span className="goal-mode-toggle-text">
                                      <span className="goal-mode-label">
                                        {translate("composer.multiLlm", "Multi-LLM")}
                                      </span>
                                    </span>
                                    <span
                                      className={`goal-mode-switch-track ${
                    ...
                    <>
                      {(executionMode !== "execute" ||
                        ...
                        <button
                          ...
                          title={translate("composer.resetMode", "Click to reset mode")}
                          onClick={() => {
                        ...
                        title={
                          voiceInput.state === "idle"
                            ...
                            : voiceInput.state === "recording"
                              ...
                              : translate("composer.voice.processing", "Processing...")
                        }
                      >
                          ...
                          {showSkillsMenu && (
                            <div className="skills-dropdown">
                              ...
                              {customSkills.length > 0 ? (
                                ...
                                <div className="skills-dropdown-empty">
                                  {translate("composer.noCustomSkills", "No custom skills yet.")}
                                </div>
                              )}
                              <div className="skills-dropdown-footer">
                                ...
                                >
                                  ...
                                  <span>
                                    {translate("composer.createNewSkill", "Create New Skill")}
                                  </span>
                                </button>
                              </div>
                            </div>
                      ...
                      {showSkillsMenu && (
                        <div className="skills-dropdown">
                          ...
                          <div className="skills-dropdown-search">
                            ...
                            <input
                              type="text"
                              placeholder={translate("composer.searchSkills", "Search skills...")}
                              value={skillsSearchQuery}
                              onChange={(e) => setSkillsSearchQuery(e.target.value)}
                              autoFocus
                            />
                          ...
                          {customSkills.length > 0 ? (
                            filteredSkills.length > 0 ? (
                              ...
                              <div className="skills-dropdown-empty">
                                {translate("composer.noSkillsMatch", 'No skills match "{query}"', {
                                  query: skillsSearchQuery,
                                })}
                              </div>
                            )
  ...
  const conversationFlow = (
    <TaskConversationFlow
      ...
      renderLatestUserMessageActionRow={
        inlineTimelineControlsWithFollowUp
          ? renderLatestUserMessageActionRow
          : undefined
      }
      onSelectChildTask={onSelectChildTask}
      ...
      <div className="main-header">
        {(task?.parentTaskId || task?.branchFromTaskId) && onSelectTask && (
          <button
            ...
            title={translate("task.header.backToParent", "Back to parent thread")}
            aria-label={translate("task.header.backToParent", "Back to parent thread")}
          >
            <MessageCircle size={14} strokeWidth={1.5} />
            <span>{translate("task.header.parentThread", "Parent thread")}</span>
          </button>
        )}
              ...
              {showTaskHeaderMenu && (
                ...
                >
                  ...
                  >
                    ...
                    <span>{translate("task.actions.rename", "Rename task")}</span>
                  </button>
                  <button
                    type="button"
                    className="main-header-task-menu-item"
                  ...
                  >
                    ...
                    <span>{translate("task.actions.archive", "Archive task")}</span>
                  </button>
                  ...
                  >
                    ...
                    <span>{translate("task.actions.openBrowser", "Open browser")}</span>
                  </button>
                  <button
                    type="button"
                    className="main-header-task-menu-item"
                  ...
                  >
                    ...
                    <span>
                      {translate("task.actions.copyWorkingDirectory", "Copy working directory")}
                    </span>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span>{translate("task.actions.copyTaskId", "Copy task ID")}</span>
                  </button>
                  <button
                    type="button"
                    className="main-header-task-menu-item"
                  ...
                  >
                    ...
                    <span>{translate("task.actions.copyDeeplink", "Copy deeplink")}</span>
                  </button>
                  <button
                    type="button"
                    className="main-header-task-menu-item"
                  ...
                  >
                    ...
                    <span>{translate("task.actions.copyMarkdown", "Copy as Markdown")}</span>
                  </button>
                  <div className="main-header-task-menu-divider" role="separator" />
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span>{translate("task.actions.forkSession", "Fork session")}</span>
                  </button>
                  <button
                    type="button"
                    className="main-header-task-menu-item"
                  ...
                  >
                    ...
                    <span>{translate("task.actions.createRoutine", "Create routine...")}</span>
                  </button>
                  {hasTaskOutputs(taskOutputSummary) && onViewTaskOutputs && (
                    ...
                    >
                      ...
                      <span>{translate("task.actions.viewOutputs", "View outputs")}</span>
                    </button>
                  )}
                </div>
              )}
        ...
        >
          ...
          {routineCreationNotice?.taskId === task.id && (
            <div className="task-automation-created-response" role="status">
              <div>
                <strong>{translate("task.routine.created", "Routine created")}</strong>
                <span>
                  {translate("task.routine.createdDetail", "{name} is saved with {summary}.", {
          ...
          {remoteSession && (
            <div className="task-status-banner task-status-banner-remote">
              <div className="task-status-banner-content">
                <strong>{translate("task.remoteSession.title", "Remote session view")}</strong>
                ...
                <strong>
                  {task.terminalStatus === "awaiting_approval"
                    ? pendingApprovalSummary.count > 1
                      ...
                      : translate("task.status.blockedNeedsApproval", "Blocked - needs approval")
                    : translate("task.status.blockedWaiting", "Blocked - waiting on you")}
                </strong>
                {latestPendingApproval?.description && (
                  <span className="task-status-banner-detail">
                    {localizeApprovalDescription(latestPendingApproval.description)}
                  </span>
                )}
              </div>
              {latestPendingApproval?.id && onOpenApproval ? (
                <div className="task-status-banner-actions">
                  ...
                  >
                    ...
                    {pendingApprovalSummary.count > 1
                      ? translate("task.status.reviewApprovals", "Review approvals ({count})", {
                          count: pendingApprovalSummary.count,
                        })
                      : translate("task.status.reviewApproval", "Review approval")}
                  </button>
                </div>
              ) : null}
            </div>
          ...
          {task.status === "interrupted" && task.terminalStatus === "resume_available" && (
            <div className="task-status-banner task-status-banner-paused">
              <div className="task-status-banner-content">
                <strong>{translate("task.status.resumeAvailable", "Resume available")}</strong>
                <span className="task-status-banner-detail">
                  {translate(
          ...
          {task.status === "completed" &&
            ...
            showPersistentNeedsUserActionBanner && (
              <div className="task-status-banner task-status-banner-blocked">
                <div className="task-status-banner-content">
                  <strong>
                    {translate("task.status.completedNeedsAction", "Completed - action required")}
                  </strong>
                  <span className="task-status-banner-detail">
          ...
          {quotedAssistantMessage && (
            <div className="composer-quoted-assistant">
              ...
              <button
                ...
                title={translate("messageActions.removeQuoted", "Remove quoted message")}
                aria-label={translate("messageActions.removeQuoted", "Remove quoted message")}
              >
                <svg
                  width="14"
                  height="14"
              ...
              {composerModeSelection !== "chat" && (
                ...
                {showPermissionDropdown && (
                  ...
                  >
                    ...
                    >
                      <ShieldCheck size={16} aria-hidden="true" />
                      <span>{translate("composer.defaultPermissions", "Default permissions")}</span>
                    </button>
                    <button
                      type="button"
                      className={`permission-access-option danger ${
                    ...
                    >
                      <Terminal size={16} aria-hidden="true" />
                      <span>{translate("composer.allowCommands", "Allow command execution")}</span>
                      <span
                        className={`goal-mode-switch-track ${
                          shellEnabled || permissionAccessMode === "full" ? "on" : ""
                        }`}
          ...
          <div className="input-below-actions">
            ...
            {!isPreparingMessage && (
              <span className="keyboard-hint">
                <span>
                  {translate("composer.keyboardHint", "Enter to send · Shift+Enter for new line")}
                </span>
              </span>
            )}
          </div>
