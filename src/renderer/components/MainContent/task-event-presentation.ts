...
export function shouldShowTimelineControls(params: {
  hasNonConversationEvents: boolean;
  isTaskWorking: boolean;
  isTaskFinished: boolean;
}): boolean {
  return (
    params.hasNonConversationEvents ||
    params.isTaskWorking ||
    params.isTaskFinished
  );
}

export type TimelineControlsPlacement =
  | "hidden"
  | "initial-query"
  | "latest-query"
  | "standalone";

export function resolveTimelineControlsPlacement(params: {
  showTimelineControls: boolean;
  hasInitialPrompt: boolean;
  hasUserFollowUp: boolean;
}): TimelineControlsPlacement {
  if (!params.showTimelineControls) return "hidden";
  if (params.hasUserFollowUp) return "latest-query";
  if (params.hasInitialPrompt) return "initial-query";
  return "standalone";
}
