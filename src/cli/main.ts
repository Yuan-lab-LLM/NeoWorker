...
async function remoteAgentSecurity(ctx: CommandContext, sub: string): Promise<number> {
  ...
  try {
    ...
    if (sub === "status") {
      ...
      if (!findingId || !status) {
        return usageError(
          "Usage: neoworker security finding <findingId> open|acknowledged|resolved|false_positive --remote",
        );
      }
      ...
      if (!agent || !["status", "install", "uninstall"].includes(action)) {
        return usageError(
          "Usage: neoworker security hooks status|install|uninstall <agent> [--yes] --remote",
        );
      }
      ...
      if (action === "build") {
        ...
        if (!identifier || !taskId) {
          return usageError(
            "Usage: neoworker security case build <caseId> --task-id <taskId> --remote",
          );
        }
        ...
        return usageError(
          "Usage: neoworker security case build <caseId> --task-id <taskId> | case verify <bundleName> --remote",
        );
      }
    } else {
