...
const SKILLS_DIR = path.join(PROJECT_ROOT, 'resources', 'skills');
...
function main(): void {
  ...
  // Remove explicitly retired skills from the generated registry. Other
  // registry-only entries are intentionally preserved for compatibility.
  for (const registryFile of fs.readdirSync(SKILLS_OUTPUT_DIR)) {
    if (
      registryFile.endsWith('.json') &&
      RETIRED_SKILL_IDS.has(path.basename(registryFile, '.json'))
    ) {
      fs.unlinkSync(path.join(SKILLS_OUTPUT_DIR, registryFile));
    }
  }
