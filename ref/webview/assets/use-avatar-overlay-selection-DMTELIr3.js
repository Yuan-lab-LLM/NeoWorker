...
import { i as it, n as at } from "./custom-avatars-query-mkYIdPJ1.js";
function ot(e, t) {
  switch (e.kind) {
    case `thread-created`:
      return {
    ...
    return [
      {
        ...
        notificationPreferenceId:
          e.task.pendingWorktreeId == null
            ? B(e.task.threadId, e.task.hostId)
            : null,
    ...
    case `tool`:
      switch (n.tool.type) {
        ...
        case `mcp`:
          ...
          break;
        case `dynamic`:
          i = J(n.tool.tool, { style: `sentence` });
          break;
        case `web-search`:
          i = n.tool.query.trim()
            ? e.formatMessage(
...
function vt() {
  let e = S(Ne),
    t = xe(),
    { accountId: r, authMethod: a, userId: o } = he(),
    s = ie(),
    { activeMode: ee } = C(),
    {
      agentMode: te,
      ...
      conversationId: null,
      cwdOverride: s.cwd ?? void 0,
      hostId: s.hostId,
    }),
    { serviceTierSettings: se } = Me(),
    p = Ce() && !0,
    { capabilityNames: le } = ce({ enabled: p, hostId: s.hostId, root: s.cwd }),
    h = D(Ae),
    g = D(ke) != null,
    _ = p && h?.session.greetingEnabled === !0 && a === `chatgpt`,
    { data: ue, isLoading: v } = Se({ accountId: r, enabled: _, userId: o }),
    y = D(u),
    b = D(i),
    x = D(f),
    fe = D(m),
    pe = D(Qe),
    w = D(O),
    [T] = c(`statsig_default_enable_features`),
    [_e, A] = (0, $.useState)(!1),
    [j, M] = (0, $.useState)(null),
    P = (0, $.useRef)(!1),
    F = (0, $.useRef)(!1),
    ...
    K = !s.isActiveWorkspaceRootLoading && !l,
    q = G && K,
    J = () => {
      e.get(u) == null &&
        e.get(O) === `inactive` &&
        (A(!1), k.realtimeVoiceRuntime?.cancelRealtimeSessionStart());
    },
    Y = (n) => {
      e.get(de).danger(
    ...
    Pe = (0, $.useEffectEvent)(async (t) => {
      let r = n(t, Xe),
        i = r.type === `exact`,
        a = r.type === `new`,
        o = i ? r.locator.hostId : r.hostId,
        c = t.preferredPresentationSurface === `main-thread`;
      if (a && y != null) {
        L.current = !0;
        try {
          await E.stop(e, y);
        ...
        let t = e.get(Re),
          n =
            t.phase !== `inactive` && t.locator.conversationId === y
              ? t.locator
              : { conversationId: y, hostId: o };
        (be({ locator: n, surface: `main-thread` }),
          je.dispatchMessage(`open-in-main-window`, {
            path: z(n.conversationId),
          }));
        return;
      }
      if (
      ...
      let l = h,
        d =
          t.source === `composer_button_existing_thread`
            ...
            : Ie(le),
        ie =
          d == null ? l.session.basePrompt : `${l.session.basePrompt}\n\n${d}`,
        ce =
          l.session.greetingEnabled && t.source !== `global_hotkey_new_thread`
            ? (ue?.displayName ?? null)
            : null,
        g = crypto.randomUUID(),
        _ = re(t.source);
      (N(e, U, {
        action:
          We.CODEX_REALTIME_VOICE_SESSION_LIFECYCLE_ACTION_START_REQUESTED,
        sessionId: g,
        startSource: _,
      }),
        A(c),
      ...
      try {
        ...
        (await Te({
          ...
          activateRealtimeConversation: async (n, r) => {
            ...
            if (
              I.current !== v ||
              (r != null && Ee(e, n, r, void 0),
              T?.concurrent_reasoning_summaries === !0 &&
                (await Oe(`update-thread-settings-for-next-turn`, {
                  conversationId: n,
              ...
              return `cancelled`;
            S = !0;
            let a = await E.start(e, {
              analyticsSessionId: g,
              codexResponseHandoffPrefix: l.session.codexResponseHandoffPrefix,
              codexResponseItemPrefix: l.session.codexResponseItemPrefix,
              codexResponsesAsItems: l.session.codexResponsesAsItems,
              conversationId: n,
              hostId: o,
              initiallyMicrophoneMuted: P.current,
              initiallyMuted: F.current,
              microphone: i,
              outputModality: `audio`,
              preferredPresentationSurface: c
                ? `main-thread`
                : `global-overlay`,
              prompt: ie,
              realtimeContinuity: l.session.continuity,
              realtimeContinuityPrompt: l.session.continuityPrompt,
              realtimeMemorySummaryEnabled:
                l.session.memorySummaryEnabled && e.get(ge, n) === !0,
              realtimeMemorySummaryPrompt: l.session.memorySummaryPrompt,
              realtimeSessionOverrides: l.session.overrides,
              source: t.source,
              userName: ce,
            });
            return (
              ...
              a === `active`
                ? I.current === v
                  ? (c &&
                      (be({
                        locator: { conversationId: n, hostId: o },
                        surface: `main-thread`,
                      }),
                      t.source !== `composer_button_existing_thread` &&
                        je.dispatchMessage(`open-in-main-window`, {
                    ...
                    `active`)
                  : (E.cancelStart(e), `cancelled`)
                : a
            );
          },
          agentMode: te,
          currentLocalExecutionCwd: s.cwd ?? ``,
          intent: r,
          memoryPreferences: n,
          onStartError: (e) => {
            c || Y(e);
          },
          serviceTier: se.serviceTierForRequest,
          permissionProfileId: ae,
          shouldSendPermissionOverrides: oe,
          scope: e,
          treatment: l,
          threadToolsEnabled: T?.[ne] === !0,
          workspaceRootsForLocalExecution: [],
        }),
          C ||
            N(e, U, {
              action:
                We.CODEX_REALTIME_VOICE_SESSION_LIFECYCLE_ACTION_START_CANCELLED,
              sessionId: g,
              startSource: _,
            }));
      } catch (t) {
        throw (
          ...
          C ||
            N(e, U, {
              action:
                We.CODEX_REALTIME_VOICE_SESSION_LIFECYCLE_ACTION_START_FAILED,
              sessionId: g,
              startSource: _,
            }),
          t
        ...
        (!S &&
          ...
          x
            ...
            .catch(() => {}),
          I.current === v && (V(!1), de || J()));
      }
    }),
    X = async () => {
      ((I.current += 1), j?.stop(), M(null), V(!1), A(!1));
      let t = e.get(u);
      (E.cancelStart(e), t ?? (e.set(f, !1), e.set(m, !1)));
    },
    Z = (0, $.useEffectEvent)(X),
    Fe = async () => {
      if (q)
        try {
          await k.realtimeVoiceRuntime?.requestRealtimeStart(
  ...
  ((0, $.useEffect)(() => {
    ...
    De(`avatar-overlay-explicit-pet-opened`, () => {
      A(!1);
    }),
    (0, $.useLayoutEffect)(() => {
      if (w !== `inactive`) {
      ...
      if (R.current) {
        ...
        ((R.current = !1), k.realtimeVoiceRuntime?.completeRealtimeSession());
      }
    }, [B, w]));
  let Le = async () => {
      if (B) {
      ...
      y == null || w === `inactive` || (await E.stop(e, y));
    },
    ze = () => {
      if (H) {
        ((F.current = !e.get(m)),
          y == null ? e.set(m, F.current) : E.toggleMute(e, y));
        return;
      }
      y == null ||
        w !== `active` ||
        ((F.current = !e.get(m)), E.toggleMute(e, y));
    },
    Be = () => {
      if (H) {
        let t = !e.get(f);
        ((P.current = t),
          y == null
            ? (j.setMuted(t), e.set(f, t))
            : E.toggleMicrophoneMute(e, y));
        return;
      }
      y == null ||
        w !== `active` ||
        ((P.current = !e.get(f)), E.toggleMicrophoneMute(e, y));
    },
    Q = w;
  return (
    H
      ...
      await window.electronBridge?.showContextMenu?.([
        {
          ...
          label: e.formatMessage(H.muteThreadNotifications),
        },
      ])
...
function tn() {
  let e = (0, nn.c)(11),
    { selectedAvatar: t, selectedAvatarId: n } = a(),
    { data: r, isFetching: i } = h(at, n ?? t.id);
  ...
  return (
    e[8] !== n || e[9] !== o
      ? ((s = { selectedAvatar: o, selectedAvatarId: n }),
        (e[8] = n),
        (e[9] = o),
        (e[10] = s))
      : (s = e[10]),
...
//# sourceMappingURL=use-avatar-overlay-selection-DMTELIr3.js.map
