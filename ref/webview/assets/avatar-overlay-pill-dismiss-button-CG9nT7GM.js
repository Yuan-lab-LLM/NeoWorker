...
var b,
  ...
  T,
  pe = e(() => {
    ((b = `_cssMaterial_1lmj6_1`),
      (x = `_activityPillMaterial_1lmj6_15`),
      (S = `_cssMaterialElevated_1lmj6_22`),
      (C = `_replyStopControl_1lmj6_40`),
      (w = `_cssControl_1lmj6_52`),
      (T = {
        cssMaterial: b,
        activityPillMaterial: x,
        cssMaterialElevated: S,
        replyStopControl: C,
        cssControl: w,
      }));
  });
function me(e, t, n = 1) {
  ...
  if (Math.hypot(r, i) <= A) return null;
  let a = (Math.atan2(r, -i) * (180 / Math.PI) + 360) % 360,
    o = Math.round(a / E) % D;
  return {
    columnIndex: o % k,
    frameDurationMs: 0,
    rowIndex: O + Math.floor(o / k),
  };
}
var E,
  D,
...
function Ae({
  conversation: e,
  includeCompactWaitingRequests: n,
  includeMcpElicitationCancelAction: r,
  intl: i,
  excludedConversationId: a,
}) {
  if (ee(e)) return null;
  let s = e.hostId ?? `local`,
    c = s === `local` ? `local` : `remote-host`,
    u = Fe(e),
    d = e.threadSource === `automation` || !1,
    f = d ? Ne(e) : Me(e, i);
  if (
    (e.threadSource === `realtime_voice` || e.id === a) &&
    u !== `waiting` &&
    u !== `failed`
  )
    return null;
  let p = n && u === `waiting` ? l(e) : null;
  return {
    ...
    showInNotificationTray:
      !d || u === `waiting` || u === `failed` || f != null,
    sortAtMs: v(e)?.turnStartedAtMs ?? e.updatedAt,
    status: u,
    subtitle: f,
    title: t(e) ?? i.formatMessage(X.newThread),
    turnKey: String(o(e)),
    updatedAtMs: e.updatedAt,
    waitingRequest: L(p, i, {
      includeMcpElicitationCancelAction: r,
  ...
  let t = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    n =
      e.resumeState === `needs_resume`
        ? t?.type === `active`
        : e.resumeState === `resuming` || v(e)?.status === `inProgress`,
    r =
      e.resumeState === `needs_resume`
...
//# sourceMappingURL=avatar-overlay-pill-dismiss-button-CG9nT7GM.js.map
