...
async function Le({
  ...
  let s = ae.pullRequestMessageGeneration;
  if (s == null) throw new k(`unsupported`);
  Be(n, i);
  let c = await Re(`gh-pr-diff`, i, a);
  if (c.status === `error`) throw new k(`diff_failed`, c.error);
  if (c.headRevision !== n.headRevision) throw new k(`head_changed`);
  a?.throwIfAborted();
  let l;
  try {
    l = await Fe(
    ...
    throw (a?.throwIfAborted(), new k(`generation_failed`, e));
  }
  let u = l?.body.trim();
  if (!u) throw new k(`empty_result`);
  let d = await Re(`gh-pr-status`, i, a);
  if (d.status === `not-found`) throw new k(`status_failed`);
  if (d.canonical !== !0 || (Be(d, i), d.baseBranch !== n.baseBranch))
    throw new k(`target_changed`);
  if (d.headRevision !== c.headRevision) throw new k(`head_changed`);
  return u;
}
async function Re(e, t, n) {
  try {
    return await D(e, t, { signal: n, source: `pull_requests_page` });
  } catch (t) {
    throw (
      n?.throwIfAborted(),
      new k(e === `gh-pr-diff` ? `diff_failed` : `status_failed`, t)
    );
  }
}
function ze({
...
function Be(e, t) {
  if (
    m(e.pullRequest) !== m(t.pullRequest) ||
    te(t.account.hostname) !== te(t.pullRequest.hostname) ||
    (e.isAuthor &&
      e.authorLogin != null &&
...
async function A(e, t, n, r) {
  (await e
    ...
    .execute(void 0),
    r != null &&
      e.setQueryData(d(`gh-pr-status`, t), (e) =>
        e?.status === `success` && e.canonical === !0 ? { ...e, body: r } : e,
      ));
  let i = Promise.allSettled([
    e.invalidateQueries({ exact: !0, queryKey: d(`gh-pr-status`, t) }),
    e.invalidateQueries({ queryKey: Ce(t.account) }),
  ]);
  r ?? (await i);
}
var qe,
  ...
  Ze = e(() => {
    (v(),
      ...
      (j = x(T, ({ request: e, submissionKey: t }) => ({
...
function nt(e, t, n, r) {
  ...
  (e.set(z, { path: i, request: t }, r), at(e, t, i, r));
  let o = e.get(H, t);
  if (o.data?.status === `success`)
    for (let n of a) {
      let r = S(o.data.headRevision, n),
        a = e.get(B, { path: i, request: t });
      a.some((e) => (0, F.default)(e, r)) ||
        (e.set(B, { path: i, request: t }, [...a, r]), ot(e, t, i, r, n));
    }
}
function rt(e, t, n, r) {
  let i = it(e, t, n);
  ...
  e.get(j, { request: t, submissionKey: r }).isPending || ot(e, t, i, r, a);
}
function it(e, t, n) {
  let r = b(n);
  return e.get(W, t).find((e) => e.path === r)?.path ?? null;
}
function at(e, t, n, r) {
  ...
  e.set(
    ...
    { path: n, request: t },
    e
      .get(B, { path: n, request: t })
      .filter(
        (n) =>
          i.has(n.commentKey) ||
          e.get(j, { request: t, submissionKey: n }).isPending,
      ),
  );
}
function ot(e, t, n, r, i) {
  e.get(j, { request: t, submissionKey: r }).mutate(i, {
    onSuccess: () => {
      (e.set(
        z,
        { path: n, request: t },
        e
          ...
          .filter((e) => !(0, F.default)(S(r.revision, e), r)),
      ),
        st(e, t, n, r));
    },
  });
...
function st(e, t, n, r) {
  e.set(
    B,
    { path: n, request: t },
    e.get(B, { path: n, request: t }).filter((e) => !(0, F.default)(e, r)),
  );
}
var F,
  ct,
  ...
  pt = e(() => {
    ((F = t(s(), 1)),
      ...
      (L = C(T, !1)),
      (R = C(T, 0)),
      (z = Ae(T, (e) => [], { isEqual: F.default })),
      (B = Ae(T, (e) => [], { isEqual: F.default })),
      (V = E(T, (e, { get: t }) =>
        t(Se, { request: e, source: `pull_requests_page` }),
      )),
      (H = E(T, (e, { get: t }) => {
        let n = t(V, e).data?.headRevision ?? null,
          i = t(h, { headRevision: n, request: e }),
          a = t(c, r(e, `diff`));
      ...
      (U = E(T, (e, { get: t }) => {
        let n = t(H, e);
        return n.data?.status === `success` ? ve(n.data.unifiedDiff) : void 0;
      })),
      (W = E(
        T,
        (e, { get: t }) =>
          (t(U, e) ?? []).map((e) => {
            let t = b(e.metadata.name);
            return { changeKind: i(e.metadata.type), displayPath: t, path: t };
          }),
        { isEqual: F.default },
      )),
      (G = E(
      ...
      (K = E(
        ...
        ({ path: e, request: t }, { get: n }) => {
          let r = b(e);
          return n(V, t).data?.commentAttachments?.filter(
            (e) => b(e.position.path) === r,
          );
        },
        { isEqual: F.default },
      )),
      (ut = E(
        ...
        (e, { get: t }) =>
          new Map(
            t(W, e).map((n) => [
              n.path,
              (t(K, { path: n.path, request: e }) ?? []).length,
            ]),
          ),
        { isEqual: F.default },
      )),
      (dt = E(
        ...
        ({ path: e, request: t }, { get: n }) =>
          new Set(
            n(B, { path: b(e), request: t }).flatMap((e) =>
              n(j, { request: t, submissionKey: e }).isPending
                ? [e.commentKey]
                : [],
            ),
          ),
      ...
      (ft = E(
        ...
        ({ path: e, request: t }, { get: n }) =>
          n(B, { path: b(e), request: t }).flatMap((e) => {
            let r = n(j, { request: t, submissionKey: e }).error;
...
function mt(e) {
  ...
  if (gt(e)) {
    ...
    return t == null
      ? !1
      : (J(e, q({ ...t, block: !0, mediaKind: `image`, raw: e.raw })), !0);
  }
  return !(`tokens` in e) || !Array.isArray(e.tokens)
    ? !1
    : e.tokens.reduce((e, t) => mt(t) || e, !1);
}
function q({ alt: e, block: t, mediaKind: n, raw: r, title: i, url: a }) {
  let o = { alt: e, kind: n, src: a };
  return (
    i != null && (o.title = i),
    { type: `codexDirective`, raw: r, name: Ct, attributes: o, block: t }
  );
}
function ht(e) {
  let t = e.tokens[0];
...
function bt(e) {
  ...
  let t = (0, X.parse)(e.raw).childNodes.filter(
      (e) => e.nodeType !== X.NodeType.TEXT_NODE || e.rawText.trim().length > 0,
    ),
    n = t[0];
  if (t.length !== 1 || !(n instanceof X.HTMLElement) || n.tagName !== `IMG`)
    return null;
...
function Dt(e) {
  ...
  if (n == null) {
    let e;
    t[8] === r ? (e = t[9]) : ((e = re(r)), (t[8] = r), (t[9] = e));
    let n;
    return (
  ...
  t[15] === i
    ? (l = t[16])
    : ((l = { ...i, [Ct]: At }), (t[15] = i), (t[16] = l));
  let u;
  t[17] === r ? (u = t[18]) : ((u = re(r)), (t[17] = r), (t[18] = u));
  let d;
...
function Ot(e) {
  ...
  if (l.isLoading) {
    ...
    t[3] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((e = (0, $.jsx)(fe, { className: `icon-sm` })), (t[3] = e))
      : (e = t[3]);
    let n;
    return (
  ...
  if (d === s || l.data?.status !== `success` || !jt(l.data.mimeType, a)) {
    let e;
    return (
      t[5] === s
        ? (e = t[6])
  ...
  let m = `data:${l.data.mimeType};base64,${l.data.contentsBase64}`;
  if (a === `video`) {
    let e;
    t[7] !== r || t[8] !== u
      ? ((e =
    ...
    let c;
    return (
      t[14] !== m || t[15] !== e || t[16] !== n || t[17] !== a || t[18] !== o
        ? ((c = (0, $.jsx)(`video`, {
            "aria-label": e,
            className: n,
            controls: !0,
  ...
  let _;
  return (
    t[24] !== r || t[25] !== m || t[26] !== h || t[27] !== g || t[28] !== o
      ? ((_ = (0, $.jsx)(`img`, {
          alt: r,
          className: h,
          loading: `lazy`,
...
function kt(e) {
  ...
  t[1] === n
    ? (i = t[2])
    : ((i = (e) => {
        le({ event: e, href: n, initiator: `open_in_browser_bridge` });
      }),
      (t[1] = n),
      (t[2] = i));
  let a;
  ...
  return (
    t[4] === i
      ...
      : ((o = (0, $.jsxs)(`div`, {
          ...
          children: [
            r,
            (0, $.jsx)(xe, { color: `outline`, onClick: i, children: a }),
          ],
        })),
        (t[4] = i),
        (t[5] = o)),
...
//# sourceMappingURL=pull-request-media-Bg1EcPt4.js.map
